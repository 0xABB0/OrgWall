...
static bool sponza_scan(Sponza_Scan_Result* out, const Mel_Alloc* alloc)
{
    assert(out != nullptr);
    assert(alloc != nullptr);
...
static bool sponza_load(Sponza_Load_Result* out, Mel_Geometry_Pool* pool,
                        Mel_Material_Base_Id forward_lit_id, const Mel_Alloc* alloc)
{
    Sponza_Gltf gltf = {0};
    if (!sponza_gltf_open(&gltf, alloc))
        return false;
    ...
    for (u32 i = 0; i < material_count; i++)
    {
        ...
        {
            mel_log_warn("demo.sponza", "material %u uses alphaMode=BLEND; current scene_forward path keeps it opaque", i);
        }
...
static void sponza_update(Mel_Sim_Ctx* sim, f32 dt, void* user)
{
    ...
    mel_render_view_set_camera(s_view, (Mel_Render_Camera){
        ...
        .projection = mel_mat4_perspective(60.0f * (3.14159265f / 180.0f), aspect, 0.1f, s_camera_far_plane),
        .viewport = {{ 0, 0, 1, 1 }},
...
void app_init(void)
{
    ...
    mel_geometry_pool_init(&s_geo_pool,
        ...
        .vertex_capacity = (u64)scan.vertex_count * sizeof(Sponza_Vertex),
    ...
    s_camera_target = transform_point3(loaded.model, loaded.bounds.center);
    s_camera_extents = transform_extents3(loaded.model, loaded.bounds.extents);
    f32 far_span = s_camera_extents.x;
    if (s_camera_extents.y > far_span) far_span = s_camera_extents.y;
    if (s_camera_extents.z > far_span) far_span = s_camera_extents.z;
    s_camera_far_plane = far_span * 6.0f;
    if (s_camera_far_plane < 100.0f)
        s_camera_far_plane = 100.0f;
