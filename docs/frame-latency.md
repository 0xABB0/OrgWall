...
- **`sensor`** (consumer-side) — `latency_get_report` does not consult sensors directly, but the app's content model joins thermal pressure (`sensor.thermal`) and power source (`sensor.power`) against the latency report to decide whether to engage Reflex `OnBoost`, drop to platform-native, or disable frame generation under sustained thermal pressure. The engine reports; the app decides (MEL-ENGINE-V).
