...
{
    Mel_Fd result = open(path, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (result < 0)
    {
        mel_log_error("process", "could not open file %s: %s", path, strerror(errno));
        return MEL_INVALID_FD;
...
void mel_fd_close(Mel_Fd fd) { close(fd); }

i32 mel_nprocs(void) { return (i32)sysconf(_SC_NPROCESSORS_ONLN); }
