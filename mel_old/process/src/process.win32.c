...
static char* mel_win32_error_message(DWORD err)
{
    ...
    DWORD       size = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, err, LANG_USER_DEFAULT, msg, MEL_WIN32_ERR_MSG_SIZE, NULL);
...
Mel_Proc mel__cmd_start_process__platform(Mel_Cmd cmd, Mel_Fd* fdin, Mel_Fd* fdout, Mel_Fd* fderr)
{
    ...
    if (!bSuccess)
    {
        mel_log_error("process", "Could not create child process for %s: %s", cmd.items[0], mel_win32_error_message(GetLastError()));
...
i32 mel__proc_wait_async(Mel_Proc proc, i32 ms)
{
    ...
    {
        mel_log_error("process", "could not wait on child process: %s", mel_win32_error_message(GetLastError()));
        return -1;
    }
    ...
    {
        mel_log_error("process", "could not get process exit code: %s", mel_win32_error_message(GetLastError()));
        return -1;
    }
...
Mel_Fd mel_fd_open_for_read(const char* path)
{
    ...
    Mel_Fd result = CreateFileA(path, GENERIC_READ, 0, &saAttr, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, NULL);

    if (result == INVALID_HANDLE_VALUE)
    {
        mel_log_error("process", "Could not open file %s: %s", path, mel_win32_error_message(GetLastError()));
...
Mel_Fd mel_fd_open_for_write(const char* path)
{
    ...
    Mel_Fd result = CreateFileA(path, GENERIC_WRITE, 0, &saAttr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void mel_fd_close(Mel_Fd fd) { CloseHandle(fd); }
