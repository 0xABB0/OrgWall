...
bool mel_cmd_run_opt(Mel_Cmd* cmd, Mel_Cmd_Opt opt)
{
    ...
    {
        if (!mel_proc_wait(proc))
            result = false;
    }

    if (opt_fdin)
        mel_fd_close(*opt_fdin);
    if (opt_fdout)
        mel_fd_close(*opt_fdout);
    if (opt_fderr)
        mel_fd_close(*opt_fderr);
    if (!opt.dont_reset)
        cmd->count = 0;
    return result;
}
