    ...
    for (size i = 0; i < src_len && out < dst_cap - 1;)
    {
        ...
        if (c == '/')
        {
            ...
            if (i + 1 < src_len && src[i + 1] == '.')
            {
                ...
                if (i + 2 < src_len && src[i + 2] == '.' && (i + 3 >= src_len || src[i + 3] == '/' || src[i + 3] == '\\'))
                {
                    if (out > 1)
                    {
                        out--;
...
static bool mel__vfs_starts_with(str8 path, str8 prefix)
{
    ...
    if (prefix.len == 1 && prefix.data[0] == '/')
        return true;
    if (memcmp(path.data, prefix.data, (size_t)prefix.len) != 0)
        return false;
...
static Mel__Vfs_Mount* mel__vfs_find_mount(str8 normalized_path, bool need_write)
{
    ...
    for (u32 i = 0; i < s_vfs.mount_count; i++)
    {
        ...
        if (!m->active)
            continue;
        if (need_write && !m->writable)
            continue;
        if (!mel__vfs_starts_with(normalized_path, m->prefix))
            continue;

        if (!best || m->prefix.len > best->prefix.len || (m->prefix.len == best->prefix.len && m->priority > best->priority))
        {
            best = m;
        }
    }
...
__attribute__((constructor(280))) void mel_vfs_init_default(void) { mel_vfs_init(mel_alloc_heap()); }

void mel_vfs_init(const Mel_Alloc* alloc)
{
    if (s_vfs.initialized)
        return;
    assert(alloc != NULL);
...
__attribute__((destructor(280))) void mel_vfs_shutdown(void)
{
    if (!s_vfs.initialized)
        return;
...
bool mel_vfs_write_file(str8 path, const void* data, i64 size)
{
    ...
    i32 fd = mount->backend->open(mount->backend, phys, MEL_VFS_OPEN_WRITE | MEL_VFS_OPEN_CREATE | MEL_VFS_OPEN_TRUNCATE);
    if (fd < 0)
        return false;
