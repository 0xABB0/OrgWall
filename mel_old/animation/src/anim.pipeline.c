...
_Static_assert(sizeof(mel__easing_table) / sizeof(mel__easing_table[0]) == MEL_EASING_COUNT, "easing table size mismatch");

static f32 mel__resolve_easing(u16 easing_id, f32 t, const f32* easing_params, u32 keyframe_index)
{
...
void mel_anim_sample(const Mel_Anim_Clip* clip, f32 current_time, u32* cursors, const Mel_Alloc* scratch, Mel_Local_Pose* out_pose)
{
    assert(clip != NULL);
    assert(cursors != NULL);
    assert(scratch != NULL);
    ...
    for (u32 g = 0; g < clip->group_count; g++)
    {
        ...
        for (u32 t = 0; t < tc; t++)
        {
            ...
            *cur = left;
            ...
            f32 t_eased = mel__resolve_easing(easing_id, t_local, grp->flat_easing_params, data_off + left);
...
void mel_anim_blend(Mel_Local_Pose* pose_a, const Mel_Local_Pose* pose_b, f32 weight, const u64* mask_hashes, u32 mask_count)
{
    assert(pose_a != NULL);
    assert(pose_b != NULL);
...
void mel_anim_blend_additive(Mel_Local_Pose* base_pose, const Mel_Local_Pose* additive_pose, const Mel_Local_Pose* reference_pose, f32 weight, const u64* mask_hashes, u32 mask_count)
{
    ...
    for (u32 gb = 0; gb < base_pose->group_count; gb++)
    {
        ...
        for (u32 g = 0; g < additive_pose->group_count; g++)
        {
            ...
            {
                grp_add = &additive_pose->groups[g];
                break;
            }
        }
        if (!grp_add)
        ...
        for (u32 g = 0; g < reference_pose->group_count; g++)
        {
            ...
            {
                grp_ref = &reference_pose->groups[g];
                break;
            }
        }
        if (!grp_ref)
        ...
        for (u32 ib = 0; ib < grp_base->count; ib++)
        {
            ...
            {
                if (grp_add->property_ids[i] == prop)
                {
                    idx_add = (i32)i;
                    break;
                }
            ...
            {
                if (grp_ref->property_ids[i] == prop)
                {
                    idx_ref = (i32)i;
                    break;
                }
