...
static const Mel_Anim_Clip* mel__player_resolve(Mel_Anim_Player* player, Mel_Anim_Clip_Handle handle)
{
...
static Mel_Anim_Clip_State mel__player_make_state(Mel_Anim_Player* player, Mel_Anim_Clip_Handle clip_handle, f32 start_time, f32 speed, f32 xfade_duration)
{
...
void mel_anim_player_init(Mel_Anim_Player* player, const Mel_Alloc* alloc, Mel_Anim_Clip_Pool* clip_pool)
{
    assert(player != NULL);
    assert(alloc != NULL);
...
void mel_anim_player_play_opt(Mel_Anim_Player* player, Mel_Anim_Clip_Handle clip, Mel_Anim_Play_Opt opt)
{
    assert(player != NULL);
    ...
    {
        Mel_Anim_Clip_State new_state = mel__player_make_state(player, clip, opt.start_time, opt.speed, opt.crossfade);
    ...
    else
    {
        ...
        Mel_Anim_Clip_State new_state = mel__player_make_state(player, clip, opt.start_time, opt.speed, 0.0f);
        mel__player_push_chain(player, new_state);
    }
}

static void mel__player_collect_events(Mel_Anim_Player* player, const Mel_Anim_Clip* clip, f32 prev_time, f32 new_time)
{
...
void mel_anim_player_update(Mel_Anim_Player* player, f32 dt)
{
    ...
    if (prune_to > 0)
    {
        ...
        memmove(&player->chain[0], &player->chain[prune_to], sizeof(Mel_Anim_Clip_State) * remaining);
    ...
    player->phase = (active_clip->duration > 0.0f) ? active->time / active_clip->duration : 0.0f;
}

void mel_anim_player_sample(Mel_Anim_Player* player, const Mel_Alloc* scratch, Mel_Local_Pose* out_pose)
{
    assert(player != NULL);
    assert(scratch != NULL);
    ...
    for (u32 i = 1; i < player->chain_count; i++)
    {
        ...
        f32 alpha = (state->xfade_duration > 0.0f) ? state->xfade_elapsed / state->xfade_duration : 1.0f;
        if (alpha > 1.0f)
            alpha = 1.0f;
...
void mel_anim_player_get_float(Mel_Anim_Player* player, u64 property_id, const Mel_Alloc* scratch, f32* out)
{
    Mel_Local_Pose pose;
...
void mel_anim_player_get_vec2(Mel_Anim_Player* player, u64 property_id, const Mel_Alloc* scratch, void* out)
{
    Mel_Local_Pose pose;
    mel_anim_player_sample(player, scratch, &pose);
...
void mel_anim_player_get_vec3(Mel_Anim_Player* player, u64 property_id, const Mel_Alloc* scratch, void* out)
{
    Mel_Local_Pose pose;
    mel_anim_player_sample(player, scratch, &pose);
...
void mel_anim_player_get_vec4(Mel_Anim_Player* player, u64 property_id, const Mel_Alloc* scratch, void* out)
{
    Mel_Local_Pose pose;
    mel_anim_player_sample(player, scratch, &pose);
...
void mel_anim_player_get_quat(Mel_Anim_Player* player, u64 property_id, const Mel_Alloc* scratch, void* out)
{
    Mel_Local_Pose pose;
    mel_anim_player_sample(player, scratch, &pose);
...
void mel_anim_player_extract_root_motion(Mel_Anim_Player* player, const Mel_Local_Pose* pose, const Mel_Skeleton* skeleton, Mel_Root_Motion* out)
{
    assert(player != NULL);
    assert(pose != NULL);
