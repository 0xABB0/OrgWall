...
__attribute__((constructor(250))) void mel_aio_init(void)
{
...
__attribute__((destructor(250))) void mel_aio_shutdown(void)
{
    if (!atomic_load(&s_initialized))
        return;
...
void mel_aio_submit(Mel_Aio_Op* op)
{
    ...
    dispatch_io_t channel = dispatch_io_create(DISPATCH_IO_RANDOM, op->fd, s_dispatch_queue, ^(int error) {
        (void)error;
    });
    ...
    dispatch_io_read(channel, (off_t)op->offset, (size_t)op->size, s_dispatch_queue, ^(bool done, dispatch_data_t data, int error) {
        if (!done)
            return;

        if (error != 0)
        {
            if (captured_op->result)
                *captured_op->result = -1;
            if (captured_op->error)
                *captured_op->error = error;
        }
        else if (data != NULL)
        {
            ...
            if (captured_op->result)
                *captured_op->result = total_copied;
            if (captured_op->error)
                *captured_op->error = 0;
        }
        else
