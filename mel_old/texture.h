...
bool mel_texture_load_and_bind_opt(Mel_Gpu_Texture* tex, Mel_Gpu_Device* dev, Mel_Gpu_Pipeline* pipeline, const Mel_Alloc* alloc, str8 path, Mel_Texture_Load_Opt opt);
#define mel_texture_load_and_bind(tex, dev, pipeline, alloc, path, ...) mel_texture_load_and_bind_opt((tex), (dev), (pipeline), (alloc), (path), (Mel_Texture_Load_Opt){ __VA_ARGS__ })
