...
static bool mel__font_desc_eq_key(const void* a, const void* b) { return (u64)(usize)a == (u64)(usize)b; }
...
Mel_Font_Desc_Handle mel_font_desc_load_ttf(str8 path)
{
    ...
    mel_log_info("font.desc", "loaded '%.*s' (upem=%d, ascent=%d, descent=%d)", (int)path.len, path.data, desc.units_per_em, desc.ascent, desc.descent);
