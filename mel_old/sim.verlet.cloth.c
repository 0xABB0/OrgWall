...
static inline u32 mel__cloth_idx(u32 row, u32 col, u32 width) { return row * width + col; }
...
void mel_verlet_cloth_opt(Mel_Verlet_System* sys, Mel_Verlet_Cloth_Opt opt)
{
    ...
    for (u32 r = 0; r < h; r++)
    {
        for (u32 c = 0; c < w; c++)
        {
            Mel_Vec3 pos = mel_vec3(opt.origin.x + (f32)c * spacing, opt.origin.y, opt.origin.z + (f32)r * spacing);
    ...
    for (u32 r = 0; r < h; r++)
    {
        for (u32 c = 0; c < w - 1; c++)
        {
            mel_verlet_add_distance(sys, mel__cloth_idx(r, c, w), mel__cloth_idx(r, c + 1, w), 0);
        }
    }
    ...
    for (u32 r = 0; r < h - 1; r++)
    {
        for (u32 c = 0; c < w; c++)
        {
            mel_verlet_add_distance(sys, mel__cloth_idx(r, c, w), mel__cloth_idx(r + 1, c, w), 0);
        }
    }
    ...
    if (opt.add_shear)
    {
        for (u32 r = 0; r < h - 1; r++)
        {
            for (u32 c = 0; c < w - 1; c++)
            {
                mel_verlet_add_distance(sys, mel__cloth_idx(r, c, w), mel__cloth_idx(r + 1, c + 1, w), 0);
                mel_verlet_add_distance(sys, mel__cloth_idx(r, c + 1, w), mel__cloth_idx(r + 1, c, w), 0);
            }
        }
    }
    ...
    if (opt.add_bend)
    {
        for (u32 r = 0; r < h; r++)
        {
            for (u32 c = 0; c < w - 2; c++)
            {
                mel_verlet_add_distance(sys, mel__cloth_idx(r, c, w), mel__cloth_idx(r, c + 2, w), 0);
            }
        }
        ...
        for (u32 r = 0; r < h - 2; r++)
        {
            for (u32 c = 0; c < w; c++)
            {
                mel_verlet_add_distance(sys, mel__cloth_idx(r, c, w), mel__cloth_idx(r + 2, c, w), 0);
            }
        }
    }
}
    ...
    for (u32 r = 0; r < height; r++)
    {
        for (u32 c = 0; c < width; c++)
        {
            ...
            if (c > 0 && r > 0)
            {
                ...
                avg = mel_vec3_add(avg, mel_vec3_cross(mel_vec3_sub(top, curr), mel_vec3_sub(left, curr)));
                n++;
            }
            ...
            if (c + 1 < width && r > 0)
            {
                ...
                Mel_Vec3 top = sys->particles[mel__cloth_idx(r - 1, c, width)].pos;
                avg = mel_vec3_add(avg, mel_vec3_cross(mel_vec3_sub(right, curr), mel_vec3_sub(top, curr)));
                n++;
            }
            ...
            if (c > 0 && r + 1 < height)
            {
                ...
                avg = mel_vec3_add(avg, mel_vec3_cross(mel_vec3_sub(left, curr), mel_vec3_sub(bottom, curr)));
                n++;
            }
            ...
            if (c + 1 < width && r + 1 < height)
            {
                Mel_Vec3 right = sys->particles[mel__cloth_idx(r, c + 1, width)].pos;
                Mel_Vec3 bottom = sys->particles[mel__cloth_idx(r + 1, c, width)].pos;
                avg = mel_vec3_add(avg, mel_vec3_cross(mel_vec3_sub(bottom, curr), mel_vec3_sub(right, curr)));
                n++;
            }
...
void mel_verlet_compute_tangents(Mel_Verlet_System* sys, u32 width, u32 height, Mel_Vec4* out_tangents)
{
    assert(sys);
    assert(width * height <= sys->count);
    assert(out_tangents);
    ...
    for (u32 r = 0; r < height; r++)
    {
        for (u32 c = 0; c < width; c++)
        {
            ...
            {
                Mel_Vec3 left = sys->particles[mel__cloth_idx(r, c - 1, width)].pos;
                tangent = mel_vec3_add(tangent, mel_vec3_sub(sys->particles[idx].pos, left));
                n++;
            }
            if (c + 1 < width)
            {
                Mel_Vec3 right = sys->particles[mel__cloth_idx(r, c + 1, width)].pos;
                tangent = mel_vec3_add(tangent, mel_vec3_sub(right, sys->particles[idx].pos));
                n++;
            }
            ...
            f32      handedness = mel_vec3_dot(bitangent, mel_vec3_cross(normal, (Mel_Vec3){ { 1, 0, 0 } })) < 0 ? -1.0f : 1.0f;
...
void mel_verlet_cloth_mesh(Mel_Verlet_System* sys, u32 width, u32 height, Mel_Vec3* out_positions, Mel_Vec3* out_normals, Mel_Vec2* out_uvs, Mel_Vec4* out_colors, u32* out_indices, Mel_Vec4 color)
{
    assert(sys);
    assert(width * height <= sys->count);
