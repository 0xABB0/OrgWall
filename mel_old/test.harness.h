...
#define MEL_TEST(tname, ...)                                                                                                                    \
    ...
    static Mel_Test_Entry                    mel__test_entry_##tname = { .name = #tname, .file = __FILE__, .func = test_##tname, __VA_ARGS__ }; \
    __attribute__((constructor)) static void mel__test_register_##tname(void) { mel__test_register(&mel__test_entry_##tname); }                 \
    static void                              test_##tname(void)
...
#define MEL_ASSERT_FLOAT_EQ(a, b, eps) \
    do                                 \
    {                                  \
        MEL_TEST_LOC();                \
        if (fabsf((a) - (b)) > (eps))  \
...
#define MEL_ASSERT_NULL(ptr)               \
    do                                     \
    {                                      \
        MEL_TEST_LOC();                    \
        if ((ptr) != nullptr)              \
        {                                  \
            MEL_FAIL(#ptr " is not null"); \
        }                                  \
    } while (0)

#define MEL_ASSERT_NOT_NULL(ptr)       \
    do                                 \
    {                                  \
        MEL_TEST_LOC();                \
        if ((ptr) == nullptr)          \
        {                              \
            MEL_FAIL(#ptr " is null"); \
        }                              \
    } while (0)

#define MEL_ASSERT_STR_EQ(a, b)     \
    do                              \
    {                               \
        MEL_TEST_LOC();             \
        if (strcmp((a), (b)) != 0)  \
