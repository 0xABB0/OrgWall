...
static const char* PASSTHROUGH_MSL = "#include <metal_stdlib>\n"
...
static const char* PASSTHROUGH_WGSL = "struct VsOut { @builtin(position) pos: vec4<f32>, @location(0) color: vec4<f32> };\n"
...
{
    return mel_gpu_shader_create(dev, .metal_source = PASSTHROUGH_MSL, .wgsl_source = PASSTHROUGH_WGSL, .spirv_vertex = TRIANGLE_VERT_SPV, .spirv_vertex_size = sizeof TRIANGLE_VERT_SPV,
...
Mel_Gpu_Pipeline* passthrough_pipeline(Mel_Gpu_Device* dev, Mel_Gpu_Shader* sh, Mel_Gpu_Topology topology, Mel_Gpu_Format color_format)
{
    ...
    return mel_gpu_pipeline_create(dev, .shader = sh, .topology = topology, .cull = MEL_GPU_CULL_NONE, .color_format = color_format, .vertex_layout = layout, .vertex_layout_count = 2,
