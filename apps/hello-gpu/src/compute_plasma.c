...
#include "instances_spv.h" // INSTANCES_FRAG_SPV: plain v_color passthrough, reused for the cells
...
static void* plasma_init(Mel_Gpu_Device* dev, Mel_Gpu_Swapchain* sc)
{
    ...
    // Reflection derives bindless + the 16-byte root record from the SPIR-V.
    ...
    // Device-local storage buffer: written by compute (UNORDERED_ACCESS), read by
    // the vertex stage (SHADER_RESOURCE). Lives in the heap from creation.
...
static void plasma_render(void* state, Mel_Gpu_Command_List* cmd, f64 dt)
{
    ...
    // Transition the buffer into UNORDERED_ACCESS for the compute write. First
    // frame the buffer is COMMON; thereafter the prior frame left it in
    // SHADER_RESOURCE (the vertex read), and that is the real read-after-write edge.
    ...
    // Compute: evaluate the plasma into the storage buffer.
    ...
    // Graphics: draw the grid, one instance per cell, colour pulled from the buffer.
