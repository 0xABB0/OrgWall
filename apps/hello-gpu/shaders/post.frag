...
// Bindless post-process: chromatic aberration + vignette + a lifted tone curve,
// sampling the offscreen scene through the heap. The root record carries the
// scene texture slot, the sampler slot, and an animated amount.
...
layout(push_constant, std430) uniform Root
{
    uint  tex;
    uint  smp;
    float amount;   // animated aberration strength
    float vignette; // vignette falloff
} root;
...
void main()
{
    ...
    // Radial chromatic split: channels diverge along the view vector, scaled by r^2.
    ...
    // Lifted-shadow tone curve for a filmic feel.
