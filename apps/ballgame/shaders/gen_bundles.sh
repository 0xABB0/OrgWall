...
gen_graphics()
{
    ...
    if try_compile "$src" "$vs" metal "$tmp/vs.metal" && try_compile "$src" "$fs" metal "$tmp/fs.metal"; then
        ...
        echo "    WARN: $stem MSL emit failed; MSL arrays omitted" >&2
    fi
    ...
    if try_compile "$src" "$vs" wgsl "$tmp/vs.wgsl" && try_compile "$src" "$fs" wgsl "$tmp/fs.wgsl"; then
        ...
        echo "    WARN: $stem WGSL emit failed; WGSL arrays omitted" >&2
    fi
...
echo "gen_bundles: slangc $SLANG_VERSION"
gen_graphics ballgame BALLGAME vs_main fs_main
echo "gen_bundles: done"
