...
static void fields_str(u32 f, char* out, size_t cap)
{
    ...
    struct
    {
        ...
        { MEL_DISPLAY_FIELD_RESOLUTION, "RES" }, { MEL_DISPLAY_FIELD_REFRESH, "REFRESH" }, { MEL_DISPLAY_FIELD_VRR, "VRR" },      { MEL_DISPLAY_FIELD_HDR, "HDR" },
        { MEL_DISPLAY_FIELD_ICC, "ICC" },        { MEL_DISPLAY_FIELD_SCALE, "SCALE" },     { MEL_DISPLAY_FIELD_POSITION, "POS" }, { MEL_DISPLAY_FIELD_STATE, "STATE" },
    };
...
static void drain_events(void)
{
    ...
    for (u32 i = 0; i < n; i++)
    {
        ...
        case MEL_DISPLAY_EVENT_ADDED:
            snprintf(line, sizeof line, "t%llu added  idx=%u gen=%u", (unsigned long long)g.tick, ev[i].display.h.index, ev[i].display.h.generation);
            break;
        case MEL_DISPLAY_EVENT_REMOVED:
            snprintf(line, sizeof line, "t%llu removed  idx=%u gen=%u", (unsigned long long)g.tick, ev[i].display.h.index, ev[i].display.h.generation);
            break;
        case MEL_DISPLAY_EVENT_POWER_STATE_CHANGED:
            snprintf(line, sizeof line, "t%llu power_state_changed  idx=%u", (unsigned long long)g.tick, ev[i].display.h.index);
            break;
        default:
        {
            char fields[64];
            fields_str(ev[i].changed_fields, fields, sizeof fields);
            snprintf(line, sizeof line, "t%llu config_changed  idx=%u  fields=%s", (unsigned long long)g.tick, ev[i].display.h.index, fields);
            break;
        }
        }
        evlog_push(line);
...
#if MEL_PLATFORM_OSX
    if (g.edr_force)
        display_edr_set(true, EDR_FORCE_LEVEL);
#endif
    mel_display_refresh();
    drain_events();
...
static void canvas_paint(Mel_Gui_Handle h, Mel_Painter* p, i32 w, i32 height, void* user)
{
    ...
    line(&pen, head, "display inspector  —  tick %llu  —  %u display(s)  [live, %.0f ms]  —  EDR force: %s", (unsigned long long)g.tick, total, (double)TICK_NS / 1e6, g.edr_force ? "ON" : "off");
    pen.y += 4;
...
#if MEL_PLATFORM_OSX
    mel_button_create(frame, .text = S8("Toggle EDR headroom window (forces 'EDR now' > 1.0)"), .pointer.on_click = edr_toggle_clicked, .layoutable = { .preferred_h = 30 });
#endif
