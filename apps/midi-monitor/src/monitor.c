...
static void format_event(const Mel_Midi_Chunk* chunk, char* out, size_t n)
{
    ...
    {
        snprintf(out, n, "Last event: raw %02X %02X %02X (len=%d)", chunk->data[0], chunk->data[1], chunk->data[2], chunk->length);
        return;
    }
    ...
    case MEL_MIDI_MSG_NOTE_ON:
        snprintf(out, n, "Last event: note_on  ch=%u note=%u vel=%u", msg.channel, msg.note_on.note, msg.note_on.velocity);
        break;
    case MEL_MIDI_MSG_NOTE_OFF:
        snprintf(out, n, "Last event: note_off ch=%u note=%u vel=%u", msg.channel, msg.note_off.note, msg.note_off.velocity);
        break;
    case MEL_MIDI_MSG_CONTROL_CHANGE:
        snprintf(out, n, "Last event: cc       ch=%u ctrl=%u val=%u", msg.channel, msg.control_change.controller, msg.control_change.value);
        break;
    case MEL_MIDI_MSG_PITCH_BEND:
        snprintf(out, n, "Last event: pitch    ch=%u bend=%d", msg.channel, (int)msg.pitch_bend.bend);
        break;
    case MEL_MIDI_MSG_PROGRAM_CHANGE:
        snprintf(out, n, "Last event: program  ch=%u prog=%u", msg.channel, msg.program_change.program);
        break;
    case MEL_MIDI_MSG_CHANNEL_PRESSURE:
        snprintf(out, n, "Last event: chpress  ch=%u val=%u", msg.channel, msg.channel_pressure.pressure);
        break;
    case MEL_MIDI_MSG_KEY_PRESSURE:
        snprintf(out, n, "Last event: keypress ch=%u note=%u val=%u", msg.channel, msg.key_pressure.note, msg.key_pressure.pressure);
        break;
    default:
...
void build_monitor(Mel_Gui_Handle frame, void* user)
{
    ...
    mel_label_create(frame, .text = S8("MIDI Monitor"), .x = 24, .y = 20, .w = 360, .h = 44);
    mel_label_create(frame, .text = S8("Tap Refresh, then Connect to read from the first listed device."), .x = 24, .y = 70, .w = 380, .h = 44);

    mel_button_create(frame, .text = S8("Refresh"), .x = 24, .y = 126, .w = 110, .h = 52, .pointer.on_click = refresh_clicked);
    mel_button_create(frame, .text = S8("Connect"), .x = 144, .y = 126, .w = 110, .h = 52, .pointer.on_click = connect_clicked);
    mel_button_create(frame, .text = S8("Disconnect"), .x = 264, .y = 126, .w = 130, .h = 52, .pointer.on_click = disconnect_clicked);

    mel_label_create(frame, .text = S8("Devices:"), .x = 24, .y = 198, .w = 360, .h = 32);
    for (i32 i = 0; i < DEVICE_ROW_COUNT; i++)
    {
        g_app.device_rows[i] = mel_label_create(frame, .text = S8(""), .x = 24, .y = 234 + i * 32, .w = 380, .h = 30);
    }
