...
void build_replace(Mel_Gui_Handle frame, void* user)
{
    ...
    mel_label_create(frame, .text = S8("Replaced Screen"), .layoutable = { .preferred_w = 380, .preferred_h = 30 });
    mel_label_create(frame, .text = S8("This screen took over the current surface."), .layoutable = { .preferred_h = 24 });

    tapcounter_create(frame, S8("Poke replaced-local handler"));

    mel_button_create(frame, .text = S8("Re-open Main Window"), .pointer.on_click = reopen_main_clicked, .layoutable = { .preferred_h = 40 });
}
