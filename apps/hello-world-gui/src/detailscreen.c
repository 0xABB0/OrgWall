...
void build_details(Mel_Gui_Handle frame, void* user)
{
    ...
    mel_label_create(frame, .text = S8("Details Screen"), .layoutable = { .preferred_w = 380, .preferred_h = 30 });
    mel_label_create(frame, .text = S8("Different surface, same melody build code."), .layoutable = { .preferred_h = 24 });

    tapcounter_create(frame, S8("Tap details-local handler"));

    Mel_Gui_Handle panel = mel_panel_create(frame, .layout = mel_column_layout(.spacing = 4, .margin = 8, .cross_align = MEL_ALIGN_STRETCH), .layoutable = { .preferred_h = 72 });
