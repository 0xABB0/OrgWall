...
typedef enum {
    MEL_RENDER_STAGE_3D_LAYER_WORLD = 0,
    MEL_RENDER_STAGE_3D_LAYER_HUD,
    MEL_RENDER_STAGE_3D_LAYER_DEBUG,
    MEL_RENDER_STAGE_3D_LAYER_UI,
    MEL_RENDER_STAGE_3D_LAYER_COUNT,
} Mel_Render_Stage_3D_Layer;
...
} Mel_Render_Stage_3D_Opt;

typedef struct {
    str8 name;
    Mel_Widget* root;
    Mel_Render_Stage_3D_Layer layer;
    f32 input_scale_x;
    f32 input_scale_y;
    const Mel_Alloc* alloc;
} Mel_Render_Stage_3D_Widget_Layer_Opt;

typedef struct {
    Mel_Render_Default_3D renderer;
    Mel_View_Handle views[MEL_RENDER_STAGE_3D_LAYER_COUNT];
} Mel_Render_Stage_3D;
...
typedef struct {
    ...
    f32 input_scale_y;
    const Mel_Alloc* alloc;
} Mel_Render_Stage_3D_Widget_Layer;

bool mel_render_stage_3d_init_opt(Mel_Render_Stage_3D* stage, Mel_Render_Stage_3D_Opt opt);
#define mel_render_stage_3d_init(stage, ...) mel_render_stage_3d_init_opt((stage), (Mel_Render_Stage_3D_Opt){__VA_ARGS__})
...
#define mel_render_stage_3d_widget_layer_init(stage, layer, ...) mel_render_stage_3d_widget_layer_init_opt((stage), (layer), (Mel_Render_Stage_3D_Widget_Layer_Opt){__VA_ARGS__})
