...
- Directional shadow support is now fitted from the camera frustum with texel snapping, which is a healthier base than the old visible-bounds fit, but the quality model is still basic: one directional light, one map, simple PCF, and simple bias terms.
...
- The current directional shadow fit is healthier than before, but it is still single-frustum and still tied to the active camera. If Sponza keeps pressuring quality here, the next honest step is cascades, not more ad-hoc fit tweaks.
...
- `scene_forward` now has a real HDR resolve path above it, and that exposed a hard PSO seam: the mesh path had precompiled swapchain-format pipelines while sprites already keyed pipelines by actual target format. Keep render-target format coupling explicit for every internal submission path, especially when offscreen HDR targets enter the picture.
