...
static void mel__render_stage_3d_widget_producer(Mel_Render_List* list, void* user)
{
    Mel_Render_Stage_3D_Widget_Layer* layer = user;
    if (layer->root)
        mel_widget_draw(layer->root, list);
}

static Mel_View_Handle mel__render_stage_3d_require_view(Mel_Render_Stage_3D* stage, Mel_Render_Stage_3D_Layer layer)
{
    assert(stage != nullptr);
    assert(layer < MEL_RENDER_STAGE_3D_LAYER_COUNT);
    Mel_View_Handle view = stage->views[layer];
    assert(mel_view_handle_valid(view));
    return view;
}

bool mel_render_stage_3d_init_opt(Mel_Render_Stage_3D* stage, Mel_Render_Stage_3D_Opt opt)
{
    assert(stage != nullptr);
    assert(opt.world_camera != nullptr);

    bool ok = mel_render_default_3d_init(&stage->renderer,
        .name = opt.name.len ? opt.name : S8("stage_3d"),
    ...
    stage->views[MEL_RENDER_STAGE_3D_LAYER_WORLD] = mel_render_default_3d_view(&stage->renderer);
    stage->views[MEL_RENDER_STAGE_3D_LAYER_HUD] = mel_render_default_3d_add_view(&stage->renderer,
        .name = S8("hud"),
        ...
        .overlay = true,
        .order = 100);
    stage->views[MEL_RENDER_STAGE_3D_LAYER_DEBUG] = mel_render_default_3d_add_view(&stage->renderer,
        .name = S8("debug"),
        ...
        .overlay = true,
        .order = 200);
    stage->views[MEL_RENDER_STAGE_3D_LAYER_UI] = mel_render_default_3d_add_view(&stage->renderer,
        .name = S8("ui"),
...
void mel_render_stage_3d_shutdown(Mel_Render_Stage_3D* stage)
{
    assert(stage != nullptr);
    mel_render_default_3d_shutdown(&stage->renderer);
    *stage = (Mel_Render_Stage_3D){0};
}
...
{
    Mel_View_Handle view = mel__render_stage_3d_require_view(stage, layer);
    return mel_render_default_3d_attach_mesh_list_to_view(&stage->renderer, view, list);
}
...
{
    Mel_View_Handle view = mel__render_stage_3d_require_view(stage, layer);
    Mel_Technique_Family_Id family = layer == MEL_RENDER_STAGE_3D_LAYER_DEBUG
        ? MEL_TECHNIQUE_DEBUG
        : (layer == MEL_RENDER_STAGE_3D_LAYER_UI ? MEL_TECHNIQUE_UI : MEL_TECHNIQUE_SPRITE);
    return mel_render_default_3d_attach_sprite_list_to_view_family(&stage->renderer, view, list, family);
}
...
{
    Mel_View_Handle view = mel__render_stage_3d_require_view(stage, layer);
    return mel_render_default_3d_attach_text_list_to_view(&stage->renderer, view, list);
}

bool mel_render_stage_3d_widget_layer_init_opt(Mel_Render_Stage_3D* stage, Mel_Render_Stage_3D_Widget_Layer* layer, Mel_Render_Stage_3D_Widget_Layer_Opt opt)
{
    assert(stage != nullptr);
    assert(layer != nullptr);

    Mel_Render_Stage_3D_Layer target_layer = opt.layer < MEL_RENDER_STAGE_3D_LAYER_COUNT ? opt.layer : MEL_RENDER_STAGE_3D_LAYER_UI;
    const Mel_Alloc* alloc = opt.alloc ? opt.alloc : stage->renderer.alloc;

    *layer = (Mel_Render_Stage_3D_Widget_Layer){
        .root = opt.root,
        .layer = target_layer,
    ...
    mel_render_list_init(&layer->list,
        .name = opt.name.len ? opt.name : S8("stage_3d_widget_layer"),
        .entry_stride = sizeof(Mel_Sprite_Entry),
        .mode = MEL_RENDER_LIST_EPHEMERAL,
        .alloc = alloc);
    mel_render_list_add_producer(&layer->list, mel__render_stage_3d_widget_producer, layer);

    if (!mel_render_stage_3d_attach_sprite_list_to_layer(stage, target_layer, &layer->list))
    {
        mel_render_list_shutdown(&layer->list);
        *layer = (Mel_Render_Stage_3D_Widget_Layer){0};
        return false;
    }
...
void mel_render_stage_3d_widget_layer_shutdown(Mel_Render_Stage_3D* stage, Mel_Render_Stage_3D_Widget_Layer* layer)
{
    MEL_UNUSED(stage);
    assert(layer != nullptr);
    mel_render_list_shutdown(&layer->list);
...
{
    return mel__render_stage_3d_require_view(stage, layer);
}
...
Mel_Render_Graph* mel_render_stage_3d_graph(Mel_Render_Stage_3D* stage)
{
    assert(stage != nullptr);
    return mel_render_default_3d_graph(&stage->renderer);
}

Mel_Frame_Recipe_Handle mel_render_stage_3d_recipe(Mel_Render_Stage_3D* stage)
{
    assert(stage != nullptr);
    return mel_render_default_3d_recipe(&stage->renderer);
}

Mel_Frame_Plan_Handle mel_render_stage_3d_plan(Mel_Render_Stage_3D* stage)
{
    assert(stage != nullptr);
    return mel_render_default_3d_plan(&stage->renderer);
}

Mel_Render_Target* mel_render_stage_3d_target(Mel_Render_Stage_3D* stage)
{
    assert(stage != nullptr);
    return mel_render_default_3d_target(&stage->renderer);
}
