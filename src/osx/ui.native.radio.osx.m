...
- (void)radioClicked:(id)sender
{
    ...
    if (parent) {
        Mel_NCtrl* sibling = parent->first_child;
        while (sibling) {
            if (sibling->vtable == radio->base.vtable && sibling != &radio->base) {
                Mel_NRadio* sibling_radio = (Mel_NRadio*)sibling;
                if (sibling_radio->group_id == radio->group_id && sibling_radio->selected) {
                    sibling_radio->selected = false;
                    if (sibling->backing) {
                        NSButton* nsBtn = (__bridge NSButton*)sibling->backing;
...
const Mel_NCtrl_VTable* mel__nradio_vtable_osx(void)
{
