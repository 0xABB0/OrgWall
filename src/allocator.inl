...
static inline void* mel__malloc(const Mel_Alloc* a, usize size,
...
static inline void mel__free(const Mel_Alloc* a, void* ptr,
                             const char* file, const char* func, u32 line)
{
    assert(a != NULL);
    assert(a->alloc_cb != NULL);
...
static inline void* mel__calloc(const Mel_Alloc* a, usize size,
                                const char* file, const char* func, u32 line)
{
    void* ptr = mel__malloc(a, size, file, func, line);
    if (ptr)
    {
        memset(ptr, 0, size);
    }
...
static inline void* mel__aligned_malloc(const Mel_Alloc* a, usize size, u32 align,
                                        const char* file, const char* func, u32 line)
{
    assert(a != NULL);
    assert(a->alloc_cb != NULL);
...
static inline void mel__aligned_free(const Mel_Alloc* a, void* ptr, u32 align,
                                     const char* file, const char* func, u32 line)
{
    assert(a != NULL);
    assert(a->alloc_cb != NULL);
