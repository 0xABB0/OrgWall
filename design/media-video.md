...
`design/provider.md` (upstream — some vendor encoders / decoders are provider-backed); `modules/sensor/spec.md` (camera sensor metadata, ambient signals); `design/platform-surface.md`, `modules/display/spec.md` (color-space enum source, HDR capabilities); `design/frame-pacing.md` (consumer — video-encoder pull-clock as a P2 pacing source); `design/frame-latency.md` (camera-to-encode latency markers); `design/render-graph.md` (peer — video pass kinds are graph pass kinds, gpu-rhi.md §8); `design/render-reconstruction.md` (consumer — TAA / upscaling over decoded video); `design/io-asset.md` (peer — both are media-IO-domain); `design/xr.md` (consumer — passthrough cameras, runtime composition layers).
