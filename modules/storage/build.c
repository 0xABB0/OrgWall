...
void build(Mel_Build* b)
{
    ...
    mel_sources(lib, WHEN(.platforms = MEL_ON(MACOS) | MEL_ON(IOS) | MEL_ON(LINUX) | MEL_ON(ANDROID)), "posix/src/space_posix.c");
    mel_sources(lib, WHEN(.platforms = MEL_ON(WIN32)), "win32/src/space_win32.c");
    mel_sources(lib, WHEN(.platforms = MEL_ON(WASM)), "wasm/src/space_wasm.c");
    mel_sources(lib, WHEN(.platforms = MEL_ON(MACOS) | MEL_ON(IOS) | MEL_ON(LINUX) | MEL_ON(WIN32) | MEL_ON(WASM)), "src/title_none.c");
    mel_sources(lib, WHEN(.platforms = MEL_ON(ANDROID)), "android/src/title_android.c");
