...
#define mel_queue_pop(q)       ((q)->count--, (q)->head = ((q)->head < (q)->capacity - 1) ? (q)->head + 1 : 0, (q)->items[((q)->head == 0) ? (q)->capacity - 1 : (q)->head - 1])
