...
#define mel_array_init(arr, alloc)  \
    do                              \
    {                               \
        (arr)->items = NULL;        \
        (arr)->count = 0;           \
...
#define mel_array_push(arr, item)                                                         \
    do                                                                                    \
    {                                                                                     \
...
#define mel_array_reserve(arr, n)                                                     \
    do                                                                                \
    {                                                                                 \
        if ((n) > (arr)->capacity)                                                    \
        {                                                                             \
...
#define mel_array_remove_unordered(arr, idx)                \
    do                                                      \
    {                                                       \
...
#define mel_array_remove_ordered(arr, idx)                                                                           \
    do                                                                                                               \
    {                                                                                                                \
        assert((idx) < (arr)->count);                                                                                \
        memmove(&(arr)->items[(idx)], &(arr)->items[(idx) + 1], sizeof(*(arr)->items) * ((arr)->count - (idx) - 1)); \
        (arr)->count--;                                                                                              \
    } while (0)

#define mel_array_insert(arr, idx, item)                                                                                    \
    do                                                                                                                      \
    {                                                                                                                       \
        assert((usize)(idx) <= (arr)->count);                                                                               \
        if ((arr)->count >= (arr)->capacity)                                                                                \
        {                                                                                                                   \
            usize new_cap = (arr)->capacity == 0 ? MEL_DA_INIT_CAP : (arr)->capacity * 2;                                   \
            usize new_size = sizeof(*(arr)->items) * new_cap;                                                               \
        ...
        if ((usize)(idx) < (arr)->count)                                                                                    \
        {                                                                                                                   \
            memmove(&(arr)->items[(idx) + 1], &(arr)->items[(idx)], sizeof(*(arr)->items) * ((arr)->count - (usize)(idx))); \
        }                                                                                                                   \
        (arr)->items[(idx)] = (item);                                                                                       \
        (arr)->count++;                                                                                                     \
    } while (0)
