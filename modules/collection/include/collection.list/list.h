...
#define mel_container_of(ptr, type, member)   ((type*)((u8*)(ptr) - offsetof(type, member)))

#define mel_list_entry(ptr, type, member)     mel_container_of(ptr, type, member)

#define mel_list_foreach(pos, head)           for (Mel_ListNode* pos = (head)->next; pos != (head); pos = pos->next)

#define mel_list_foreach_safe(pos, tmp, head) for (Mel_ListNode* pos = (head)->next, *tmp = pos->next; pos != (head); pos = tmp, tmp = pos->next)
