...
void mel_set_free(Mel_Set* s) { mel_hashmap_free(&s->map); }

bool mel_set_add(Mel_Set* s, void* key) { return mel_hashmap_put(&s->map, key, nullptr); }

bool mel_set_contains(Mel_Set* s, void* key) { return mel_hashmap_contains(&s->map, key); }

bool mel_set_remove(Mel_Set* s, void* key) { return mel_hashmap_remove(&s->map, key); }

usize mel_set_count(Mel_Set* s) { return mel_hashmap_count(&s->map); }

bool mel_set_empty(Mel_Set* s) { return mel_hashmap_empty(&s->map); }

void mel_set_clear(Mel_Set* s) { mel_hashmap_clear(&s->map); }

void mel_set_reserve(Mel_Set* s, usize capacity) { mel_hashmap_reserve(&s->map, capacity); }
...
bool mel_set_is_superset(Mel_Set* a, Mel_Set* b) { return mel_set_is_subset(b, a); }
