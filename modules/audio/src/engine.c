...
static void mel_audio__scratch_free(Mel_Audio* eng)
{
...
static bool mel_audio__scratch_alloc(Mel_Audio* eng, u32 frames, u32 fetch_frames, u32 channels)
{
...
void mel_audio__scratch_size_worst(Mel_Audio* eng, u32 frames)
{
    ...
    u32 fetch = mel_audio__fetch_frames_for(frames, eng->worst_ratio, 1.0) + 2u;
    if (!mel_audio__scratch_alloc(eng, frames, fetch, channels))
    {
...
void mel_audio__scratch_ensure_offline(Mel_Audio* eng, u32 frames)
{
    ...
    {
        Mel_Audio__Voice* v = &eng->voices.packed.items[i];
        if (v->source == NULL)
            continue;
    ...
    if (!mel_audio__scratch_alloc(eng, want_frames, want_fetch, want_channels))
    {
...
{
    mel_audio__end_futures_free(eng);
...
static Mel_Audio* mel_audio__alloc_engine(const Mel_Alloc* a, Mel_Audio_Opt opt)
{
    ...
    *eng = (Mel_Audio){ 0 };
    ...
    mel_audio__voices_init(&eng->voices, a, 16u);
...
static int mel_audio__mix_thread_fn(void* user)
{
    Mel_Audio* eng = (Mel_Audio*)user;
    ...
    while (atomic_load_explicit(&eng->mix_stop, memory_order_acquire) == 0u)
    {
        ...
        while (mel_audio_ring_write_available(eng->ring) >= block_samples)
        {
            mel_audio__mix_block(eng, eng->scratch_planar, block);
            ...
            mel_audio_ring_write(eng->ring, interleave, block_samples);
        }

        if (atomic_load_explicit(&eng->mix_stop, memory_order_acquire) != 0u)
            break;
        if (mel_audio_ring_write_available(eng->ring) < block_samples)
...
Mel_Audio* mel_audio_create(const Mel_Alloc* a, Mel_Audio_Opt opt)
{
    ...
    mel_audio_ring_set_wake(eng->ring, &eng->mix_wake);

    if (!mel_thread_spawn(&eng->mix_thread, mel_audio__mix_thread_fn, eng, .name = "mel-audio-mix"))
    {
...
{
    assert(eng != NULL);
    return mel_audio__voice_count(&eng->voices);
}
