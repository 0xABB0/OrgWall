...
static f32 first_left(Mel_Audio* eng, f32* scratch)
{
    mel_audio_render(eng, scratch, STEP);
    return scratch[0];
}
...
MEL_TEST(fader, schedule_stop_silences_voice_on_time)
{
    ...
    const f64 seconds = (f64)dur_frames / (f64)SR;
    mel_audio_schedule_stop(eng, v, seconds);
...
MEL_TEST(fader, schedule_pause_freezes_voice_on_time)
{
    ...
    const u32 dur_frames = 2u * STEP;
    const f64 seconds = (f64)dur_frames / (f64)SR;
    mel_audio_schedule_pause(eng, v, seconds);
