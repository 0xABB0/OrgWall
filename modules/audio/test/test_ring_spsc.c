...
MEL_TEST(ring, bulk_write_then_read_identical)
{
    ...
    Mel_Audio_Ring*  r = mel_audio_ring_create(a, 64u);
    MEL_REQUIRE_NOT_NULL(r);

    MEL_EXPECT_EQ(mel_audio_ring_capacity(r), 64u);
    MEL_EXPECT_EQ(mel_audio_ring_read_available(r), 0u);
    MEL_EXPECT_EQ(mel_audio_ring_write_available(r), 64u);

    f32 in[32];
    for (u32 i = 0; i < 32u; i++)
        in[i] = (f32)i + 0.5f;

    u32 wrote = mel_audio_ring_write(r, in, 32u);
    MEL_EXPECT_EQ(wrote, 32u);
    MEL_EXPECT_EQ(mel_audio_ring_read_available(r), 32u);
    MEL_EXPECT_EQ(mel_audio_ring_write_available(r), 32u);

    f32 out[32];
    u32 read = mel_audio_ring_read(r, out, 32u);
    ...
    mel_audio_ring_destroy(r);
}

MEL_TEST(ring, wrap_around_boundary)
{
    const Mel_Alloc* a = test_alloc();
    Mel_Audio_Ring*  r = mel_audio_ring_create(a, 8u);
    MEL_REQUIRE_NOT_NULL(r);

    f32 a0[6] = { 0.f, 1.f, 2.f, 3.f, 4.f, 5.f };
    MEL_EXPECT_EQ(mel_audio_ring_write(r, a0, 6u), 6u);

    f32 d0[4];
    MEL_EXPECT_EQ(mel_audio_ring_read(r, d0, 4u), 4u);
    for (u32 i = 0; i < 4u; i++)
        MEL_EXPECT_FLOAT_EQ(d0[i], (f32)i, 0.0f);

    f32 a1[6] = { 6.f, 7.f, 8.f, 9.f, 10.f, 11.f };
    MEL_EXPECT_EQ(mel_audio_ring_write(r, a1, 6u), 6u);

    f32 d1[8];
    MEL_EXPECT_EQ(mel_audio_ring_read(r, d1, 8u), 8u);
...
{
    const Mel_Alloc* a = test_alloc();
    Mel_Audio_Ring*  r = mel_audio_ring_create(a, 16u);
    MEL_REQUIRE_NOT_NULL(r);

    f32 in[4] = { 9.f, 9.f, 9.f, 9.f };
    mel_audio_ring_write(r, in, 4u);

    f32 out[8];
    for (u32 i = 0; i < 8u; i++)
        out[i] = -1.f;

    u32 read = mel_audio_ring_read(r, out, 8u);
    ...
    u32 got = mel_audio_ring_read(r, again, 4u);
...
MEL_TEST(ring, full_applies_backpressure)
{
    ...
    u32 wrote = mel_audio_ring_write(r, in, 12u);
    MEL_EXPECT_EQ(wrote, 8u);
    MEL_EXPECT_EQ(mel_audio_ring_write_available(r), 0u);
    MEL_EXPECT_EQ(mel_audio_ring_read_available(r), 8u);

    u32 wrote2 = mel_audio_ring_write(r, in, 4u);
    MEL_EXPECT_EQ(wrote2, 0u);

    f32 out[8];
    mel_audio_ring_read(r, out, 8u);
    for (u32 i = 0; i < 8u; i++)
...
static int spsc_producer(void* user)
{
    ...
    while (produced < ctx->total)
    {
        ...
        while (off < want)
        {
            u32 n = mel_audio_ring_write(ctx->ring, buf + off, want - off);
            off += n;
            if (n == 0u)
                mel_thread_yield();
        }
...
static int spsc_consumer(void* user)
{
    ...
    while (consumed < ctx->total)
    {
        ...
        u32 avail = mel_audio_ring_read_available(ctx->ring);
        if (avail == 0u)
        {
            mel_thread_yield();
            continue;
        ...
        u32 got = mel_audio_ring_read(ctx->ring, buf, take);
        for (u32 i = 0; i < got; i++)
        {
...
{
    const Mel_Alloc* a = test_alloc();
    Mel_Audio_Ring*  r = mel_audio_ring_create(a, SPSC_CAPACITY);
    MEL_REQUIRE_NOT_NULL(r);
