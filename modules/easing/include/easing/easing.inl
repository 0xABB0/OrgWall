...
static inline f32 mel_ease_out_cubic(f32 t)
{
    f32 u = 1.0f - t;
    return 1.0f - u * u * u;
}
...
static inline f32 mel_ease_out_quart(f32 t)
{
    f32 u = 1.0f - t;
    return 1.0f - u * u * u * u;
}
...
static inline f32 mel_ease_out_quint(f32 t)
{
    f32 u = 1.0f - t;
...
static inline f32 mel_ease_in_out_circ(f32 t) { return t < 0.5f ? (1.0f - mel_sqrtf(1.0f - mel_powf(2.0f * t, 2.0f))) * 0.5f : (mel_sqrtf(1.0f - mel_powf(-2.0f * t + 2.0f, 2.0f)) + 1.0f) * 0.5f; }
...
static inline f32 mel_ease_in_out_expo(f32 t)
{
    ...
    if (t == 1.0f)
        return 1.0f;
    return t < 0.5f ? mel_powf(2.0f, 20.0f * t - 10.0f) * 0.5f : (2.0f - mel_powf(2.0f, -20.0f * t + 10.0f)) * 0.5f;
}
...
static inline f32 mel_ease_in_out_elastic(f32 t)
{
    ...
    if (t == 1.0f)
        return 1.0f;
    return t < 0.5f ? -(mel_powf(2.0f, 20.0f * t - 10.0f) * mel_sinf((20.0f * t - 11.125f) * (MEL_TAU / 4.5f))) * 0.5f
...
static inline f32 mel_ease_in_out_back(f32 t)
{
    ...
    return t < 0.5f ? (mel_powf(2.0f * t, 2.0f) * ((c2 + 1.0f) * 2.0f * t - c2)) * 0.5f : (mel_powf(2.0f * t - 2.0f, 2.0f) * ((c2 + 1.0f) * (t * 2.0f - 2.0f) + c2) + 2.0f) * 0.5f;
}
