...
const char *mel_platform_name(Mel_Platform p) {
    switch (p) {
        case MEL_PLATFORM_MACOS: return "macos";
        case MEL_PLATFORM_IOS: return "ios";
        case MEL_PLATFORM_LINUX: return "linux";
        case MEL_PLATFORM_ANDROID: return "android";
        case MEL_PLATFORM_WIN32: return "win32";
        case MEL_PLATFORM_WASM: return "wasm";
...
bool mel_target_available(Mel_Target *t, const Mel_Variant *v) {
    for (size_t i = 0; i < t->unavailable.len; i++)
        if (mel_when_match(t->unavailable.items[i], v)) return false;
    return true;
}
    ...
    for (size_t i = 0; i < order.len; i++) {
        ...
        if (!mel_target_available(d, v)) {
            fprintf(stderr, "build: '%s' (needed by '%s') is unavailable on %s\n", d->name,
                    target->name, mel_platform_name(v->platform));
            free(order.items);
            return false;
        }
...
void mel_gather_link(Mel_Graph *g, size_t idx, const Mel_Variant *v, Mel_StrVec *ldflags) {
    ...
    for (size_t i = 0; i < order.len; i++) {
        ...
        for (size_t k = 0; k < d->links.len; k++) {
            ...
            if (!own && f.vis != MEL_PUBLIC) continue;
            if (!mel_when_match(f.when, v)) continue;
