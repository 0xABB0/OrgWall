...
static void build_vars(Mel_Target* t, const char* icon, Mel_KVVec* vars)
{
    for (size_t i = 0; i < t->manifest.len; i++)
        mel_da_push(vars, t->manifest.items[i]);
    ...
    for (size_t i = 0; i < t->manifest.len; i++)
    {
        if (strcmp(t->manifest.items[i].key, "VERSION") == 0)
            has_version = true;
        if (strcmp(t->manifest.items[i].key, "APP_LABEL") == 0)
            has_label = true;
        if (strcmp(t->manifest.items[i].key, "BUNDLE_ID") == 0)
            has_bundle = true;
    }
    if (!has_version)
        mel_da_push(vars, ((Mel_KV){ "VERSION", "1.0.0" }));
    if (!has_label)
        mel_da_push(vars, ((Mel_KV){ "APP_LABEL", t->name }));
    if (!has_bundle)
...
static bool package_apple(Mel_Target* t, const char* outdir, const char* exe, const char* plat, bool flat)
{
    ...
    if (!tpl_path)
        tpl_path = mel_str_fmt("modules/build/%s/Info.plist.in", plat);
...
char* mel_win32_resource(Mel_Target* t, const char* outdir)
{
    ...
    if (!tpl_path)
        tpl_path = mel_str_dup("modules/build/win32/app.rc");
