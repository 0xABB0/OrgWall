...
static inline str8 mel__from_mg(struct mg_str s) { return (str8){ .data = (u8*)s.buf, .len = (size)s.len }; }

static inline struct mg_str mel__to_mg(str8 s) { return mg_str_n((const char*)s.data, (size_t)s.len); }

static inline str8 mel__cstr(const char* s) { return s ? (str8){ .data = (u8*)s, .len = (size)strlen(s) } : (str8){ 0 }; }
...
i32 mel_server_module_init(const Mel_Alloc* alloc)
{
    if (g_module_initialized)
        return MEL_SERVER_ERR_ALREADY_INITIALIZED;
    if (alloc == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    g_module_alloc = alloc;
    mel_slotmap_init(&g_registry, alloc, .item_size = sizeof(Mel_Server), .initial_capacity = MEL_SERVER_REGISTRY_INITIAL_CAPACITY);
...
void mel_server_module_shutdown(void)
{
    ...
    if (n > 0)
    {
        mel_log_warn(MEL_SERVER_LOG_DOMAIN, "shutdown with %u live servers; destroying them", n);
    }
...
{
    if (!g_module_initialized)
        return MEL_SERVER_ERR_NOT_INITIALIZED;
    if (out == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    *out = MEL_SERVER_HANDLE_NULL;
    ...
    Mel_Server*       s = (Mel_Server*)mel_slotmap_get(&g_registry, h);
    if (s == NULL)
        return MEL_SERVER_ERR_OOM;

    s->alive = true;
    s->generation = h.generation;
    s->self = h;
    s->opt = opt;
    if (s->opt.publish_queue_capacity == 0)
        s->opt.publish_queue_capacity = MEL_SERVER_DEFAULT_PUBLISH_QUEUE_CAPACITY;
    if (s->opt.conn_outbox_capacity == 0)
        s->opt.conn_outbox_capacity = MEL_SERVER_DEFAULT_CONN_OUTBOX_CAPACITY;
    if (opt.bind != NULL)
        s->bind_url = mel__str8_dup(g_module_alloc, mel__cstr(opt.bind));
...
i32 mel_server_listen_opt(Mel_Server_Handle h, Mel_Server_Listen_Opt opt, Mel_Server_Listener_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    if (out)
        *out = MEL_SERVER_HANDLE_NULL;
...
i32 mel_server_call_main(Mel_Server_Handle h, Mel_Server_Main_Fn fn, void* user_data)
{
    ...
    if (fn == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    if (!s->wakeup_initialized)
        return MEL_SERVER_ERR_UNIMPLEMENTED;
    ...
    while (mel_mpmc_pop(&s->xt_queue, &p))
    {
        ...
        else if (msg->kind == MEL__SERVER_MSG_KIND_PUBLISH)
        {
            for (u32 i = 0; i < s->conns.slot_count; i++)
            {
                ...
                if (cn == NULL || !cn->alive)
                    continue;
                if (cn->kind != MEL__SERVER_CONN_KIND_PUBSUB)
                    continue;
...
static void mel__server_translate_path(const Mel_Alloc* a, str8 input, str8* out_pattern, Mel__Server_Str8_Array* out_names)
{
    ...
    while (i < (usize)input.len)
    {
        ...
        if (ch == ':' || ch == '*')
        {
            ...
            i++;
            usize start = i;
            while (i < (usize)input.len && ((char)input.data[i] != '/') && ((char)input.data[i] != '.'))
            {
                i++;
            }
...
i32 mel_server_route_add_opt(Mel_Server_Handle h, Mel_Server_Route_Opt opt, Mel_Server_Route_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    if (opt.path == NULL || opt.handler == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    if (out)
        *out = MEL_SERVER_HANDLE_NULL;
    ...
    Mel__Server_Route_Slot* r = (Mel__Server_Route_Slot*)mel_slotmap_get(&s->routes, rh);
    if (r)
        r->generation = rh.generation;
    if (out)
        *out = rh;
    mel_log_debug(MEL_SERVER_LOG_DOMAIN_ROUTER, "route + %.*s %s", (int)z.method.len, (const char*)z.method.data, opt.path);
    return MEL_SERVER_OK;
}
...
i32 mel_server_serve_dir_opt(Mel_Server_Handle h, Mel_Server_Serve_Dir_Opt opt, Mel_Server_Route_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    ...
    {
        char tmp[1024];
        snprintf(tmp, sizeof(tmp), "%s%s*splat", prefix, (prefix[strlen(prefix) - 1] == '/') ? "" : "/");
    ...
    z.static_extra_headers = opt.extra_headers ? mel__str8_dup(g_module_alloc, mel__cstr(opt.extra_headers)) : (str8){ 0 };
...
i32 mel_server_serve_file_opt(Mel_Server_Handle h, Mel_Server_Serve_File_Opt opt, Mel_Server_Route_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    if (opt.url_path == NULL || opt.fs_path == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    if (out)
        *out = MEL_SERVER_HANDLE_NULL;
...
i32 mel_server_ws_add_opt(Mel_Server_Handle h, Mel_Server_WS_Opt opt, Mel_Server_Route_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
...
i32 mel_server_rpc_add_opt(Mel_Server_Handle h, Mel_Server_RPC_Opt opt, Mel_Server_RPC_Method_Handle* out)
{
    Mel_Server* s = mel__server_resolve(h);
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    if (opt.method == NULL || opt.handler == NULL)
        return MEL_SERVER_ERR_INVALID_ARGUMENT;
    if (out)
        *out = MEL_SERVER_HANDLE_NULL;
...
static void mel__server_rpc_dispatch(Mel_Server* s, Mel__Server_Conn* cn, str8 frame)
{
    ...
    for (u32 i = 0; i < s->rpc_methods.slot_count; i++)
    {
        ...
        if (m == NULL || !m->alive)
            continue;
        if (str8_equals(m->method, method_str))
        {
            found = m;
            break;
        }
...
Mel_Server_Conn_Handle mel_server_rpc_conn(Mel_Server_RPC_Req* r) { return (r && r->conn) ? r->conn->self : MEL_SERVER_HANDLE_NULL; }
...
void mel_server_rpc_ok(Mel_Server_RPC_Req* r, const char* fmt, ...)
{
    ...
    va_end(ap);
    mg_ws_printf(r->conn->mg, WEBSOCKET_OP_TEXT, "{\"jsonrpc\":\"2.0\",\"id\":%.*s,\"result\":%s}", (int)r->id_json.len, (const char*)r->id_json.data, body);
    r->responded = true;
}
...
static void mel__server_pubsub_ws_msg(Mel_Server_Handle sh, Mel_Server_Conn_Handle ch, str8 data, bool is_binary)
{
    ...
    if (sub_tok.len > 0)
    {
        mel__server_pubsub_handle_sub(s, cn, sub_tok, false);
        return;
    }
    str8 unsub_tok = mel__from_mg(mg_json_get_tok(j, "$.unsub"));
    if (unsub_tok.len > 0)
    {
        mel__server_pubsub_handle_sub(s, cn, unsub_tok, true);
        return;
    }
}
...
static void mel__server_pubsub_handle_sub(Mel_Server* s, Mel__Server_Conn* cn, str8 topic_tok, bool is_unsub)
{
    ...
    mel_log_debug(MEL_SERVER_LOG_DOMAIN_PUBSUB, "+sub %.*s", (int)t.len, (const char*)t.data);
    (void)s;
}
...
u32 mel_server_topic_subs(Mel_Server_Handle h, const char* topic)
{
    ...
    for (u32 i = 0; i < s->conns.slot_count; i++)
    {
        ...
        for (usize j = 0; j < cn->subbed_topics.count; j++)
        {
            if (str8_equals(cn->subbed_topics.items[j], t))
            {
                count++;
                break;
            }
...
i32 mel_server_tls_set_opt(Mel_Server_Handle h, Mel_Server_TLS_Opt opt)
{
    ...
    if (opt.cert_pem)
        s->tls_cert_pem = mel__str8_dup(g_module_alloc, mel__cstr(opt.cert_pem));
    if (opt.key_pem)
        s->tls_key_pem = mel__str8_dup(g_module_alloc, mel__cstr(opt.key_pem));
    if (opt.ca_pem)
        s->tls_ca_pem = mel__str8_dup(g_module_alloc, mel__cstr(opt.ca_pem));
...
i32 mel_server_ws_send_text(Mel_Server_Handle h, Mel_Server_Conn_Handle ch, str8 data)
{
    ...
    if (s == NULL)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    Mel__Server_Conn* cn = (Mel__Server_Conn*)mel_slotmap_get(&s->conns, ch);
    if (cn == NULL || !cn->alive || !cn->mg)
        return MEL_SERVER_ERR_INVALID_HANDLE;
    if (cn->mg->is_closing)
        return MEL_SERVER_ERR_CONN_CLOSED;
...
i32 mel_server_ws_broadcast_text(Mel_Server_Handle h, const char* path, str8 data)
{
    ...
    for (u32 i = 0; i < s->conns.slot_count; i++)
    {
        ...
        if (cn == NULL || !cn->alive || !cn->mg)
            continue;
        if (cn->kind != MEL__SERVER_CONN_KIND_WS && cn->kind != MEL__SERVER_CONN_KIND_PUBSUB && cn->kind != MEL__SERVER_CONN_KIND_RPC)
            continue;
        if (cn->mg->is_closing)
            continue;
        if (path && !str8_equals(cn->ws_path, want))
            continue;
        mg_ws_send(cn->mg, data.data, (size_t)data.len, WEBSOCKET_OP_TEXT);
    }
    return MEL_SERVER_OK;
...
{
    if (index >= req->captures.count)
        return false;
    if (out)
        *out = req->captures.items[index];
    return true;
}
...
void mel_server_reply_bytes(Mel_Server_Req* req, i32 status, const void* p, usize n, const char* content_type)
{
    ...
    mg_printf(req->mg_conn, "HTTP/1.1 %d \r\n%sContent-Length: %lu\r\n\r\n", status, hdr, (unsigned long)n);
...
void mel_server_stream_write(Mel_Server_Req* req, const void* p, usize n) { mg_http_write_chunk(req->mg_conn, (const char*)p, n); }
...
void mel_server_stream_end(Mel_Server_Req* req) { mg_http_write_chunk(req->mg_conn, "", 0); }
...
{
    if (want.len == 1 && want.data[0] == '*')
        return true;
    return str8_ieq(want, actual);
}

static Mel__Server_Route_Slot* mel__server_match_route(Mel_Server* s, struct mg_http_message* hm, Mel__Server_Str8_Array* out_caps, bool want_ws)
{
    ...
    for (u32 i = 0; i < s->routes.slot_count; i++)
    {
        ...
        if (is_ws != want_ws)
            continue;
        if (!is_ws && !mel__server_method_matches(r->method, method))
            continue;
        if (is_ws && !str8_ieq(r->method, method))
            continue;

        struct mg_str caps_buf[8] = { 0 };
        if (!mg_match(mel__to_mg(uri), mel__to_mg(r->pattern), caps_buf))
            continue;
...
cleanup:
    if (req.user_data_initialized)
        mel_hashmap_free(&req.user_data);
