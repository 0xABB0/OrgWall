...
void build(Mel_Build* b)
{
    ...
    mel_sources(t, WHEN(.platforms = MEL_ON(MACOS)), "macos/src/*.c");
    mel_sources(t, WHEN(.platforms = MEL_ON(IOS)), "ios/src/*.m");
    mel_includes(t, MEL_PUBLIC, WHEN(.platforms = MEL_ON(IOS)), "ios/include");
    mel_sources(t, WHEN(.platforms = MEL_ON(LINUX)), "linux/src/*.c");
    mel_includes(t, MEL_PUBLIC, WHEN(.platforms = MEL_ON(LINUX)), "linux/include");
    mel_sources(t, WHEN(.platforms = MEL_ON(WIN32)), "win32/src/*.c");
    mel_includes(t, MEL_PUBLIC, WHEN(.platforms = MEL_ON(WIN32)), "win32/include");
    mel_sources(t, WHEN(.platforms = MEL_ON(ANDROID)), "android/src/*.c");
    mel_includes(t, MEL_PUBLIC, WHEN(.platforms = MEL_ON(ANDROID)), "android/include");
    mel_sources(t, WHEN(.platforms = MEL_ON(WASM)), "web/src/*.c");
