...
Mel_Gui_Handle mel_splitter_create_opt(Mel_Gui_Handle parent, Mel_Splitter_Opt o)
{
    ...
    jobject  view = (*env)->CallStaticObjectMethod(env, s_cls, s_create, mel_gui__android_pack(h), vertical);
    if ((*env)->ExceptionCheck(env))
    {
        (*env)->ExceptionClear(env);
        return h;
...
Mel_Gui_Handle mel_splitpane_create_opt(Mel_Gui_Handle splitter, Mel_SplitPane_Opt o)
{
    ...
    jobject pane = (*env)->CallStaticObjectMethod(env, s_cls, s_addPane, (jobject)sn->native, (jint)o.min_size, (jint)o.initial_size);
    if ((*env)->ExceptionCheck(env))
    {
        (*env)->ExceptionClear(env);
        return h;
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelSplitter_nativeLayout(JNIEnv* env, jclass cls, jlong h)
{
    (void)cls;
    ...
    for (u32 i = 0; i < count; i++)
    {
        ...
        if (!mel_gui_handle_eq(c->parent, splitter) || !c->native)
            continue;
