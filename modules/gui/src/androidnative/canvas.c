...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelCanvasView_nativePaint(JNIEnv* env, jclass cls, jlong h, jlong fn, jobject canvas, jobject paint, jint w, jint height)
{
    (void)cls;
    if (!fn)
        return;
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelCanvasView_nativeKey(JNIEnv* env, jclass cls, jlong h, jlong fn, jint key)
{
    (void)env;
    (void)cls;
    if (!fn)
