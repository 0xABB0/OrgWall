...
Mel_Gui_Handle mel_button_create_opt(Mel_Gui_Handle parent, Mel_Button_Opt o)
{
    ...
    {
        (*env)->CallStaticVoidMethod(env, s_cls, s_installClick, view, mel_gui__android_pack(h), (jlong)(intptr_t)o.pointer.on_click);
    }
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelButton_nativeClick(JNIEnv* env, jclass cls, jlong h, jlong fn)
{
    (void)env;
    (void)cls;
    if (!fn)
