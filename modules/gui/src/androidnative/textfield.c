...
Mel_Gui_Handle mel_textfield_create_opt(Mel_Gui_Handle parent, Mel_TextField_Opt o)
{
    ...
    jobject view = (*env)->CallStaticObjectMethod(env, s_cls, s_create, mel_gui__android_pack(h), s, (jlong)(intptr_t)o.on_.on_text_changed);
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelTextField_nativeTextChanged(JNIEnv* env, jclass cls, jlong h, jlong fn, jstring text)
{
    (void)cls;
    if (!fn)
        return;
