...
Mel_Gui_Handle mel_dialog_create_opt(Mel_Dialog_Opt o)
{
    ...
    jobject content = (*env)->CallStaticObjectMethod(env, s_cls, s_create, s, mel_gui__android_pack(h), (jlong)(intptr_t)o.on_.on_result);
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelDialog_nativeResult(JNIEnv* env, jclass cls, jlong h, jlong fn, jint result)
{
    (void)env;
    (void)cls;
    Mel_Gui_Handle handle = mel_gui__android_unpack(h);
    if (fn)
        ((Mel_Cb_I32)(intptr_t)fn)(handle, (i32)result, mel_gui_user(handle));
