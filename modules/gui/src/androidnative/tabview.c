...
Mel_Gui_Handle mel_tabview_create_opt(Mel_Gui_Handle parent, Mel_TabView_Opt o)
{
    ...
    jobject view = (*env)->CallStaticObjectMethod(env, s_cls, s_create, mel_gui__android_pack(h), (jlong)(intptr_t)o.on_select);
    if ((*env)->ExceptionCheck(env))
    {
        (*env)->ExceptionClear(env);
        return h;
...
Mel_Gui_Handle mel_tab_create_opt(Mel_Gui_Handle tabview, Mel_Tab_Opt o)
{
    ...
    Mel_Gui_Handle h = mel_gui__node_new(tabview, 0, 0, tn->width, tn->height - TABSTRIP_DP, o.id, o.user, false, &o.layoutable, o.layout);
    Mel_Gui_Node*  n = mel_gui__node(h);
    if (!n)
        return h;
...
void mel_tabview_select(Mel_Gui_Handle tabview, i32 index)
{
    ...
    JNIEnv* env = mel_gui__android_env();
    if (env)
        (*env)->CallStaticVoidMethod(env, s_cls, s_select, (jobject)n->native, (jint)index);
}
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelTabView_nativeSelect(JNIEnv* env, jclass cls, jlong h, jlong fn, jint index)
{
    (void)env;
    (void)cls;
    if (!fn)
