...
} s_windows[MEL_GPU_VIEW_MAX];

static ANativeWindow** window_slot(Mel_Gui_Handle h)
{
    for (int i = 0; i < MEL_GPU_VIEW_MAX; i++)
        if (mel_gui_handle_eq(s_windows[i].handle, h))
            return &s_windows[i].window;
    for (int i = 0; i < MEL_GPU_VIEW_MAX; i++)
        if (mel_gui_handle_is_none(s_windows[i].handle))
        {
            s_windows[i].handle = h;
            return &s_windows[i].window;
        }
    return NULL;
}
...
Mel_Gui_Handle mel_gpu_view_create_opt(Mel_Gui_Handle parent, Mel_Gpu_View_Opt o)
{
    ...
    jobject view = (*env)->CallStaticObjectMethod(env, s_cls, s_create, mel_gui__android_pack(h), (jlong)(intptr_t)o.on_.on_resize, (jlong)(intptr_t)o.pointer.on_pointer_down,
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelGpuView_nativeSurfaceChanged(JNIEnv* env, jclass cls, jlong h, jlong fn, jobject surface, jint w, jint height)
{
    (void)cls;
    ...
    if (fn)
        ((Mel_Cb_Resize)(intptr_t)fn)(handle, (i32)w, (i32)height, mel_gui_user(handle));
}

JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelGpuView_nativeSurfaceDestroyed(JNIEnv* env, jclass cls, jlong h, jlong fn)
{
    (void)env;
    (void)cls;
    Mel_Gui_Handle handle = mel_gui__android_unpack(h);
    ...
    if (fn)
        ((Mel_Cb_Resize)(intptr_t)fn)(handle, 0, 0, mel_gui_user(handle));
    ANativeWindow** slot = window_slot(handle);
    if (slot && *slot)
    {
        ANativeWindow_release(*slot);
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelGpuView_nativePointer(JNIEnv* env, jclass cls, jlong h, jlong fn, jint x, jint y)
{
    (void)env;
    (void)cls;
    if (!fn)
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelGpuView_nativeKey(JNIEnv* env, jclass cls, jlong h, jlong fn, jint key)
{
    (void)env;
    (void)cls;
    if (!fn)
