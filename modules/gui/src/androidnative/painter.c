...
bool mel_gui__android_painter_register_jni(JNIEnv* env)
{
    ...
    return m_drawRect && m_drawOval && m_drawLine && m_drawRoundRect && m_drawText && m_setColor && m_setStyle && m_setStrokeWidth && m_setTextSize && m_ascent && s_style_fill && s_style_stroke;
}

static inline jint argb(Mel_Color k) { return (jint)(((u32)k.a << 24) | ((u32)k.r << 16) | ((u32)k.g << 8) | (u32)k.b); }
...
void mel_painter_fill_round_rect(Mel_Painter* p, Mel_Rect r, f32 radius, Mel_Color k)
{
    ...
    jvalue v[7] = { { .f = r.x }, { .f = r.y }, { .f = r.x + r.w }, { .f = r.y + r.h }, { .f = radius }, { .f = radius }, { .l = p->paint } };
