...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelFrame_nativeResize(JNIEnv* env, jclass cls, jlong h, jlong fn, jint w, jint height)
{
    (void)env;
    (void)cls;
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelFrame_nativeInsets(JNIEnv* env, jclass cls, jlong h, jlong fn, jintArray arr)
{
    (void)cls;
    if (!fn)
        return;
...
Mel_Frame_Insets mel_frame_insets(Mel_Gui_Handle h)
{
    ...
    jintArray arr = (jintArray)(*env)->CallStaticObjectMethod(env, s_cls, s_insets, (jobject)n->native);
    if ((*env)->ExceptionCheck(env))
    {
        (*env)->ExceptionClear(env);
        return out;
