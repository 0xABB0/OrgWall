...
Mel_Gui_Handle mel_checkbox_create_opt(Mel_Gui_Handle parent, Mel_CheckBox_Opt o)
{
    ...
    if (o.on_.on_toggled)
    {
        (*env)->CallStaticVoidMethod(env, s_cls, s_installToggle, view, mel_gui__android_pack(h), (jlong)(intptr_t)o.on_.on_toggled);
    }
...
JNIEXPORT void JNICALL Java_orgwall_melody_platform_MelCheckBox_nativeToggle(JNIEnv* env, jclass cls, jlong h, jlong fn, jboolean checked)
{
    (void)env;
    (void)cls;
    if (!fn)
