...
- (void)keyUp:(NSEvent*)e
{
    MelGuiTextField* tf = [self mel_owning_textfield];
    if (tf)
        mel_gui__macos_key(tf.handle, tf.keyboard, e, false);
    [super keyUp:e];
}
...
{
    if (![client isKindOfClass:[MelGuiTextField class]])
        return nil;
    ...
    if (!fe)
    {
        ...
        objc_setAssociatedObject(window, "mel_gui_field_editor", fe, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return fe;
}
...
Mel_Gui_Handle mel_textfield_create_opt(Mel_Gui_Handle parent, Mel_TextField_Opt o)
{
    ...
    @autoreleasepool
    {
        ...
        objc_setAssociatedObject(tf, "mel_gui_tf_delegate", delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
