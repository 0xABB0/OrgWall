...
NSView* mel_gui__macos_parent_view(Mel_Gui_Node* n)
{
    ...
    if (!p)
        return nil;
    if (p->content)
        return (__bridge NSView*)p->content;
    if (!p->native)
        return nil;
...
void mel_gui__macos_focus_in(Mel_Gui_Handle h, Mel_Gui_Focus_Cb fc)
{
    ...
    if (fc.on_focus_in)
        fc.on_focus_in(h, mel_gui_user(h));
}
...
void mel_gui__macos_focus_out(Mel_Gui_Handle h, Mel_Gui_Focus_Cb fc)
{
    ...
    if (fc.on_focus_out)
        fc.on_focus_out(h, mel_gui_user(h));
}
...
void mel_gui__macos_key(Mel_Gui_Handle h, Mel_Gui_Keyboard_Cb kc, NSEvent* e, bool down)
{
    ...
    {
        if (kc.on_key_down)
            kc.on_key_down(h, k, mel_gui_user(h));
    }
    else
    {
        if (kc.on_key_up)
            kc.on_key_up(h, k, mel_gui_user(h));
    }
}
