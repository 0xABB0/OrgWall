...
- (void)setFrameSize:(NSSize)newSize
{
    ...
    {
        self.on_.on_resize(self.handle, (i32)newSize.width, (i32)newSize.height, mel_gui_user(self.handle));
    }
}
...
{
    (void)e;
    if (self.pointer.on_pointer_enter)
        self.pointer.on_pointer_enter(self.handle, mel_gui_user(self.handle));
}
- (void)mouseExited:(NSEvent*)e
{
    (void)e;
    if (self.pointer.on_pointer_leave)
        self.pointer.on_pointer_leave(self.handle, mel_gui_user(self.handle));
}
...
- (void)mouseDragged:(NSEvent*)e
{
    ...
    if (self.pointer.on_pointer_move)
        self.pointer.on_pointer_move(self.handle, (i32)p.x, (i32)p.y, mel_gui_user(self.handle));
}
...
- (BOOL)becomeFirstResponder
{
    ...
    if (ok)
        mel_gui__macos_focus_in(self.handle, self.focus);
    return ok;
}
- (BOOL)resignFirstResponder
{
    ...
    if (ok)
        mel_gui__macos_focus_out(self.handle, self.focus);
    return ok;
}
