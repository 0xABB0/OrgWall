...
- (void)windowWillClose:(NSNotification*)note
{
    ...
    if (mel_gui_handle_is_none(dialog_h))
        return;
    ...
    for (u32 i = 0; i < count; i++)
    {
        ...
        if (!mel_gui_handle_eq(cw->parent, dialog_h))
            continue;
        if (!cw->native)
            continue;
...
- (void)windowDidResize:(NSNotification*)note
{
    ...
    {
        self.lifecycle.on_resize(self.dialog_handle, (i32)sz.width, (i32)sz.height, mel_gui_user(self.dialog_handle));
    }
}
...
{
    Mel_Gui_Handle h = mel_gui__node_new(MEL_GUI_HANDLE_NONE, o.x, o.y, o.w, o.h, 0, o.user, false, NULL, o.layout);
    ...
    @autoreleasepool
    {
        ...
        objc_setAssociatedObject(window, "mel_gui_dialog_delegate", delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        ...
        {
            [window setContentMinSize:NSMakeSize(o.min_w > 0 ? o.min_w : 0, o.min_h > 0 ? o.min_h : 0)];
        }
        if (o.max_w > 0 || o.max_h > 0)
        {
            [window setContentMaxSize:NSMakeSize(o.max_w > 0 ? o.max_w : CGFLOAT_MAX, o.max_h > 0 ? o.max_h : CGFLOAT_MAX)];
        }
        ...
        if (ow && ow->native)
        {
            ...
            NSPoint   origin = NSMakePoint(of.origin.x + (of.size.width - cw) * 0.5, of.origin.y + (of.size.height - ch) * 0.5);
...
void mel_dialog_close(Mel_Gui_Handle dialog, i32 result)
{
    ...
    MelGuiDialogDelegate* delegate = objc_getAssociatedObject(window, "mel_gui_dialog_delegate");
