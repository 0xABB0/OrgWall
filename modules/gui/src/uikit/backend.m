...
UIView* mel_gui__ios_parent_view(Mel_Gui_Node* n)
{
    ...
    if (!p)
        return nil;
    if (p->content)
        return (__bridge UIView*)p->content;
    if (!p->native)
        return nil;
...
void mel_gui_set_text(Mel_Gui_Handle h, str8 text)
{
    ...
    if ([obj isKindOfClass:[UIViewController class]])
        [(UIViewController*)obj setTitle:s];
    else if ([obj isKindOfClass:[UILabel class]])
        [(UILabel*)obj setText:s];
    else if ([obj isKindOfClass:[UITextField class]])
        [(UITextField*)obj setText:s];
    else if ([obj isKindOfClass:[UIButton class]])
        [(UIButton*)obj setTitle:s forState:UIControlStateNormal];
}
...
size mel_gui_get_text(Mel_Gui_Handle h, char* buf, size cap)
{
    ...
    if ([obj isKindOfClass:[UILabel class]])
        ...
        s = [(UITextField*)obj text];
    else if ([obj isKindOfClass:[UIButton class]])
        s = [(UIButton*)obj titleForState:UIControlStateNormal];
    if (!s)
        return 0;
...
void mel_gui_set_focus(Mel_Gui_Handle h)
{
    ...
    id obj = (__bridge id)n->native;
    if ([obj isKindOfClass:[UIView class]])
        [(UIView*)obj becomeFirstResponder];
