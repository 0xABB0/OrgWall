...
EM_JS(void, mel_web__js_init, (void), {
    ...
    style.textContent = '.mel-frame{background:#2b2b2b;color:#e0e0e0;overflow:hidden;font-size:14px}' +
                        '.mel-label{display:flex;align-items:center;white-space:pre-wrap;color:#e0e0e0;font-size:14px}' + '.mel-button{font-size:14px;cursor:pointer}' +
                        '.mel-textfield{font-size:14px}' + '.mel-checkbox{display:flex;align-items:center;color:#e0e0e0;font-size:14px;gap:6px}' +
                        '.mel-panel{background:rgba(255,255,255,0.04);border-radius:4px}' + '.mel-groupbox{border:1px solid #555;border-radius:4px;color:#cfcfcf;padding:0}' +
                        '.mel-groupbox > legend{padding:0 6px;font-size:13px}' + '.mel-groupbox-body{position:absolute;left:8px;top:24px;right:8px;bottom:8px}' +
                        '.mel-scroll{overflow:auto;background:#252525;border:1px solid #444}' + '.mel-canvas{display:block;background:#26333f}' + '.mel-gpu{display:block;background:#101418}' +
                        '.mel-tabview{display:flex;flex-direction:column}' + '.mel-splitter{display:flex}';
    document.head.appendChild(style);
});

EM_JS(int, mel_web__el_create, (const char* tag), {
    if (!globalThis.MelWeb)
        return 0;
...
{
    if (id <= 0 || id >= g_ctl_cap || !g_ctls[id].used)
        return NULL;
    return &g_ctls[id];
}
...
static Mel_Key web_key(int code)
{
    ...
    {
    case 8:
        return MEL_KEY_BACKSPACE;
    case 9:
        return MEL_KEY_TAB;
    case 13:
        return MEL_KEY_ENTER;
    case 27:
        return MEL_KEY_ESCAPE;
    case 32:
        return MEL_KEY_SPACE;
    case 37:
        return MEL_KEY_LEFT;
    case 38:
        return MEL_KEY_UP;
    case 39:
        return MEL_KEY_RIGHT;
    case 40:
        return MEL_KEY_DOWN;
    case 36:
        return MEL_KEY_HOME;
    case 35:
        return MEL_KEY_END;
    case 33:
        return MEL_KEY_PAGE_UP;
    case 34:
        return MEL_KEY_PAGE_DOWN;
    case 45:
        return MEL_KEY_INSERT;
    case 46:
        return MEL_KEY_DELETE;
    default:
        return MEL_KEY_NONE;
    }
}
...
EMSCRIPTEN_KEEPALIVE void mel_web__ev_click(int id)
{
    ...
    if (c && c->pointer.on_click)
        c->pointer.on_click(c->handle, mel_gui_user(c->handle));
}

EMSCRIPTEN_KEEPALIVE void mel_web__ev_input(int id)
{
    Mel_Web_Ctl* c = mel_web__ctl(id);
    if (!c || !c->textfield.on_text_changed)
        return;
    char buf[1024];
...
EMSCRIPTEN_KEEPALIVE void mel_web__ev_focus(int id, int in)
{
    ...
    if (in)
    {
        ...
        if (c->focus.on_focus_in)
            c->focus.on_focus_in(c->handle, mel_gui_user(c->handle));
    }
    else
    {
        if (mel_gui_handle_eq(mel_gui_focused(), c->handle))
            mel_gui__set_focused(MEL_GUI_HANDLE_NONE);
        if (c->focus.on_focus_out)
            c->focus.on_focus_out(c->handle, mel_gui_user(c->handle));
    }
}
...
EMSCRIPTEN_KEEPALIVE void mel_web__ev_pointer(int id, int type, int x, int y)
{
    ...
    {
    case 0:
        if (c->pointer.on_pointer_down)
            c->pointer.on_pointer_down(c->handle, x, y, u);
        break;
    case 1:
        if (c->pointer.on_pointer_move)
            c->pointer.on_pointer_move(c->handle, x, y, u);
        break;
    case 2:
        if (c->pointer.on_pointer_up)
            c->pointer.on_pointer_up(c->handle, x, y, u);
        break;
    }
}
...
EMSCRIPTEN_KEEPALIVE void mel_web__ev_key(int id, int down, int code, int codepoint)
{
    ...
    if (down)
    {
        if (c->keyboard.on_key_down)
            c->keyboard.on_key_down(c->handle, web_key(code), u);
    }
    else
    {
        if (c->keyboard.on_key_up)
            c->keyboard.on_key_up(c->handle, web_key(code), u);
    }
    if (down && codepoint && c->keyboard.on_char)
        c->keyboard.on_char(c->handle, (u32)codepoint, u);
}
...
EMSCRIPTEN_KEEPALIVE void mel_web__ev_select(int id, int index)
{
    Mel_Web_Ctl* c = mel_web__ctl(id);
    if (c && c->on_select)
        c->on_select(c->handle, index, mel_gui_user(c->handle));
}
...
void mel_gui_set_text(Mel_Gui_Handle h, str8 text)
{
    ...
    else if (k == MEL_WEB_INPUT)
        mel_web__el_set_value(id, buf);
    else
        mel_web__el_text(id, buf);
}
...
size mel_gui_get_text(Mel_Gui_Handle h, char* buf, size cap)
{
    ...
    if (!id)
        return 0;
    Mel_Web_Ctl* c = mel_web__ctl(id);
    if (c && c->kind == MEL_WEB_INPUT)
        return (size)mel_web__el_get_value(id, buf, (int)cap);
...
void mel_gui_set_bounds(Mel_Gui_Handle h, i32 x, i32 y, i32 width, i32 height)
{
    ...
    if (c && c->gpu_view.on_resize)
        c->gpu_view.on_resize(h, width, height, mel_gui_user(h));
}
...
void mel_gui_set_visible(Mel_Gui_Handle h, bool visible)
{
    ...
    int id = mel_web__id_of(n);
    if (id)
        mel_web__el_visible(id, visible);
}
...
void mel_gui_set_enabled(Mel_Gui_Handle h, bool enabled)
{
    ...
    if (id)
        mel_web__el_enabled(id, enabled);
}
