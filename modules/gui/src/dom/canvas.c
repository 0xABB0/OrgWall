...
void mel_painter_clear(Mel_Painter* p, Mel_Color color) { mel_web__paint_clear(p->canvas, mel_web__rgba(color)); }
void mel_painter_fill_rect(Mel_Painter* p, Mel_Rect r, Mel_Color color) { mel_web__paint_fill_rect(p->canvas, r.x, r.y, r.w, r.h, mel_web__rgba(color)); }
void mel_painter_fill_ellipse(Mel_Painter* p, Mel_Rect r, Mel_Color color) { mel_web__paint_fill_ellipse(p->canvas, r.x, r.y, r.w, r.h, mel_web__rgba(color)); }
void mel_painter_stroke_rect(Mel_Painter* p, Mel_Rect r, Mel_Color color, f32 width) { mel_web__paint_stroke_rect(p->canvas, r.x, r.y, r.w, r.h, mel_web__rgba(color), width); }
void mel_painter_draw_line(Mel_Painter* p, Mel_Vec2 a, Mel_Vec2 b, Mel_Color color, f32 width) { mel_web__paint_line(p->canvas, a.x, a.y, b.x, b.y, mel_web__rgba(color), width); }
void mel_painter_fill_round_rect(Mel_Painter* p, Mel_Rect r, f32 radius, Mel_Color color) { mel_web__paint_round_rect(p->canvas, r.x, r.y, r.w, r.h, radius, mel_web__rgba(color)); }
void mel_painter_draw_text(Mel_Painter* p, str8 text, Mel_Vec2 pos, Mel_Color color, f32 size)
{
    char b[1024];
    mel_web__paint_text(p->canvas, mel_web__cstr(text, b, sizeof b), pos.x, pos.y, mel_web__rgba(color), size);
}
