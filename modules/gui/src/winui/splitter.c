...
static void split_relayout(Mel_Win32_Splitter* s)
{
    ...
    for (i32 i = 0; i < s->pane_count; i++)
    {
        i32 ext = (i == s->pane_count - 1) ? (avail - used) : (s->sizes[i] * avail / sum);
        if (ext < 0)
            ext = 0;
        s->sizes[i] = ext;

        if (s->vertical)
            MoveWindow(s->panes[i], 0, used, cross, ext, TRUE);
        else
            MoveWindow(s->panes[i], used, 0, ext, cross, TRUE);
        ...
        if (pn)
        {
            ...
            if (pn->layout)
                mel_gui__layout_arrange(pn->self);
        }
...
static i32 hit_divider(Mel_Win32_Splitter* s, i32 axis_pos)
{
    ...
    {
        cursor += s->sizes[i];
        if (axis_pos >= cursor && axis_pos < cursor + DIVIDER_THICK)
            return i;
...
Mel_Gui_Handle mel_splitter_create_opt(Mel_Gui_Handle parent, Mel_Splitter_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(0, SPLITTER_CLASS, NULL, mel_gui__win32_child_style(n, o.disabled) | WS_CLIPCHILDREN, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
...
Mel_Gui_Handle mel_splitpane_create_opt(Mel_Gui_Handle splitter, Mel_SplitPane_Opt o)
{
    ...
    if (!s || s->pane_count >= MEL_SPLITTER_MAX_PANES)
        return MEL_GUI_HANDLE_NONE;

    Mel_Gui_Handle h = mel_gui__node_new(splitter, 0, 0, 0, 0, o.id, o.user, false, &o.layoutable, o.layout);
    Mel_Gui_Node*  n = mel_gui__node(h);
    if (!n)
        return h;

    HWND pane = mel_gui__win32_make_container(swnd, 0, 0, 1, 1, h, (Mel_Gui_Pointer_Cb){ 0 }, (Mel_Gui_Focus_Cb){ 0 }, (Mel_Gui_Keyboard_Cb){ 0 }, false, false);
