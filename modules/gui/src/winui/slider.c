...
static LRESULT CALLBACK slider_subclass(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, UINT_PTR id, DWORD_PTR ref)
{
    (void)ref;
...
void mel_slider_set_value(Mel_Gui_Handle h, i32 value)
{
    Mel_Gui_Node* n = mel_gui__node(h);
    if (n && n->native)
        SendMessageW((HWND)n->native, TBM_SETPOS, TRUE, value);
}
