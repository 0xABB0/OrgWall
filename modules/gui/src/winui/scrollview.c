...
static void scroll_apply(HWND hwnd, Mel_Win32_Scroll* s)
{
    ...
    i32 maxx = s->content_w - pw;
    if (maxx < 0)
        maxx = 0;
    i32 maxy = s->content_h - ph;
    if (maxy < 0)
        maxy = 0;
    if (s->pos_x > maxx)
        s->pos_x = maxx;
...
static LRESULT CALLBACK scroll_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_VSCROLL:
        if (lp)
        {
            SendMessageW((HWND)lp, MEL_REFLECT(WM_VSCROLL), wp, lp);
            return 0;
        }
        if (s)
        {
...
Mel_Gui_Handle mel_scrollview_create_opt(Mel_Gui_Handle parent, Mel_ScrollView_Opt o)
{
    ...
    HWND outer = CreateWindowExW(0, SCROLL_CLASS, NULL, style, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = outer;
    if (!outer)
        return h;
    ...
    HWND inner = mel_gui__win32_make_container(outer, 0, 0, cw, ch, h, (Mel_Gui_Pointer_Cb){ 0 }, o.focus, (Mel_Gui_Keyboard_Cb){ 0 }, false, false);
