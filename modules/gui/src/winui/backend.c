...
bool mel_gui__win32_subclass_common(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_KEYDOWN:
        ...
        if (c->keyboard.on_key_down)
            c->keyboard.on_key_down(h, vk_to_key(wp), u);
        return false;
    case WM_KEYUP:
        if (c->keyboard.on_key_up)
            c->keyboard.on_key_up(h, vk_to_key(wp), u);
        return false;
    case WM_CHAR:
        if (wp == VK_TAB)
            return true;
        if (c->keyboard.on_char)
            c->keyboard.on_char(h, (u32)wp, u);
        return false;
    default:
        return false;
    }
...
void mel_gui_set_text(Mel_Gui_Handle h, str8 text)
{
    ...
    if (n->is_screen)
    {
        ...
        target = top ? (HWND)top->native : NULL;
    }
    else
    {
        target = (HWND)n->native;
    }
    if (!target)
        return;
...
void mel_gui_set_visible(Mel_Gui_Handle h, bool visible)
{
    ...
    if (mel_gui__is_toplevel(n))
    {
        ...
        if (f && !f->first_show_done)
        {
            ...
            {
            case MEL_FRAME_MINIMIZED:
                ncmd = SW_SHOWMINIMIZED;
                break;
            case MEL_FRAME_MAXIMIZED:
...
void mel_gui_set_enabled(Mel_Gui_Handle h, bool enabled)
{
    Mel_Gui_Node* n = mel_gui__node(h);
    if (n && n->native)
        EnableWindow((HWND)n->native, enabled);
}
...
void mel_gui_invalidate(Mel_Gui_Handle h)
{
    Mel_Gui_Node* n = mel_gui__node(h);
    if (n && n->native)
        InvalidateRect((HWND)n->native, NULL, TRUE);
}
