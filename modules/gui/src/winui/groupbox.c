...
static LRESULT CALLBACK groupbox_subclass(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, UINT_PTR id, DWORD_PTR ref)
{
    HWND inner = (HWND)ref;
    if (msg == WM_SIZE)
    {
...
Mel_Gui_Handle mel_groupbox_create_opt(Mel_Gui_Handle parent, Mel_GroupBox_Opt o)
{
    ...
    HWND box = CreateWindowExW(0, L"BUTTON", wbuf, mel_gui__win32_child_style(n, o.disabled) | BS_GROUPBOX, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = box;
    if (!box)
        return h;
    ...
    RECT ib;
    inner_bounds(box, &ib);
    HWND inner = mel_gui__win32_make_container(box, ib.left, ib.top, ib.right, ib.bottom, h, (Mel_Gui_Pointer_Cb){ 0 }, o.focus, (Mel_Gui_Keyboard_Cb){ 0 }, false, false);
    n->content = inner;
