...
static LRESULT CALLBACK button_subclass(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, UINT_PTR id, DWORD_PTR ref)
{
    (void)ref;
...
Mel_Gui_Handle mel_button_create_opt(Mel_Gui_Handle parent, Mel_Button_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(0, L"BUTTON", wbuf, mel_gui__win32_child_style(n, o.disabled) | WS_TABSTOP | BS_PUSHBUTTON, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
