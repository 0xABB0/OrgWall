...
static LRESULT CALLBACK container_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_LBUTTONDOWN:
        ...
        return 0;
    case WM_MOUSEMOVE:
        if (p && p->pointer.on_pointer_move)
            p->pointer.on_pointer_move(h, GET_X_LPARAM(lp), GET_Y_LPARAM(lp), u);
        return 0;
    case WM_LBUTTONUP:
        if (p && p->pointer.on_pointer_up)
            p->pointer.on_pointer_up(h, GET_X_LPARAM(lp), GET_Y_LPARAM(lp), u);
        if (p && p->pointer.on_click)
            p->pointer.on_click(h, u);
        return 0;
    case WM_NCDESTROY:
                                   ...
                                   bool disabled)
{
    ...
    HWND hwnd = CreateWindowExW(0, CONTAINER_CLASS, NULL, style, x, y, w, h, parent, NULL, current_hinst, NULL);
    if (!hwnd)
        return NULL;
