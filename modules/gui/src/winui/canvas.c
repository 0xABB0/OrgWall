...
static LRESULT CALLBACK canvas_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_LBUTTONDOWN:
        SetFocus(hwnd);
        SetCapture(hwnd);
        if (c && c->pointer.on_pointer_down)
            c->pointer.on_pointer_down(h, GET_X_LPARAM(lp), GET_Y_LPARAM(lp), u);
        return 0;
    case WM_MOUSEMOVE:
        if (c && c->pointer.on_pointer_move)
            c->pointer.on_pointer_move(h, GET_X_LPARAM(lp), GET_Y_LPARAM(lp), u);
        return 0;
    case WM_LBUTTONUP:
        ReleaseCapture();
        if (c && c->pointer.on_pointer_up)
            c->pointer.on_pointer_up(h, GET_X_LPARAM(lp), GET_Y_LPARAM(lp), u);
        return 0;
    case WM_GETDLGCODE:
...
Mel_Gui_Handle mel_canvas_create_opt(Mel_Gui_Handle parent, Mel_Canvas_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(0, CANVAS_CLASS, NULL, mel_gui__win32_child_style(n, false) | WS_TABSTOP, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
