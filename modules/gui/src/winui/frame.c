...
static Mel_Key vk_to_key_frame(WPARAM vk)
{
    ...
    case VK_SHIFT:
        return MEL_KEY_SHIFT;
    case VK_CONTROL:
        return MEL_KEY_CONTROL;
    case VK_MENU:
        return MEL_KEY_ALT;
    case VK_F1:
        return MEL_KEY_F1;
...
static LRESULT CALLBACK frame_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_SIZE:
    {
        ...
        if (f && f->lifecycle.on_resize)
            f->lifecycle.on_resize(h, cw, ch, u);
        return 0;
    }
    case WM_MOVE:
    {
    ...
    case WM_SHOWWINDOW:
        if (f)
        {
            if (wp)
            {
                if (f->lifecycle.on_show)
                    f->lifecycle.on_show(h, u);
            }
            else
            {
                if (f->lifecycle.on_hide)
                    f->lifecycle.on_hide(h, u);
            }
        }
        return 0;
    case WM_ENABLE:
        if (f && f->lifecycle.on_enable)
            f->lifecycle.on_enable(h, wp != 0, u);
        return 0;
    ...
    case WM_SETFOCUS:
        ...
        if (f && f->base.focus.on_focus_in)
            f->base.focus.on_focus_in(h, u);
        return 0;
    case WM_KILLFOCUS:
        if (mel_gui_handle_eq(mel_gui_focused(), h))
            mel_gui__set_focused(MEL_GUI_HANDLE_NONE);
        if (f && f->base.focus.on_focus_out)
            f->base.focus.on_focus_out(h, u);
        return 0;
    case WM_KEYDOWN:
        if (f && f->base.keyboard.on_key_down)
            f->base.keyboard.on_key_down(h, vk_to_key_frame(wp), u);
        return 0;
    case WM_KEYUP:
        if (f && f->base.keyboard.on_key_up)
            f->base.keyboard.on_key_up(h, vk_to_key_frame(wp), u);
        return 0;
    case WM_CHAR:
        if (f && f->base.keyboard.on_char)
            f->base.keyboard.on_char(h, (u32)wp, u);
        return 0;
    case WM_CLOSE:
    {
...
Mel_Gui_Handle mel_frame_create_opt(Mel_Frame_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(ex_style, FRAME_CLASS, wbuf, style, wx, wy, rc.right - rc.left, rc.bottom - rc.top, owner_hwnd, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
    ...
    if (o.icon_large)
        SendMessageW(hwnd, WM_SETICON, ICON_BIG, (LPARAM)o.icon_large);
    if (o.icon_small)
        SendMessageW(hwnd, WM_SETICON, ICON_SMALL, (LPARAM)o.icon_small);
    if (o.not_closable)
    {
