...
static LRESULT CALLBACK checkbox_subclass(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, UINT_PTR id, DWORD_PTR ref)
{
    (void)ref;
...
Mel_Gui_Handle mel_checkbox_create_opt(Mel_Gui_Handle parent, Mel_CheckBox_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(0, L"BUTTON", wbuf, mel_gui__win32_child_style(n, o.disabled) | WS_TABSTOP | BS_AUTOCHECKBOX, n->x, n->y, n->width, n->height, par, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
