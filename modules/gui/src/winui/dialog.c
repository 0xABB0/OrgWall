...
static LRESULT CALLBACK dialog_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_SIZE:
    {
        ...
        if (d && d->frame.lifecycle.on_resize)
            d->frame.lifecycle.on_resize(h, cw, ch, u);
        return 0;
    }
    ...
    case WM_SETFOCUS:
        ...
        if (d && d->frame.base.focus.on_focus_in)
            d->frame.base.focus.on_focus_in(h, u);
        return 0;
    case WM_KILLFOCUS:
        if (mel_gui_handle_eq(mel_gui_focused(), h))
            mel_gui__set_focused(MEL_GUI_HANDLE_NONE);
        if (d && d->frame.base.focus.on_focus_out)
            d->frame.base.focus.on_focus_out(h, u);
        return 0;
    case WM_KEYDOWN:
        if (d && d->frame.base.keyboard.on_key_down)
            d->frame.base.keyboard.on_key_down(h, (Mel_Key)wp, u);
        return 0;
    case WM_CLOSE:
    {
    ...
    case WM_DESTROY:
    {
        ...
        if (d && d->on_.on_result)
            d->on_.on_result(h, result, u);
...
Mel_Gui_Handle mel_dialog_create_opt(Mel_Dialog_Opt o)
{
    ...
    HWND hwnd = CreateWindowExW(ex_style, DIALOG_CLASS, wbuf, style, wx, wy, rc.right - rc.left, rc.bottom - rc.top, owner_hwnd, NULL, current_hinst, NULL);
    n->native = hwnd;
    if (!hwnd)
        return h;
