...
static bool is_child_of(const Mel_Gui_Node* child, Mel_Gui_Handle parent) { return mel_gui_handle_eq(child->parent, parent); }
...
{
    *pw = c->layoutable.fixed_w ? c->layoutable.fixed_w : c->layoutable.preferred_w ? c->layoutable.preferred_w : c->width;
    *ph = c->layoutable.fixed_h ? c->layoutable.fixed_h : c->layoutable.preferred_h ? c->layoutable.preferred_h : c->height;
}

static void column_measure(Mel_Layout* layout, Mel_Gui_Handle container, i32 avail_w, i32 avail_h, i32* out_w, i32* out_h)
{
    ...
    for (u32 i = 0; i < count; i++)
    {
        ...
        if (!is_child_of(c, container) || c->hidden)
            continue;
    ...
    if (visible_count > 1)
        primary += col->spacing * (visible_count - 1);
