...
void mel_latch_init(Mel_Latch* l, u32 count) { atomic_store_explicit(&l->_count, count, memory_order_relaxed); }
...
bool mel_latch_wait_for(Mel_Latch* l, i64 timeout_ns)
{
    ...
    if (c == 0)
        return true;
    if (!mel_futex_wait_for(&l->_count, c, timeout_ns))
        return false;
    return atomic_load_explicit(&l->_count, memory_order_acquire) == 0;
}

bool mel_latch_is_ready(const Mel_Latch* l) { return atomic_load_explicit((_Atomic(u32)*)&l->_count, memory_order_acquire) == 0; }
