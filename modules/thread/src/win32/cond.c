...
void mel_cond_destroy(Mel_Cond* c) { (void)c; }

void mel_cond_wait(Mel_Cond* c, Mel_Mutex* m) { SleepConditionVariableSRW(MEL__COND(c), (SRWLOCK*)m->_storage, INFINITE, 0); }
...
void mel_cond_signal(Mel_Cond* c) { WakeConditionVariable(MEL__COND(c)); }

void mel_cond_broadcast(Mel_Cond* c) { WakeAllConditionVariable(MEL__COND(c)); }
