...
bool mel_thread_spawn_opt(Mel_Thread* t, Mel_Thread_Fn fn, void* user, Mel_Thread_Spawn_Opt opt)
{
    ...
    if (pthread_attr_init(&attr) != 0)
        return false;
    if (opt.stack_size)
        pthread_attr_setstacksize(&attr, opt.stack_size);
    ...
    t->id = (Mel_Thread_Id)(uintptr_t)MEL__THREAD_HANDLE(t);
    return true;
}
...
{
    void* retval = NULL;
    if (pthread_join(MEL__THREAD_HANDLE(t), &retval) != 0)
        return false;
    if (exit_code)
...
bool mel_thread_detach(Mel_Thread* t) { return pthread_detach(MEL__THREAD_HANDLE(t)) == 0; }

bool mel_thread_is_finished(const Mel_Thread* t) { return atomic_load_explicit(&t->finished, memory_order_acquire) != 0; }

Mel_Thread_Id mel_thread_current_id(void)
{
#if defined(__linux__) || defined(__ANDROID__)
    return (Mel_Thread_Id)syscall(SYS_gettid);
#else
    return (Mel_Thread_Id)(uintptr_t)pthread_self();
#endif
}

bool mel_thread_id_equal(Mel_Thread_Id a, Mel_Thread_Id b) { return a == b; }

void mel_thread_yield(void) { sched_yield(); }
