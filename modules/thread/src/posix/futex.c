...
static int mel__futex(_Atomic(u32)* addr, int op, u32 val, const struct timespec* ts) { return (int)syscall(SYS_futex, (uint32_t*)addr, op | FUTEX_PRIVATE_FLAG, val, ts, NULL, 0); }

void mel_futex_wait(_Atomic(u32)* addr, u32 expected) { mel__futex(addr, FUTEX_WAIT, expected, NULL); }
...
void mel_futex_wake_one(_Atomic(u32)* addr) { mel__futex(addr, FUTEX_WAKE, 1, NULL); }

void mel_futex_wake_all(_Atomic(u32)* addr) { mel__futex(addr, FUTEX_WAKE, INT32_MAX, NULL); }

#elif defined(__EMSCRIPTEN__)
#include <emscripten/threading.h>

void mel_futex_wait(_Atomic(u32)* addr, u32 expected) { emscripten_futex_wait((void*)addr, expected, INFINITY); }
...
void mel_futex_wake_one(_Atomic(u32)* addr) { emscripten_futex_wake((void*)addr, 1); }

void mel_futex_wake_all(_Atomic(u32)* addr) { emscripten_futex_wake((void*)addr, INT32_MAX); }
