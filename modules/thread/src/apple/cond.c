...
bool mel_cond_init(Mel_Cond* c) { return pthread_cond_init(MEL__COND(c), NULL) == 0; }

void mel_cond_destroy(Mel_Cond* c) { pthread_cond_destroy(MEL__COND(c)); }

void mel_cond_wait(Mel_Cond* c, Mel_Mutex* m) { pthread_cond_wait(MEL__COND(c), MEL__MUTEX(m)); }
...
void mel_cond_signal(Mel_Cond* c) { pthread_cond_signal(MEL__COND(c)); }

void mel_cond_broadcast(Mel_Cond* c) { pthread_cond_broadcast(MEL__COND(c)); }
