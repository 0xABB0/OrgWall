...
{
    if (WaitForSingleObject(MEL__THREAD_HANDLE(t), INFINITE) != WAIT_OBJECT_0)
        return false;
    if (exit_code)
    {
        DWORD code = 0;
...
Mel_Thread_Id mel_thread_current_id(void) { return (Mel_Thread_Id)GetCurrentThreadId(); }

bool mel_thread_id_equal(Mel_Thread_Id a, Mel_Thread_Id b) { return a == b; }

void mel_thread_yield(void) { SwitchToThread(); }
