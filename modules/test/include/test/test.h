...
#define MEL_TEST(suite_, name_)                                                                                                                       \
    ...
    static Mel_Test                          mel__node_##suite_##_##name_ = { #suite_, #name_, mel__test_##suite_##_##name_, __FILE__, __LINE__, 0 }; \
    __attribute__((constructor)) static void mel__ctor_##suite_##_##name_(void) { mel_test_register(&mel__node_##suite_##_##name_); }                 \
    static void                              mel__test_##suite_##_##name_(void)
...
#define MEL_FAIL(msg)                                   \
    do                                                  \
    {                                                   \
        mel_test_fail(__FILE__, __LINE__, "%s", (msg)); \
        mel_test_abort();                               \
    } while (0)
#define MEL_SKIP(reason)       \
    do                         \
    {                          \
        mel_test_skip(reason); \
        mel_test_abort();      \
    } while (0)
