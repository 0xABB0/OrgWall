...
static inline f32 mel_quat_angle(Mel_Quat a, Mel_Quat b)
{
    f32x4 m = a.v * b.v;
    f32 dot = m[0] + m[1] + m[2] + m[3];
    if (dot < 0.0f) dot = -dot;
    if (dot > 1.0f) dot = 1.0f;
    return 2.0f * __builtin_acosf(dot);
}

static inline Mel_Quat mel_quat_rotateX(f32 radians)
{
    f32 half = radians * 0.5f;
    return (Mel_Quat){ .v = (f32x4){__builtin_sinf(half), 0, 0, __builtin_cosf(half)} };
}

static inline Mel_Quat mel_quat_rotateY(f32 radians)
{
    f32 half = radians * 0.5f;
    return (Mel_Quat){ .v = (f32x4){0, __builtin_sinf(half), 0, __builtin_cosf(half)} };
}

static inline Mel_Quat mel_quat_rotateZ(f32 radians)
{
    f32 half = radians * 0.5f;
    return (Mel_Quat){ .v = (f32x4){0, 0, __builtin_sinf(half), __builtin_cosf(half)} };
}

static inline Mel_Quat mel_quat_lerp(Mel_Quat a, Mel_Quat b, f32 t)
{
    Mel_Quat r = { .v = a.v + (b.v - a.v) * t };
    return mel_quat_normalize(r);
}

static inline Mel_Vec3 mel_quat_to_euler(Mel_Quat q)
{
    f32 sinr_cosp = 2.0f * (q.w * q.x + q.y * q.z);
    f32 cosr_cosp = 1.0f - 2.0f * (q.x * q.x + q.y * q.y);
    f32 pitch = __builtin_atan2f(sinr_cosp, cosr_cosp);

    f32 sinp = 2.0f * (q.w * q.y - q.z * q.x);
    f32 yaw;
    if (__builtin_fabsf(sinp) >= 1.0f)
        yaw = __builtin_copysignf(3.14159265358979323846f * 0.5f, sinp);
    else
        yaw = __builtin_asinf(sinp);

    f32 siny_cosp = 2.0f * (q.w * q.z + q.x * q.y);
    f32 cosy_cosp = 1.0f - 2.0f * (q.y * q.y + q.z * q.z);
    f32 roll = __builtin_atan2f(siny_cosp, cosy_cosp);

    return mel_vec3(pitch, yaw, roll);
}

static inline Mel_Vec3 mel_quat_mulXYZ(Mel_Quat a, Mel_Quat b)
{
    Mel_Quat r = mel_quat_mul(a, b);
    return mel_vec3(r.x, r.y, r.z);
}
