...
static mpfr_prec_t mel_tuning_precision(void) { return MEL_TUNING_PRECISION; }
...
void mel_tuning_custom_set_step(Mel_Tuning* t, uint32_t idx, mpfr_srcptr ratio) { mpfr_set(t->custom.steps[idx], ratio, MPFR_RNDN); }
...
void mel_tuning_custom_set_eq_ratio(Mel_Tuning* t, mpfr_srcptr ratio) { mpfr_set(t->custom.eq_ratio, ratio, MPFR_RNDN); }
...
uint32_t mel_tuning_period_length(const Mel_Tuning* t)
{
    ...
    {
    case MEL_TUNING_ED:
        return t->ed.divisions;
    case MEL_TUNING_EDO:
        return t->edo.divisions;
    case MEL_TUNING_CUSTOM:
        return t->custom.steps_count;
    }
    return 0;
}

uint8_t mel_tuning_is_periodic(const Mel_Tuning* t) { return mel_tuning_period_length(t) > 0; }
