...
int32_t mel_note_scale_count(const Mel_NoteScale* ns) { return ns->pitch_scale.count; }
...
Mel_Note mel_note_scale_get(const Mel_NoteScale* ns, int32_t idx)
{
    ...
    {
        Mel_Note guessed = ns->notation->enharmonic.guess_note(ns->notation->enharmonic.ctx, (Mel_Notation*)ns->notation, note.pitch);
        return guessed;
    }
