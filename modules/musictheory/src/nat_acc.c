...
void mel_nat_acc_add_accidental(Mel_NatAccNotation* nn, const char* symbol, int32_t value) { mel_symbol_code_add(&nn->acc_code, symbol, value); }

void mel_nat_acc_add_accidental_at(Mel_NatAccNotation* nn, const char* symbol, int32_t value, int32_t position) { mel_symbol_code_add_at(&nn->acc_code, symbol, value, position); }
...
Mel_Note mel_nat_acc_note_by_index(Mel_NatAccNotation* nn, int32_t nat_index, int32_t* acc_vector, int32_t acc_count)
{
    ...
    int32_t acc_sum = 0;
    for (int32_t i = 0; i < acc_count; i++)
        acc_sum += acc_vector[i];
