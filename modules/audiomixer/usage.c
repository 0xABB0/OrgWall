...
int main(void)
{
    ...
    Mel_Mixer_Voice monitor = { 0 };
    if (!mel_audiocapture_status_failed(cap.status))
    {
        Mel_Mixer_Source* mic = mel_mixer_pull_source(alloc, (Mel_Mixer_Pull_Fn)mel_audiocapture_read,
                                                      cap.capture, 1, 48000);
        monitor = mel_mixer_play(eng, mic);
        mel_mixer_set_volume(eng, monitor, 0.8f);
    }

    Mel_Mixer_Tap* tap = mel_mixer_tap_open(eng, alloc, 48000 / 4);
    ...
    {
        if (mel_mixer_tap_available(tap) < WINDOW)
        {
            mel_thread_sleep(5 * 1000 * 1000);
            continue;
        }

        u32 got = mel_mixer_tap_read(tap, frames, WINDOW);
        for (u32 i = 0; i < got; i++)
    ...
    if (mel_mixer_voice_valid(eng, monitor))
        mel_mixer_stop(eng, monitor);

    Mel_Mixer_Status moved = mel_mixer_set_device(eng, MEL_AUDIOOUT_NULL);
    if (mel_mixer_warned(moved))
