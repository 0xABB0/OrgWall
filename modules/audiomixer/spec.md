# audiomixer — device binding, taps & pull source specification
...
```
Mel_Mixer_Opt.device : Mel_AudioOut
```

`mel_mixer_create` requires `mel_audioout_init`: the engine's device plane is
...
`mel_mixer_set_device(eng, device)` rebinds live: the provider stream closes,
...
(dead handle; previous binding stays). `mel_mixer_device` returns the current
binding; `mel_mixer_device_status` is the live binding state.

## Device loss & interruption

Pinned device vanishes: `Mel_Mixer_Device_Event{ lost = true }` fires on
`mel_mixer_device_events` and the engine holds — mixing pauses, voices
freeze, the clock stops; `mel_mixer_device_status` reports
...
```
Mel_Mixer_Tap* mel_mixer_tap_open(eng, alloc, ring_frames);               // post-master mix
Mel_Mixer_Tap* mel_mixer_voice_tap_open(eng, voice, alloc, ring_frames);  // one voice, post-fader
u32 mel_mixer_tap_read(tap, interleaved_dst, max_frames);
```
...
recording, streaming. Engine-format interleaved f32 (`mel_mixer_caps`).
...
in `mel_mixer_tap_dropped_frames` — measured, never silent. A voice tap dies
...
- `audioplayback` — the device plane (pull-mode stream the mix thread
  serves).
- `pcm` — resampler contract (`Mel_Mixer_Resampler` = `Mel_Pcm_Resampler`),
  tap rings.
- `audiopolicy` — interruption events (hold/auto-resume). Subscribed only
  ...
  `mel_mixer_create` to receive `{ interrupted }`/`{ resumed }` — without it
  the OS taking the hardware surfaces as device loss. During the hold,
  `mel_mixer_device_status` reports `ERROR | INTERRUPTED`.
