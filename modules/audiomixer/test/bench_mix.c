...
static Mel_Mixer_Opt bench_opt(void)
{
...
static Mel_Mixer_Source* bench_stereo_source(const Mel_Alloc* a)
{
    ...
    Mel_Mixer_Source* src = mel_mixer_pcm_from_float(a, buf, BENCH_SRC_FRAMES, BENCH_CH, BENCH_SR, MEL_MIXER_OWNERSHIP_OWNED);
    mel_dealloc(a, buf);
...
static void bench_run(u32 voices, Bench_Result* out)
{
    ...
    Mel_Mixer* eng = mel_mixer_create_offline(a, bench_opt());
    MEL_REQUIRE_NOT_NULL(eng);

    Mel_Mixer_Source* src = bench_stereo_source(a);
    ...
    mel_mixer_render(eng, dst, BENCH_BLOCK);
    MEL_REQUIRE_EQ(mel_mixer_active_voice_count(eng), voices);
    ...
    {
        u32 produced = mel_mixer_render(eng, dst, BENCH_BLOCK);
