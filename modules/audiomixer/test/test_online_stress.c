...
static Mel_Mixer_Opt online_opt(void)
{
...
static Mel_Mixer_Opt offline_opt(void)
{
    Mel_Mixer_Opt opt = online_opt();
    return opt;
}

static Mel_Mixer_Source* sine_source(const Mel_Alloc* a, u32 frames, u32 channels, u32 samplerate, f32 freq)
{
    ...
    Mel_Mixer_Source* src = mel_mixer_pcm_from_float(a, buf, frames, channels, samplerate, MEL_MIXER_OWNERSHIP_OWNED);
    mel_dealloc(a, buf);
    return src;
}
...
static int stress_player(void* user)
{
    ...
    {
        Mel_Mixer_Source* src = ctx->sources[i % ctx->source_count];
        Mel_Mixer_Voice   v = mel_mixer_play_ex(ctx->eng, src, 0.3f, ((f32)(i % 3u) - 1.0f), false);
        plays++;

        if ((i & 7u) == 0u)
            mel_mixer_set_play_speed(ctx->eng, v, 1.0 + (f64)(i % 4u) * 0.7);
        if ((i & 15u) == 0u)
            mel_mixer_stop(ctx->eng, v);
        if ((i & 63u) == 0u)
            mel_mixer_stop_all(ctx->eng);
...
static void run_concurrent_burst(const Mel_Alloc* a, Mel_Mixer* eng, Mel_Mixer_Source** sources, u32 source_count, i64 run_ns)
{
    Stress_Ctx ctx = {
        .eng = eng,
        .sources = sources,
...
static Mel_Mixer_Source** make_sources(const Mel_Alloc* a, u32* out_count)
{
    u32                count = 4u;
    Mel_Mixer_Source** s = mel_alloc(a, sizeof(Mel_Mixer_Source*) * count);
    ...
    for (u32 i = 0; i < count; i++)
        mel_mixer_pcm_set_loop(s[i], true, 0.0);
    *out_count = count;
    return s;
}

static void free_sources(const Mel_Alloc* a, Mel_Mixer_Source** s, u32 count)
{
    for (u32 i = 0; i < count; i++)
...
MEL_TEST(online, live_play_destroy_no_crash_no_leak)
{
    ...
    if (plane)
    {
        eng = mel_mixer_create(a, online_opt());
        online = eng != NULL;
        if (!online)
            device_plane_down();
    }
    ...
    if (!online)
    {
        ...
        eng = mel_mixer_create_offline(a, offline_opt());
    ...
    u32                source_count = 0u;
    Mel_Mixer_Source** sources = make_sources(a, &source_count);
    ...
    if (!online)
    {
        ...
        {
            mel_mixer_render(eng, dst, BLOCK);
            if ((blk & 255u) == 0u)
    ...
    if (online)
    {
        Mel_Mixer* eng2 = mel_mixer_create(a, online_opt());
        MEL_REQUIRE_NOT_NULL(eng2);
        for (u32 i = 0; i < source_count; i++)
            mel_mixer_play_ex(eng2, sources[i], 0.2f, 0.0f, false);
        mel_mixer_destroy(eng2);
    }
...
MEL_TEST(online, play_then_immediate_destroy)
{
    ...
    for (u32 iter = 0; iter < 8u; iter++)
    {
        ...
        Mel_Mixer_Source* s0 = sine_source(a, SR, 1u, SR, 220.0f);
        Mel_Mixer_Source* s1 = sine_source(a, SR / 2u, MAX_VOICE_CH, SR, 660.0f);

        f32* dst = online ? NULL : mel_alloc(a, sizeof(f32) * BLOCK * CH);
        for (u32 i = 0; i < 32u; i++)
        {
            mel_mixer_play_ex(eng, s0, 0.2f, 0.0f, false);
            Mel_Mixer_Voice v = mel_mixer_play_ex(eng, s1, 0.2f, 0.0f, false);
            mel_mixer_set_play_speed(eng, v, 3.5);
            if (!online && (i & 7u) == 0u)
                mel_mixer_render(eng, dst, BLOCK);
        }
        if (dst != NULL)
