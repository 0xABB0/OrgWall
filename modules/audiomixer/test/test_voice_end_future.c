...
static Mel_Mixer_Source* const_source(const Mel_Alloc* a, u32 frames, f32 value)
{
...
MEL_TEST(voice_end, resolves_when_source_exhausts)
{
    ...
    Mel_Mixer* eng = mel_mixer_create_offline(a, offline_opt());
    MEL_REQUIRE_NOT_NULL(eng);

    Mel_Mixer_Source* src = const_source(a, 8u, 1.0f);
    Mel_Mixer_Voice   v = mel_mixer_play_ex(eng, src, 1.0f, 0.0f, false);

    Mel_Future* fut = mel_mixer_voice_end_future(eng, v);
    ...
    mel_mixer_voice_end_future_free(eng, fut);
...
MEL_TEST(voice_end, resolves_on_explicit_stop)
{
    ...
    Mel_Mixer_Source* src = const_source(a, SR, 0.5f);
    ...
    f32 out[BLOCK * CH];
    mel_mixer_render(eng, out, BLOCK);
    MEL_EXPECT(!mel_future_resolved(fut));
...
MEL_TEST(device_events, engine_owns_a_fireable_event)
{
    ...
    Mel_Event* ev = mel_mixer_device_events(eng);
