...
static Mel_Mixer_Source* const_source(const Mel_Alloc* a, f32 value)
{
    f32* buf = mel_alloc(a, sizeof(f32) * SR);
    for (u32 i = 0; i < SR; i++)
        buf[i] = value;
    Mel_Mixer_Source* s = mel_mixer_pcm_from_float(a, buf, SR, 1u, SR, MEL_MIXER_OWNERSHIP_OWNED);
    mel_dealloc(a, buf);
    mel_mixer_pcm_set_loop(s, true, 0.0);
    return s;
}
...
{
    Event_Tally* t = user;
    t->last = *(const Mel_Mixer_Device_Event*)payload;
    t->count++;
}
...
MEL_TEST(device, create_without_audioout_init_fails_loudly)
{
    ...
    Mel_Mixer*       eng = mel_mixer_create(a, opt());
    MEL_EXPECT(eng == NULL);
}
...
MEL_TEST(device, create_pinned_plays_through_the_provider)
{
    ...
    Mel_Mixer* eng = mel_mixer_create(a, o);
    MEL_REQUIRE_NOT_NULL(eng);
    MEL_EXPECT(mock.a.opened);
    MEL_EXPECT(mel_audioout_equal(mel_mixer_device(eng), mel_audioout_find(S8("mock:a"))));
    MEL_EXPECT_EQ(mel_mixer_device_status(eng) & MEL_MIXER_SEVERITY_MASK, (u32)MEL_MIXER_OK);

    Mel_Mixer_Caps caps = mel_mixer_caps(eng);
    MEL_EXPECT_EQ(caps.latency_frames, 100u + 4u * BLOCK);

    Mel_Mixer_Source* src = const_source(a, 0.5f);
...
MEL_TEST(device, set_device_voices_survive)
{
    ...
    Mel_Mixer_Status st = mel_mixer_set_device(eng, mel_audioout_find(S8("mock:b")));
    MEL_EXPECT(!mel_mixer_failed(st));
...
MEL_TEST(device, set_device_failure_keeps_previous_binding)
{
    ...
    Mel_Mixer_Status st = mel_mixer_set_device(eng, MEL_AUDIOOUT_NULL);
    MEL_EXPECT(!mel_mixer_failed(st));
    st = mel_mixer_set_device(eng, mel_audioout_find(S8("mock:a")));
    MEL_EXPECT(!mel_mixer_failed(st));
    ...
    st = mel_mixer_set_device(eng, dead);
    MEL_EXPECT(mel_mixer_failed(st));
    MEL_EXPECT(st & MEL_MIXER_RESULT_NO_DEVICE);
    MEL_EXPECT(mock.a.opened);
    MEL_EXPECT(mel_audioout_equal(mel_mixer_device(eng), mel_audioout_find(S8("mock:a"))));
...
MEL_TEST(device, pinned_loss_holds_until_rebind)
{
    ...
    Event_Tally tally = { 0 };
    mel_event_subscribe_push(mel_mixer_device_events(eng), mel_executor_inline(), on_device_event, &tally);
    ...
    Mel_Mixer_Status st = mel_mixer_device_status(eng);
    MEL_EXPECT(mel_mixer_failed(st));
    MEL_EXPECT(st & MEL_MIXER_RESULT_DEVICE_LOST);

    st = mel_mixer_set_device(eng, mel_audioout_find(S8("mock:b")));
    MEL_EXPECT(!mel_mixer_failed(st));
    MEL_EXPECT_EQ(mel_mixer_device_status(eng) & MEL_MIXER_SEVERITY_MASK, (u32)MEL_MIXER_OK);
    MEL_EXPECT(mock.b.opened);
...
MEL_TEST(device, follow_mode_migrates_on_default_change)
{
    ...
    MEL_EXPECT(!mock.a.opened);
    MEL_EXPECT(mock.b.opened);
    MEL_EXPECT(mel_audioout_equal(mel_mixer_device(eng), mel_audioout_find(S8("mock:b"))));
...
MEL_TEST(device, interruption_holds_and_resumes)
{
    ...
    Mel_Mixer_Status st = mel_mixer_device_status(eng);
    MEL_EXPECT(mel_mixer_failed(st));
    MEL_EXPECT(st & MEL_MIXER_RESULT_INTERRUPTED);
