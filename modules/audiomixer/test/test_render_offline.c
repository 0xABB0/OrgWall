...
static Mel_Mixer_Source* mono_ramp(const Mel_Alloc* a, u32 frames)
{
...
MEL_TEST(render, unity_gain_single_voice)
{
    ...
    f32 out[64u * CH];
    u32 produced = mel_mixer_render(eng, out, 64u);
...
MEL_TEST(render, volume_scaling)
{
    ...
    Mel_Mixer_Source* src = mono_const(a, 32u, 1.0f);
    mel_mixer_play_ex(eng, src, 0.25f, 0.0f, false);

    f32 out[32u * CH];
    mel_mixer_render(eng, out, 32u);
...
MEL_TEST(render, constant_power_pan)
{
    ...
    Mel_Mixer_Source* hard_left = mono_const(a, 16u, 1.0f);
    mel_mixer_play_ex(eng, hard_left, 1.0f, -1.0f, false);

    f32 out[16u * CH];
    mel_mixer_render(eng, out, 16u);
    ...
    eng = mel_mixer_create_offline(a, base_opt());
    Mel_Mixer_Source* hard_right = mono_const(a, 16u, 1.0f);
    mel_mixer_play_ex(eng, hard_right, 1.0f, 1.0f, false);
    mel_mixer_render(eng, out, 16u);
    for (u32 i = 0; i < 16u; i++)
    {
...
MEL_TEST(render, two_voice_mix)
{
    ...
    Mel_Mixer_Source* a_src = mono_const(a, 16u, 0.5f);
    Mel_Mixer_Source* b_src = mono_const(a, 16u, 0.25f);
    mel_mixer_play_ex(eng, a_src, 1.0f, -1.0f, false);
    mel_mixer_play_ex(eng, b_src, 1.0f, 1.0f, false);
...
MEL_TEST(render, loop_wrap)
{
    ...
    Mel_Mixer_Source* src = mel_mixer_pcm_from_float(a, pattern, 4u, 1u, SR, MEL_MIXER_OWNERSHIP_OWNED);
    mel_mixer_pcm_set_loop(src, true, 0.0);
    mel_mixer_play_ex(eng, src, 1.0f, -1.0f, false);

    f32 out[10u * CH];
    mel_mixer_render(eng, out, 10u);
...
MEL_TEST(render, voice_ends_when_source_exhausted)
{
    ...
    Mel_Mixer_Source* src = mono_const(a, 8u, 1.0f);
    mel_mixer_play_ex(eng, src, 1.0f, -1.0f, false);

    f32 out[20u * CH];
    mel_mixer_render(eng, out, 20u);
...
MEL_TEST(render, resample_double_speed)
{
    ...
    Mel_Mixer_Source* src = mono_ramp(a, 64u);
    Mel_Mixer_Voice   v = mel_mixer_play_ex(eng, src, 1.0f, -1.0f, false);
    mel_mixer_set_play_speed(eng, v, 2.0);
