...
static void mel_mixer__fade_arm(Mel_Mixer__Scalar_Fade* fade, f64 from, f64 to, f64 seconds, f64 clock, u32 samplerate)
{
    fade->active = 1u;
    fade->kind = MEL_MIXER__FADER_LINEAR;
    fade->from = from;
    fade->to = to;
...
void mel_mixer__cmd_create(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    mel_mixer__pan_gains(payload.pan, &payload.gain_l, &payload.gain_r);

    if (!mel_mixer__voice_activate(eng, cmd->handle, &payload))
    {
        mel_mixer__voice_remove_reserved(eng, cmd->handle, cmd->source, cmd->instance, cmd->tail);
        mel_mixer__end_future_resolve(eng, cmd->handle, MEL_FUTURE_BROKEN);
    }
}

void mel_mixer__cmd_set_volume(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    Mel_Mixer__Voice* v = mel_mixer__voice_get(&eng->voices, cmd->handle);
    if (v == NULL)
    {
...
void mel_mixer__cmd_set_pan(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    v->pan = cmd->f0;
    mel_mixer__pan_gains(v->pan, &v->gain_l, &v->gain_r);
...
void mel_mixer__cmd_set_paused(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    if (cmd->u0)
        v->flags |= MEL_MIXER_VOICE_PAUSED;
    else
        v->flags &= ~(u32)MEL_MIXER_VOICE_PAUSED;
}
...
void mel_mixer__cmd_set_loop(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    if (cmd->u0)
        v->flags |= MEL_MIXER_VOICE_LOOPING;
    else
        v->flags &= ~(u32)MEL_MIXER_VOICE_LOOPING;
}
...
{
    if (!mel_mixer__voice_alive(&eng->voices, cmd->handle))
    {
        mel_log_debug("audio", "stop on stale voice");
        return;
    }
    mel_mixer__voice_remove(eng, cmd->handle);
}
...
void mel_mixer__cmd_stop_all(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    while (t->packed.count > 0u)
        mel_mixer__voice_remove(eng, mel_array_last(&t->packed).self);
}
...
void mel_mixer__cmd_fade_volume(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    mel_mixer__fade_arm(&v->fade_volume, (f64)v->volume, (f64)cmd->f0, cmd->d0, eng->stream_clock, eng->caps.samplerate);
}
...
void mel_mixer__cmd_fade_pan(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    mel_mixer__fade_arm(&v->fade_pan, (f64)v->pan, (f64)cmd->f0, cmd->d0, eng->stream_clock, eng->caps.samplerate);
}
...
void mel_mixer__cmd_fade_speed(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    mel_mixer__fade_arm(&v->fade_speed, v->play_speed, cmd->d0, cmd->d1, eng->stream_clock, eng->caps.samplerate);
}
...
void mel_mixer__cmd_oscillate_volume(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    v->fade_volume.kind = MEL_MIXER__FADER_OSC;
...
void mel_mixer__cmd_schedule_pause(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    ...
    mel_mixer__fade_arm(&v->fade_volume, (f64)v->volume, 0.0, cmd->d0, eng->stream_clock, eng->caps.samplerate);
...
{
    mel_mixer__fade_arm(&eng->master_fade, (f64)eng->master_volume, (f64)cmd->f0, cmd->d0, eng->stream_clock, eng->caps.samplerate);
}
...
void mel_mixer__end_future_register(Mel_Mixer* eng, Mel_SlotMap_Handle handle, Mel_Future* fut)
{
    assert(eng != NULL);
    assert(fut != NULL);
    Mel_Mixer__End_Future entry = { .handle = handle, .fut = fut, .resolved = 0u };
...
void mel_mixer__end_future_resolve(Mel_Mixer* eng, Mel_SlotMap_Handle handle, Mel_Future_Status status)
{
    ...
    {
        Mel_Mixer__End_Future* e = &eng->end_futures.items[i];
...
{
    mel_mixer__end_future_register(eng, cmd->handle, cmd->fut);
    if (!mel_mixer__voice_alive(&eng->voices, cmd->handle))
        mel_mixer__end_future_resolve(eng, cmd->handle, MEL_FUTURE_OK);
}

void mel_mixer__cmd_release_end_future(Mel_Mixer* eng, const Mel_Mixer__Command* cmd)
{
    mel_mixer__end_future_release(eng, cmd->fut);
}
...
void mel_mixer__commands_drain(Mel_Mixer* eng)
{
    ...
    mel_spinlock_lock(&eng->command_lock);
    Mel_Mixer__Command_Queue front = eng->commands;
