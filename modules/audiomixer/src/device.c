...
static u32 mel_mixer__device_pull(void* user, f32* dst, u32 frames)
{
    Mel_Mixer* eng = user;
    u32        channels = eng->caps.channels;

    u32 avail_frames = mel_mixer_ring_read_available(eng->ring) / channels;
    u32 take = frames < avail_frames ? frames : avail_frames;
    u32 got = take > 0u ? mel_mixer_ring_read(eng->ring, dst, take * channels) / channels : 0u;
    ...
    if (got < frames)
    {
        ...
        atomic_fetch_or_explicit(&eng->device_bits, MEL_MIXER_WARN_RING_UNDERRUN, memory_order_release);
    }
    else
        atomic_fetch_and_explicit(&eng->device_bits, ~MEL_MIXER_WARN_RING_UNDERRUN, memory_order_release);
...
bool mel_mixer__device_open(Mel_Mixer* eng, Mel_AudioOut target, Mel_Mixer_Status* status)
{
    if (!mel_audioout_alive(target))
    {
        ...
        *status = MEL_MIXER_ERROR | MEL_MIXER_RESULT_NO_DEVICE;
        return false;
    }
    ...
    Mel_AudioPlayback_Open_Result r = mel_audioplayback_open(eng->alloc,
                                                             ...
                                                             (Mel_AudioPlayback_Opt){
                                                                 ...
                                                                 .pull = mel_mixer__device_pull,
                                                                 .user = eng,
                                                             });
    if (mel_audioplayback_status_failed(r.status))
    {
        u32 bit = MEL_MIXER_RESULT_FORMAT_UNGRANTED;
        if (r.status & MEL_AUDIOPLAYBACK_RESULT_NO_DEVICE)
            bit = MEL_MIXER_RESULT_NO_DEVICE;
        else if (r.status & MEL_AUDIOPLAYBACK_RESULT_BUSY)
            bit = MEL_MIXER_RESULT_DEVICE_DENIED;
        mel_log_error("audio", "device open failed (audioplayback status 0x%x)", r.status);
        *status = MEL_MIXER_ERROR | bit;
        return false;
    }

    eng->playback = r.playback;
    eng->bound = target;
    atomic_fetch_and_explicit(&eng->device_bits, ~(u32)(MEL_MIXER_RESULT_DEVICE_LOST | MEL_MIXER_RESULT_INTERRUPTED), memory_order_release);
...
static void mel_mixer__device_hold(Mel_Mixer* eng, u32 bit)
{
    mel_mixer__device_close(eng);
    atomic_fetch_or_explicit(&eng->device_bits, bit, memory_order_release);
}

static void mel_mixer__on_hotplug(const Mel_AudioOut_Event* ev, void* user)
{
    Mel_Mixer* eng = user;
    ...
    if (ev->removed && mel_audioout_equal(ev->device, eng->bound) && eng->playback != NULL && !eng->follow)
    {
        ...
        mel_mixer__device_hold(eng, MEL_MIXER_RESULT_DEVICE_LOST);
        mel_mixer__device_event_fire(eng, (Mel_Mixer_Device_Event){ .device = eng->bound, .lost = true });
        return;
    }
    ...
    if (ev->default_changed && eng->follow)
    {
        ...
        mel_mixer__device_close(eng);
        Mel_Mixer_Status st;
        if (!mel_mixer__device_open(eng, next, &st))
        {
            atomic_fetch_or_explicit(&eng->device_bits, MEL_MIXER_RESULT_DEVICE_LOST, memory_order_release);
            mel_mixer__device_event_fire(eng, (Mel_Mixer_Device_Event){ .device = next, .lost = true });
            return;
        }

        bool renegotiated = eng->native_rate != prev_rate || eng->native_channels != prev_channels;
        mel_mixer__device_event_fire(eng,
...
static void mel_mixer__on_policy(const Mel_AudioPolicy_Event* ev, void* user)
{
    Mel_Mixer* eng = user;
    ...
    if (ev->interruption_began && eng->playback != NULL)
    {
        ...
        eng->interrupted = 1u;
        mel_mixer__device_hold(eng, MEL_MIXER_RESULT_INTERRUPTED);
        mel_mixer__device_event_fire(eng, (Mel_Mixer_Device_Event){ .device = eng->bound, .interrupted = true });
        return;
    }
    ...
    if (ev->interruption_ended && ev->should_resume && eng->interrupted)
    {
        ...
        mel_mixer__device_event_fire(eng, (Mel_Mixer_Device_Event){ .device = eng->bound, .resumed = true });
    }
}

void mel_mixer__device_subscribe(Mel_Mixer* eng)
{
    eng->out_sub = mel_audioout_subscribe(eng->exec, mel_mixer__on_hotplug, eng);
    if (mel_audiopolicy_active())
    {
        eng->policy_sub = mel_audiopolicy_subscribe(eng->exec, mel_mixer__on_policy, eng);
        eng->policy_bound = 1u;
    }
}
...
Mel_Mixer_Status mel_mixer_set_device(Mel_Mixer* eng, Mel_AudioOut device)
{
    ...
    if (eng->online == 0u)
    {
        ...
        return MEL_MIXER_ERROR | MEL_MIXER_RESULT_NO_DEVICE;
    }

    if (!mel_mixer__api_enter(eng))
        return MEL_MIXER_ERROR | MEL_MIXER_RESULT_DESTROYING;
...
Mel_Mixer_Status mel_mixer_device_status(const Mel_Mixer* eng)
{
    ...
    if (bits & (MEL_MIXER_RESULT_DEVICE_LOST | MEL_MIXER_RESULT_INTERRUPTED))
        return MEL_MIXER_ERROR | bits;
    if (bits != 0)
        return MEL_MIXER_WARNED | bits;
    return MEL_MIXER_OK;
}
