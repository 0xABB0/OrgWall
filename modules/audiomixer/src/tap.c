...
static void mel_mixer__tap_destroy(Mel_Mixer_Tap* t)
{
    if (t->ring != NULL)
...
{
    Mel_Mixer_Tap* t = cmd->instance;
    mel_array_push(&eng->taps, t);
    if (!mel_mixer__tap_scratch_ensure(eng, eng->scratch_frames))
...
static void mel_mixer__tap_ring_write(Mel_Mixer_Tap* t, const f32* interleaved, u32 frames)
{
...
static void mel_mixer__tap_interleave(Mel_Mixer* eng, const f32* planar, u32 frames)
{
    u32 channels = eng->caps.channels;
...
void mel_mixer__taps_master_write(Mel_Mixer* eng, const f32* planar_out, u32 frames)
{
    ...
    mel_mixer__tap_interleave(eng, planar_out, frames);
    for (usize i = 0; i < eng->taps.count; i++)
        if (!eng->taps.items[i]->is_voice)
            mel_mixer__tap_ring_write(eng->taps.items[i], eng->scratch_tap_inter, frames);
}
...
void mel_mixer__taps_voice_write(Mel_Mixer* eng, Mel_SlotMap_Handle voice, u32 src_channels, u32 frames, f32 gain_l, f32 gain_r)
{
    ...
    for (usize i = 0; i < eng->taps.count; i++)
    {
        Mel_Mixer_Tap* t = eng->taps.items[i];
    ...
    mel_mixer__pan_accumulate(eng->scratch_resampled, src_channels, frames, gain_l, gain_r, eng->scratch_tap_planar, out_channels);
    mel_mixer__tap_interleave(eng, eng->scratch_tap_planar, frames);

    for (usize i = 0; i < eng->taps.count; i++)
    {
        Mel_Mixer_Tap* t = eng->taps.items[i];
        if (t->is_voice && t->voice.index == voice.index && t->voice.generation == voice.generation)
            mel_mixer__tap_ring_write(t, eng->scratch_tap_inter, frames);
    }
}
...
void mel_mixer__taps_free_all(Mel_Mixer* eng)
{
    ...
    for (usize i = 0; i < eng->taps.count; i++)
        mel_mixer__tap_destroy(eng->taps.items[i]);
    eng->taps.count = 0;
}

static Mel_Mixer_Tap* mel_mixer__tap_open_impl(Mel_Mixer* eng, Mel_SlotMap_Handle voice, u32 is_voice, const Mel_Alloc* a, u32 ring_frames)
{
    assert(eng != NULL);
    assert(a != NULL);
    ...
    Mel_Mixer_Tap* t = mel_alloc_type(a, Mel_Mixer_Tap);
    if (t == NULL)
    {
    ...
    Mel_Mixer__Command cmd = { .apply = mel_mixer__cmd_tap_attach, .instance = t };
    if (eng->online == 0u)
        mel_mixer__cmd_tap_attach(eng, &cmd);
    else
...
Mel_Mixer_Tap* mel_mixer_tap_open(Mel_Mixer* eng, const Mel_Alloc* a, u32 ring_frames) { return mel_mixer__tap_open_impl(eng, MEL_SLOTMAP_HANDLE_NULL, 0u, a, ring_frames); }

Mel_Mixer_Tap* mel_mixer_voice_tap_open(Mel_Mixer* eng, Mel_Mixer_Voice v, const Mel_Alloc* a, u32 ring_frames)
{
    assert(eng != NULL);
    if (!mel_mixer__voice_alive(&eng->voices, v.slot))
    {
        mel_log_error("audio", "voice tap open on a stale voice handle");
        return NULL;
    }
    return mel_mixer__tap_open_impl(eng, v.slot, 1u, a, ring_frames);
}

u32 mel_mixer_tap_read(Mel_Mixer_Tap* t, f32* interleaved_dst, u32 max_frames)
{
    assert(t != NULL);
...
u32 mel_mixer_tap_available(const Mel_Mixer_Tap* t)
{
    assert(t != NULL);
...
void mel_mixer_tap_close(Mel_Mixer_Tap* t)
{
    ...
    Mel_Mixer__Command cmd = { .apply = mel_mixer__cmd_tap_detach, .instance = t };
    if (eng->online == 0u)
        mel_mixer__cmd_tap_detach(eng, &cmd);
    else
        mel_mixer__command_push(eng, &cmd);
