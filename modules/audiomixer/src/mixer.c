...
static void mel_mixer__voice_apply_fades(Mel_Mixer* eng, Mel_Mixer__Voice* v)
{
    ...
    {
        u32 done = 0;
        f32 val = mel_mixer__fade_eval(&v->fade_volume, clock, &done);
        v->volume = val;
        if (done)
        {
            ...
            if (v->fade_volume.on_complete_stop)
                v->flags |= MEL_MIXER__VOICE_ENDED;
    ...
    {
        u32 done = 0;
        v->pan = mel_mixer__fade_eval(&v->fade_pan, clock, &done);
    ...
    {
        u32 done = 0;
        v->play_speed = (f64)mel_mixer__fade_eval(&v->fade_speed, clock, &done);
        if (done)
...
u32 mel_mixer__mix_block(Mel_Mixer* eng, f32* planar_out, u32 frames)
{
    ...
    {
        eng->master_volume = mel_mixer__fade_eval(&eng->master_fade, eng->stream_clock, &master_done);
        if (master_done)
    ...
    {
        Mel_Mixer__Voice* v = &eng->voices.packed.items[vi];

        if (v->source == NULL)
            continue;

        mel_mixer__voice_apply_fades(eng, v);

        if (v->flags & MEL_MIXER__VOICE_ENDED)
        {
            ended_count++;
            continue;
        }

        if (v->flags & MEL_MIXER_VOICE_PAUSED)
            continue;
        ...
        u32 ended = 0;
        if (got_fresh < fresh && !(v->flags & MEL_MIXER_VOICE_LOOPING))
        {
            if (wb_got == 0u)
            {
                valid_out = 0u;
        ...
        mel_mixer__pan_accumulate(eng->scratch_resampled, src_channels, frames, v->gain_l, v->gain_r, planar_out, out_channels);
        mel_mixer__taps_voice_write(eng, v->self, src_channels, frames, v->gain_l, v->gain_r);
    ...
    for (;;)
    {
        ...
        {
            if (eng->voices.packed.items[i].flags & MEL_MIXER__VOICE_ENDED)
            {
        ...
        if (!mel_slotmap_handle_valid(victim))
            break;
        mel_mixer__voice_remove(eng, victim);
    }
    ...
    mel_mixer__taps_master_write(eng, planar_out, frames);
