...
str8 str8_fmt_arena(Mel_Arena* arena, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    va_list args_copy;
    ...
    u8* buf = (u8*)mel_arena_push(arena, (usize)(needed + 1));
...
str8 str8_fmt_alloc(const Mel_Alloc* alloc, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    va_list args_copy;
