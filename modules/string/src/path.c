...
str8 mel_path_join(str8 base, str8 relative, u8* buf, usize buf_cap)
{
    ...
    if (relative.len == 0)
        return mel_path_normalize(base, buf, buf_cap);
...
str8 mel_path_parent(str8 path)
{
    ...
    size end = path.len;
    if (path.data[end - 1] == '/' || path.data[end - 1] == '\\')
        end--;
    ...
    if (end <= 1)
        return (str8){ .data = path.data, .len = 1 };
...
bool mel_path_is_absolute(str8 path) { return path.len > 0 && (path.data[0] == '/' || path.data[0] == '\\'); }
