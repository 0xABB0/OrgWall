...
static Mel_SlotMap_Handle to_sm(Mel_Window w)
{
    return (Mel_SlotMap_Handle){ .index = w.index, .generation = w.generation };
}

static Mel_Window from_sm(Mel_SlotMap_Handle h)
{
    return (Mel_Window){ .index = h.index, .generation = h.generation };
}
...
void mel_window_init(Mel_Reactor* reactor)
{
    ...
    g_reactor = reactor;
    mel_slotmap_init(&g_windows, g_alloc,
        .item_size = sizeof(Mel_Window_Node), .initial_capacity = 8);
...
void mel_window_shutdown(void)
{
    ...
    for (u32 i = 0; i < count; i++) {
        if (data[i].native) mel_window__backend_destroy(&data[i]);
    }
...
void mel_window__resized(Mel_Window w, i32 pixel_w, i32 pixel_h)
{
    ...
    n->w = pixel_w;
    n->h = pixel_h;
    if (n->lifecycle.on_resize) n->lifecycle.on_resize(w, pixel_w, pixel_h, n->user);
}
...
void mel_window__closed(Mel_Window w)
{
    ...
    if (n->lifecycle.on_closed) n->lifecycle.on_closed(w, n->user);
...
{
    Mel_Window_Node* n = mel_window__node(w);
    if (out_w) *out_w = n ? n->w : 0;
    if (out_h) *out_h = n ? n->h : 0;
}

void mel_window_point_extent(Mel_Window w, i32* out_w, i32* out_h)
{
    Mel_Window_Node* n = mel_window__node(w);
    if (out_w) *out_w = n ? n->point_w : 0;
    if (out_h) *out_h = n ? n->point_h : 0;
}
