...
- (NSPoint)pt:(NSEvent*)e { return [self convertPoint:e.locationInWindow fromView:nil]; }

- (void)mouseDown:(NSEvent*)e
{
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (!n || !n->input.on_pointer_down) return;
    NSPoint p = [self pt:e];
...
{
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (!n || !n->input.on_pointer_up) return;
    NSPoint p = [self pt:e];
...
{
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (!n || !n->input.on_pointer_move) return;
    NSPoint p = [self pt:e];
...
{
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->input.on_key_down) n->input.on_key_down(self.window_handle, (u32)e.keyCode, n->user);
}

- (void)keyUp:(NSEvent*)e
{
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->input.on_key_up) n->input.on_key_up(self.window_handle, (u32)e.keyCode, n->user);
}
...
- (void)onMove:(NSNotification*)note
{
    ...
    n->x = x;
    n->y = y;
    if (n->lifecycle.on_move) n->lifecycle.on_move(self.window_handle, x, y, n->user);
}
...
- (void)onBacking:(NSNotification*)note
{
    ...
    if (!n) return;
    f32 scale = (f32)window.backingScaleFactor;
    if (n->display.on_scale_changed) n->display.on_scale_changed(self.window_handle, scale, n->user);
...
- (void)onBecomeKey:(NSNotification*)note
{
    (void)note;
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->lifecycle.on_focus_in) n->lifecycle.on_focus_in(self.window_handle, n->user);
}
...
- (void)onResignKey:(NSNotification*)note
{
    (void)note;
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->lifecycle.on_focus_out) n->lifecycle.on_focus_out(self.window_handle, n->user);
}
...
- (void)onWillClose:(NSNotification*)note
{
    ...
    if (mel_window_is_none(w)) return;
...
- (void)onAppActive:(NSNotification*)note
{
    (void)note;
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->app.on_foreground) n->app.on_foreground(self.window_handle, n->user);
}
...
- (void)onAppResign:(NSNotification*)note
{
    (void)note;
    Mel_Window_Node* n = mel_window__node(self.window_handle);
    if (n && n->app.on_background) n->app.on_background(self.window_handle, n->user);
}
...
void mel_window__backend_create(Mel_Window_Node* n, const Mel_Window_Opt* o)
{
    @autoreleasepool {
        ...
        if (!o->not_closable)  style |= NSWindowStyleMaskClosable;
        ...
        if (o->min_w > 0 || o->min_h > 0) [window setContentMinSize:NSMakeSize(o->min_w, o->min_h)];
