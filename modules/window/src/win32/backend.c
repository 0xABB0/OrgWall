...
static u64 mel_window__pack(Mel_Window w)
{
    return ((u64)w.generation << 32) | (u64)w.index;
}

static Mel_Window mel_window__unpack(u64 v)
{
    return (Mel_Window){ .index = (u32)v, .generation = (u32)(v >> 32) };
}

static Mel_Window mel_window__from_hwnd(HWND hwnd)
{
    return mel_window__unpack((u64)GetWindowLongPtrW(hwnd, GWLP_USERDATA));
}
...
static LRESULT CALLBACK mel_window__wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    Mel_Window_Node* n = mel_window__node(w);
    if (!n) return DefWindowProcW(hwnd, msg, wp, lp);
    ...
    case WM_DPICHANGED: {
        ...
        f32  s   = (f32)dpi / 96.0f;
        n->scale = s;
        if (n->display.on_scale_changed) n->display.on_scale_changed(w, s, n->user);
        if (n->display.on_hdr_changed)   n->display.on_hdr_changed(w, n->user);
        RECT* sug = (RECT*)lp;
    ...
    case WM_MOVE: {
        ...
        n->x = x;
        n->y = y;
        if (n->lifecycle.on_move) n->lifecycle.on_move(w, x, y, n->user);
        return 0;
    }

    case WM_SETFOCUS:
        if (n->lifecycle.on_focus_in) n->lifecycle.on_focus_in(w, n->user);
        return 0;

    case WM_KILLFOCUS:
        if (n->lifecycle.on_focus_out) n->lifecycle.on_focus_out(w, n->user);
        return 0;

    case WM_ACTIVATEAPP:
        if (wp) { if (n->app.on_foreground) n->app.on_foreground(w, n->user); }
        else    { if (n->app.on_background) n->app.on_background(w, n->user); }
        return 0;
    ...
    case WM_KEYDOWN:
        if (n->input.on_key_down) n->input.on_key_down(w, (u32)wp, n->user);
        return 0;

    case WM_KEYUP:
        if (n->input.on_key_up) n->input.on_key_up(w, (u32)wp, n->user);
        return 0;

    case WM_CLOSE:
        if (!mel_window_should_close(w)) return 0;
        DestroyWindow(hwnd);
        return 0;
...
void mel_window__backend_create(Mel_Window_Node* n, const Mel_Window_Opt* o)
{
    ...
    HWND hwnd = CreateWindowExW(0, g_class_name, title, style,
                                CW_USEDEFAULT, CW_USEDEFAULT,
                                rc.right - rc.left, rc.bottom - rc.top,
                                NULL, NULL, g_hinst, (LPVOID)(uintptr_t)packed);
    ...
    if (!o->start_hidden) ShowWindow(hwnd, SW_SHOW);
}
