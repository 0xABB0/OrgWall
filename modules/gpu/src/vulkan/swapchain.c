...
static void destroy_swapchain_objects(Mel_Gpu_Swapchain* sc)
{
    ...
    for (u32 i = 0; i < sc->image_count; i++)
    {
        if (sc->framebuffers[i])
            vkDestroyFramebuffer(d, sc->framebuffers[i], NULL);
        if (sc->views[i])
            vkDestroyImageView(d, sc->views[i], NULL);
    }
    sc->image_count = 0;
    if (sc->swapchain)
    {
...
static bool create_swapchain_objects(Mel_Gpu_Swapchain* sc)
{
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(dev->phys, sc->surf->surface, &caps);
    ...
    if (image_count > MEL_GPU_VK_MAX_IMAGES)
        image_count = MEL_GPU_VK_MAX_IMAGES;

    VkSwapchainCreateInfoKHR ci = {
        .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
        .surface = sc->surf->surface,
    ...
    if (vkCreateSwapchainKHR(d, &ci, NULL, &sc->swapchain) != VK_SUCCESS)
        return false;

    vkGetSwapchainImagesKHR(d, sc->swapchain, &sc->image_count, NULL);
    if (sc->image_count > MEL_GPU_VK_MAX_IMAGES)
        sc->image_count = MEL_GPU_VK_MAX_IMAGES;
    ...
    for (u32 i = 0; i < sc->image_count; i++)
    {
        ...
        if (vkCreateImageView(d, &vci, NULL, &sc->views[i]) != VK_SUCCESS)
            return false;
        ...
        if (vkCreateFramebuffer(d, &fci, NULL, &sc->framebuffers[i]) != VK_SUCCESS)
            return false;
    }
    return true;
}
...
Mel_Gpu_Swapchain* mel_gpu_swapchain_create_opt(Mel_Gpu_Device* dev, Mel_Gpu_Swapchain_Opt opt)
{
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(dev->phys, dev->queue_family, sc->surf->surface, &present_ok);
    ...
    {
        u32 fc = 0;
        vkGetPhysicalDeviceSurfaceFormatsKHR(dev->phys, sc->surf->surface, &fc, NULL);
        VkSurfaceFormatKHR fmts[32];
        if (fc > 32)
            fc = 32;
        if (fc > 0)
        {
            vkGetPhysicalDeviceSurfaceFormatsKHR(dev->phys, sc->surf->surface, &fc, fmts);
            int chosen = -1;
            for (u32 i = 0; i < fc; i++)
                if (fmts[i].format == sc->format)
                {
                    chosen = (int)i;
                    break;
                }
    ...
    VkFenceCreateInfo     fen = { .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, .flags = VK_FENCE_CREATE_SIGNALED_BIT };
    ...
    if (!create_swapchain_objects(sc))
    {
        mel_gpu_swapchain_destroy(sc);
        return NULL;
    }
...
void mel_gpu_swapchain_destroy(Mel_Gpu_Swapchain* sc)
{
    ...
    if (sc->in_flight)
        vkDestroyFence(d, sc->in_flight, NULL);
    if (sc->image_available)
        vkDestroySemaphore(d, sc->image_available, NULL);
    if (sc->render_finished)
        vkDestroySemaphore(d, sc->render_finished, NULL);
    if (sc->cmd_buffer)
        vkFreeCommandBuffers(d, sc->device->cmd_pool, 1, &sc->cmd_buffer);
    if (sc->render_pass)
        vkDestroyRenderPass(d, sc->render_pass, NULL);
    free(sc);
}
...
void mel_gpu_swapchain_resize(Mel_Gpu_Swapchain* sc, i32 width, i32 height)
{
    ...
    mel_gpu_surface_reconfigure(sc->surf, width, height);
