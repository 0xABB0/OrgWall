...
static VkBuffer create_raw(Mel_Gpu_Device* dev, usize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkDeviceMemory* out_mem)
{
    ...
    VkBuffer buf;
    if (vkCreateBuffer(dev->device, &bci, NULL, &buf) != VK_SUCCESS)
...
static void upload_via_staging(Mel_Gpu_Device* dev, VkBuffer dst, const void* data, usize size)
{
    ...
    VkBuffer       staging = create_raw(dev, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &staging_mem);
    if (!staging)
        return;
...
Mel_Gpu_Buffer* mel_gpu_buffer_create_opt(Mel_Gpu_Device* dev, Mel_Gpu_Buffer_Opt opt)
{
    ...
    if (opt.usage & MEL_GPU_BUFFER_VERTEX)
        usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    if (opt.usage & MEL_GPU_BUFFER_INDEX)
    ...
    VkMemoryPropertyFlags props = host_visible ? (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) : VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
...
{
    if (!buf || !buf->host_visible || !data || size == 0)
        return;
    if (size > buf->size)
        size = buf->size;
    void* p;
    if (vkMapMemory(buf->device->device, buf->mem, 0, size, 0, &p) != VK_SUCCESS)
        return;
    memcpy(p, data, size);
