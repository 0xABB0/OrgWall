...
Mel_Gpu_Shader* mel_gpu_shader_create_opt(Mel_Gpu_Device* dev, Mel_Gpu_Shader_Opt opt)
{
    ...
    {
        if (err)
            NSLog(@"mel_gpu: metal shader compile failed: %@", err.localizedDescription);
        return NULL;
    }
    ...
    sh->library = lib;
    if (opt.vertex_entry)
        sh->vertex_fn = [lib newFunctionWithName:@(opt.vertex_entry)];
    if (opt.fragment_entry)
        sh->fragment_fn = [lib newFunctionWithName:@(opt.fragment_entry)];
