...
void mel_gpu_frame_begin(Mel_Gpu_Swapchain* sc)
{
    if (!sc)
        return;
    sc->drawable = [sc->surface->layer nextDrawable];
...
void mel_gpu_frame_end(Mel_Gpu_Swapchain* sc)
{
    if (!sc || !sc->cmd_buffer)
        return;
    if (sc->drawable)
        [sc->cmd_buffer presentDrawable:sc->drawable];
...
void mel_gpu_cmd_begin_pass(Mel_Gpu_Command_List* cmd, Mel_Gpu_Color clear)
{
    ...
    MTLViewport vp = { 0.0, 0.0, (double)sc->surface->layer.drawableSize.width, (double)sc->surface->layer.drawableSize.height, 0.0, 1.0 };
...
void mel_gpu_cmd_draw(Mel_Gpu_Command_List* cmd, u32 vertex_count, u32 instance_count)
{
    if (!cmd || !cmd->encoder)
        return;
    [cmd->encoder drawPrimitives:cmd->primitive vertexStart:0 vertexCount:vertex_count instanceCount:instance_count];
}
