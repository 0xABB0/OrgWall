...
Mel_Gpu_Buffer* mel_gpu_buffer_create_opt(Mel_Gpu_Device* dev, Mel_Gpu_Buffer_Opt opt)
{
    ...
    MTLResourceOptions res = buf->host_visible ? MTLResourceStorageModeShared : MTLResourceStorageModePrivate;

    if (buf->host_visible)
    {
        buf->mtl = opt.data ? [dev->mtl newBufferWithBytes:opt.data length:opt.size options:res] : [dev->mtl newBufferWithLength:opt.size options:res];
    }
    else
    {
