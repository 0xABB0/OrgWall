...
{
    (void)dev;
    /* WebGPU core has no true bindless heap (caps.memory.bindless tier=capped). */
    return false;
}
...
u32 mel_gpu_texture_view_bindless_slot(Mel_Gpu_Device* dev, Mel_Gpu_Texture_View view)
{
    (void)dev;
    (void)view;
    mel_log_error("gpu", "texture_view_bindless_slot: true bindless is not in WebGPU core (caps.memory.bindless tier=capped); MissingFeature");
    return 0;
}
...
u32 mel_gpu_buffer_bindless_slot(Mel_Gpu_Device* dev, Mel_Gpu_Buffer buf)
{
    (void)dev;
    (void)buf;
    mel_log_error("gpu", "buffer_bindless_slot: true bindless is not in WebGPU core (caps.memory.bindless tier=capped); MissingFeature");
    return 0;
}
...
u32 mel_gpu_sampler_bindless_slot(Mel_Gpu_Device* dev, Mel_Gpu_Sampler sampler)
{
    (void)dev;
    (void)sampler;
    mel_log_error("gpu", "sampler_bindless_slot: true bindless is not in WebGPU core (caps.memory.bindless tier=capped); MissingFeature");
    return 0;
}
...
u64 mel_gpu_buffer_device_address(Mel_Gpu_Device* dev, Mel_Gpu_Buffer buf)
{
    (void)dev;
    (void)buf;
    mel_log_error("gpu", "buffer_device_address: GPU virtual addresses are not exposed in WebGPU core; MissingFeature");
    return 0;
}
...
{
    (void)cmd;
    mel_log_warn("gpu", "cmd_bind_bindless: bindless is not implemented on the WebGPU backend; MissingFeature");
}
...
Mel_Gpu_Format_Properties mel_gpu_format_properties(Mel_Gpu_Device* dev, Mel_Gpu_Format format, Mel_Gpu_Tiling tiling)
{
    ...
    if (!dev || mel_gpu__wgpu_format(format) == WGPUTextureFormat_Undefined)
        return p;
    ...
    if (!mel_gpu_format_is_depth(format))
        f |= MEL_GPU_FMT_COLOR_ATTACHMENT | MEL_GPU_FMT_COLOR_BLEND | MEL_GPU_FMT_LINEAR_FILTER;
    else
...
Mel_Gpu_Sync_Create_Result mel_gpu_sync_create(Mel_Gpu_Device* dev, Mel_Gpu_Sync_Kind kind, u64 initial_value)
{
    ...
    (void)kind;
    (void)initial_value;
    mel_log_error("gpu", "sync_create: explicit sync primitives are not exposed in WebGPU core (use the per-submission completion future); MissingFeature");
...
Mel_Gpu_Query_Pool_Create_Result mel_gpu_query_pool_create_opt(Mel_Gpu_Device* dev, Mel_Gpu_Query_Pool_Opt opt)
{
    (void)dev;
    (void)opt;
    mel_log_error("gpu", "query_pool_create: timestamp queries are not implemented on the WebGPU backend this round; MissingFeature");
...
Mel_Gpu_Bind_Group_Layout mel_gpu_bind_group_layout_create(Mel_Gpu_Device* dev, const Mel_Gpu_Bind_Group_Layout_Entry* entries, u32 count)
{
    ...
    (void)entries;
    (void)count;
    mel_log_error("gpu", "bind_group_layout_create: explicit descriptor sets are not implemented on the WebGPU backend this round; MissingFeature");
...
Mel_Gpu_Bind_Group mel_gpu_bind_group_create(Mel_Gpu_Device* dev, Mel_Gpu_Bind_Group_Layout layout)
{
    (void)dev;
    (void)layout;
    mel_log_error("gpu", "bind_group_create: explicit descriptor sets are not implemented on the WebGPU backend this round; MissingFeature");
...
void mel_gpu_cmd_bind_descriptor_set(Mel_Gpu_Command_List* cmd, u32 set_index, Mel_Gpu_Bind_Group group)
{
    ...
    (void)set_index;
    (void)group;
    mel_log_warn("gpu", "cmd_bind_descriptor_set: descriptor sets are not implemented on the WebGPU backend this round; MissingFeature");
}
