...
void mel_gpu_frame_begin(Mel_Gpu_Swapchain* sc)
{
    ...
    wgpuSurfaceGetCurrentTexture(sc->surface->surface, &st);
    if (!st.texture)
    {
        sc->cur_texture = NULL;
        sc->cur_view = NULL;
...
void mel_gpu_frame_end(Mel_Gpu_Swapchain* sc)
{
    ...
    wgpuSurfacePresent(sc->surface->surface);
#endif
...
{
    if (!cmd || !cmd->encoder || !cmd->swapchain->cur_view)
        return;
