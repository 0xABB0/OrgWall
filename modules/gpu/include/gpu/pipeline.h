...
/* One field of a from-slang inlined bindless argument buffer (the Metal lane of dual-lane
   ...
   field lowers to a per-dispatch (or per-draw) argument buffer mixing resolved resources and
   inline uniforms. `is_uniform` distinguishes the two roles; `host_offset` is the field's byte
