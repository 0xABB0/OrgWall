...
void build(Mel_Build *b) {
    ...
    mel_exclude_source(lib, WHEN(.gpu = "vulkan", .platforms = MEL_ON(MACOS) | MEL_ON(LINUX)),
                       "src/vulkan/android_surface.c");
    mel_sources(lib, WHEN(.gpu = "vulkan", .platforms = MEL_ON(MACOS)), "src/vulkan/surface.m");
    ...
    mel_exclude_source(lib, WHEN(.gpu = "webgpu", .platforms = MEL_ON(MACOS) | MEL_ON(WASM)),
                       "src/webgpu/surface_android.c");
    mel_exclude_source(lib, WHEN(.gpu = "webgpu", .platforms = MEL_ON(MACOS) | MEL_ON(ANDROID)),
                       "src/webgpu/surface_web.c");
    ...
    mel_link(lib, MEL_PUBLIC, WHEN(.runtime = "emscripten"), "--use-port=emdawnwebgpu", "--pre-js",
             "modules/gpu/src/webgpu/device_preinit.js");
