...
static void mel__log_sink_test_write(Mel_Log_Sink* self, const Mel_Log_Entry* entry)
{
    ...
    usize total = (usize)entry->domain.len + (usize)entry->message.len + (usize)entry->file.len + (usize)entry->context.len;
...
static void mel__log_sink_test_flush(Mel_Log_Sink* self) { (void)self; }
...
bool mel_log_test_has_entry(Mel_Log_Sink* sink, u32 level, str8 domain)
{
    ...
    for (usize i = 0; i < test->entries.count; i++)
    {
        ...
        if (e->level == level && e->domain.len == domain.len && memcmp(e->domain.data, domain.data, (usize)domain.len) == 0)
        {
            return true;
        }
    }
