...
static u64 mel__log_timestamp(void) { return mel_nanos_since_unspecified_epoch(); }

static u32 mel__log_thread_id(void) { return (u32)mel_thread_current_id(); }
...
Mel_Log_Sink_Handle mel_log_sink_add(Mel_Log_Sink* sink)
{
    ...
    if (sink_count >= sink_capacity)
    {
        ...
        const Mel_Alloc*    heap = mel_alloc_heap();
        Mel__Log_Sink_Slot* new_slots = sinks ? mel_realloc(heap, sinks, sizeof(Mel__Log_Sink_Slot) * new_cap) : mel_alloc(heap, sizeof(Mel__Log_Sink_Slot) * new_cap);
        assert(new_slots);
        sinks = new_slots;
...
void mel_log_level_register(u32 value, str8 name)
{
    ...
    if (level_count >= level_capacity)
    {
        ...
        const Mel_Alloc*      heap = mel_alloc_heap();
        Mel__Log_Level_Entry* new_levels = levels ? mel_realloc(heap, levels, sizeof(Mel__Log_Level_Entry) * new_cap) : mel_alloc(heap, sizeof(Mel__Log_Level_Entry) * new_cap);
        assert(new_levels);
        levels = new_levels;
