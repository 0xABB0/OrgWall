...
static u64 mel__sqlite_sink_now_ns(void) { return (u64)mel_nanos_since_unspecified_epoch(); }
...
{
    if (!mel__sqlite_sink_exec(db, "PRAGMA journal_mode=WAL"))
        return false;
    ...
    if (!mel__sqlite_sink_exec(db, "CREATE INDEX IF NOT EXISTS idx_entries_level ON entries(level)"))
        return false;
    if (!mel__sqlite_sink_exec(db, "CREATE INDEX IF NOT EXISTS idx_entries_domain ON entries(domain)"))
        return false;
    if (!mel__sqlite_sink_exec(db, "CREATE INDEX IF NOT EXISTS idx_entries_timestamp ON entries(timestamp_ns)"))
        return false;
...
static bool mel__sqlite_sink_insert_metadata(sqlite3* db)
{
    ...
    int           rc = sqlite3_prepare_v2(db, "INSERT OR REPLACE INTO metadata(key, value) VALUES (?1, ?2)", -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
