...
static float mel__tf_linear(float c) {
    return c;
}

static float mel__eotf_gamma22(float c) {
    return c < 0.0f ? -powf(-c, 2.19921875f) : powf(c, 2.19921875f);
}

static float mel__oetf_gamma22(float c) {
    return c < 0.0f ? -powf(-c, 1.0f / 2.19921875f) : powf(c, 1.0f / 2.19921875f);
}
...
mel_white_point mel_white_d65(void) {
    return (mel_white_point){0.3127f, 0.3290f};
}

mel_white_point mel_white_d50(void) {
    return (mel_white_point){0.34567f, 0.35850f};
}

mel_white_point mel_white_aces(void) {
    return (mel_white_point){0.32168f, 0.33767f};
}

mel_white_point mel_white_e(void) {
    return (mel_white_point){1.0f / 3.0f, 1.0f / 3.0f};
}
...
mel_color_space mel_color_space_srgb(void) {
    return (mel_color_space){
        {0.640f, 0.330f}, {0.300f, 0.600f}, {0.150f, 0.060f}, mel_white_d65(),
        mel_color_srgb_to_linear, mel_color_linear_to_srgb,
    };
}
...
mel_color_space mel_color_space_linear_srgb(void) {
    return (mel_color_space){
        {0.640f, 0.330f}, {0.300f, 0.600f}, {0.150f, 0.060f}, mel_white_d65(),
        mel__tf_linear, mel__tf_linear,
    };
}

mel_color_space mel_color_space_display_p3(void) {
    return (mel_color_space){
        {0.680f, 0.320f}, {0.265f, 0.690f}, {0.150f, 0.060f}, mel_white_d65(),
        mel_color_srgb_to_linear, mel_color_linear_to_srgb,
    };
}
...
mel_color_space mel_color_space_rec2020(void) {
    return (mel_color_space){
        {0.708f, 0.292f}, {0.170f, 0.797f}, {0.131f, 0.046f}, mel_white_d65(),
        mel__eotf_rec2020, mel__oetf_rec2020,
    };
}
...
mel_color_space mel_color_space_rec2020_hlg(void) {
    return (mel_color_space){
        {0.708f, 0.292f}, {0.170f, 0.797f}, {0.131f, 0.046f}, mel_white_d65(),
        mel__eotf_hlg, mel__oetf_hlg,
    };
}

mel_color_space mel_color_space_adobe_rgb(void) {
    return (mel_color_space){
        {0.640f, 0.330f}, {0.210f, 0.710f}, {0.150f, 0.060f}, mel_white_d65(),
        mel__eotf_gamma22, mel__oetf_gamma22,
    };
}

mel_color_space mel_color_space_prophoto(void) {
    return (mel_color_space){
        {0.7347f, 0.2653f}, {0.1596f, 0.8404f}, {0.0366f, 0.0001f}, mel_white_d50(),
        mel__eotf_prophoto, mel__oetf_prophoto,
    };
}

mel_color_space mel_color_space_aces_cg(void) {
    return (mel_color_space){
        {0.713f, 0.293f}, {0.165f, 0.830f}, {0.128f, 0.044f}, mel_white_aces(),
        mel__tf_linear, mel__tf_linear,
    };
}
...
mel_color_space mel_color_space_aces2065_1(void) {
    return (mel_color_space){
        {0.7347f, 0.2653f}, {0.0f, 1.0f}, {0.0001f, -0.0770f}, mel_white_aces(),
        mel__tf_linear, mel__tf_linear,
    };
}
...
static Mel_Mat3 mel__space_to_xyz(const mel_color_space *s) {
    ...
    return mel__mat3_cols(mel__vec3_scale(r, scale.x), mel__vec3_scale(g, scale.y),
                          mel__vec3_scale(b, scale.z));
}

static Mel_Mat3 mel__space_from_xyz(const mel_color_space *s) {
    return mel_mat3_inverse(mel__space_to_xyz(s));
}
