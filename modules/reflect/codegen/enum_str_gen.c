...
static enum CXChildVisitResult visit_item(CXCursor c, CXCursor parent, CXClientData d)
{
    ...
    if (clang_getCursorKind(c) == CXCursor_AnnotateAttr)
    {
        ...
        if (strncmp(a, "mel:str:", 8) == 0)
            it->label = dup_cstr(a + 8);
        else if (strcmp(a, "mel:skip") == 0)
            it->skip = 1;
        free(a);
    }
...
static enum CXChildVisitResult visit_tu(CXCursor c, CXCursor parent, CXClientData d)
{
    ...
    if (clang_Cursor_getNumArguments(c) != 1)
        return CXChildVisit_Continue;
...
static size_t common_prefix_len(const Func* f)
{
    ...
    for (size_t i = 0; i < f->n; i++)
    {
        ...
        size_t j = 0;
        while (j < len && name[j] && base[j] == name[j])
            j++;
        len = j;
    }
    while (len > 0 && base[len - 1] != '_')
        len--;
    return len;
}
...
int main(int argc, char** argv)
{
    ...
    if (sep < 0)
    {
        fprintf(stderr, "enum_str_gen: missing '--' before headers\n");
        return 2;
    }
    ...
    if (spelling_n == 0)
    {
        fprintf(stderr, "enum_str_gen: no headers given\n");
        return 2;
    }

    size_t cap = 1;
    for (int i = 0; i < spelling_n; i++)
        cap += strlen(spellings[i]) + 12;
    ...
    CXTranslationUnit    tu = clang_parseTranslationUnit(idx, "mel_umbrella.c", clang_args, clang_n, &unsaved, 1, CXTranslationUnit_None);
    if (!tu)
    {
        fprintf(stderr, "enum_str_gen: failed to parse translation unit\n");
        return 1;
    }
