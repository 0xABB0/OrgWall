...
static void mel_smc_init(void)
{
    ...
    if (!mel_smc_key_info(mel_smc_fourcc("#KEY"), &count_type, &count_size))
        return;
    ...
    for (uint32_t i = 0; i < total; i++)
    {
        ...
        ein.data32 = i;
        if (mel_smc_call(&ein, &eout) != kIOReturnSuccess)
            continue;
        ...
        uint32_t type, size;
        if (!mel_smc_key_info(key, &type, &size))
            continue;
        if (type != flt)
            continue;

        if (c1 == 'p' || c1 == 'e')
            mel_smc_group_push(&g_cpu, key, type, size);
        else if (c1 == 'g')
            mel_smc_group_push(&g_gpu, key, type, size);
        else if (c1 == 'A' || c1 == 'a')
            mel_smc_group_push(&g_ambient, key, type, size);
    }
}

static Mel_Thermal_Temperature mel_smc_group_mean(const Mel_Smc_Group* g)
{
    f32      sum = 0.0f;
    uint32_t n = 0;
    ...
    return (Mel_Thermal_Temperature){ .celsius = sum / (f32)n, .fidelity = MEL_THERMAL_TEMP_MEASURED };
}
...
Mel_Thermal_Temperature mel_thermal_temperature(Mel_Thermal_Temp_Domain domain)
{
    ...
    switch (domain)
    {
    case MEL_THERMAL_TEMP_DOMAIN_CPU:
    case MEL_THERMAL_TEMP_DOMAIN_PRIMARY:
        return mel_smc_group_mean(&g_cpu);
    case MEL_THERMAL_TEMP_DOMAIN_GPU:
        return mel_smc_group_mean(&g_gpu);
    case MEL_THERMAL_TEMP_DOMAIN_AMBIENT:
    {
        Mel_Thermal_Temperature t = mel_smc_group_mean(&g_ambient);
        if (t.fidelity == MEL_THERMAL_TEMP_MEASURED)
            t.fidelity = MEL_THERMAL_TEMP_DERIVED;
        return t;
    }
    }
    return (Mel_Thermal_Temperature){ .celsius = 0.0f, .fidelity = MEL_THERMAL_TEMP_NONE };
