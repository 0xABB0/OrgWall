...
suspension set. Prefix `mel_coro_`.
...
Mel_Coro_Status sum_to__resume(Mel_Coro_Frame_sum_to* f, Mel_Coro_Yield* out);
```

The frame `struct` is emitted into the user's own header via the `mel_coro` macro, so it is
...
- `mel_coro(name, params, ret[, yield])` — marks the function via
  `__attribute__((annotate("mel:coro")))`. An optional fourth argument types the
...
- `mel_coro_yield(v)` / `mel_coro_yield()` — suspension point; surfaces `v` through the typed
  `Mel_Coro_Yield` channel, or suspends purely.
- `mel_coro_await(child)` — suspension point that drives a child continuation to completion,
...
- `mel_coro_return(v)` / `mel_coro_return()` — completion; writes the return slot and sets the
  terminal state. Falling off the end is an implicit `mel_coro_return()`.
...
Mel_Coro_Yield y;
while (sum_to__resume(&f, &y) == MEL_CORO_PENDING) {
    use(y);
}
i64 total = f.__ret;
```
...
2. lowers the body to a control-flow IR — basic blocks, edges, and a distinguished suspend
   edge at each `mel_coro_yield` / `mel_coro_await`;
...
switch (f->state) {
case 0:
    f->state = 1; return MEL_CORO_PENDING;
case 1:
    ;
}
```
...
`mel_coro_yield`, `mel_coro_await`, `mel_coro_return`, automatic lifting including loop
induction variables.
...
- **Tool** `tools/codegen/coro_gen.c` — libclang C API, same discovery and caching path
  as `enum_str_gen.c`. Parses an umbrella TU, finds `mel:coro`-annotated
...
- **Marker macros** in `modules/reflect/include/reflect/continuation.h`: `mel_coro`,
  `mel_coro_yield`, `mel_coro_await`, `mel_coro_return`, with their annotate-attribute
...
- **Module** `modules/coro/` (specified as `modules/continuation/`; renamed), top-level, namespace
  `<coro/...>`. Not under `async/`: a continuation is the non-async, non-fiber primitive, and
  filing it under async repeats a category error.
...
- **`mel_coro_await` resume cost** — a depth-N await chain resumes through N parent dispatchers
...
- **`continuation/abi`** depends on nothing — the frame struct shape, `Mel_Coro_Status`,
  `Mel_Coro_Yield`, the `resume` and return ABI, and a hand-written reference continuation with
  no codegen, which doubles as the differential-test oracle. Everything else targets this ABI.
- **`continuation/macros`** depends on the fixed ABI — `reflect/continuation.h`, the `mel_coro*`
...
- **`continuation/build-wiring`** depends on the tool — runner, build API, `nob.c` dep-list
  entry.
- **`continuation/compose`** depends on the ABI — `mel_coro_await`, the fiber-side reactor
