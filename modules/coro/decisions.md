...
3. **Single-TU codegen tool.** `codegen/coro_gen.c` is one file, mirroring
...
4. **The frame struct + prototype are injected into the user's own header; there is no separate
   ...
   `/* >>> mel_coro generated frames >>> */ … /* <<< */` region (idempotent) with the frame struct,
   layout-hash macro, and resume prototype, placed just before the first `mel_coro(`. Callers
...
5. **[DEVIATION] `Mel_Coro_Suspended` (a `bool`), not a `Mel_Coro_Status` enum.** MEL-CODE-001
...
6. **State sentinels are integer indices, not an enum.** `MEL_CORO_STATE_START` = 0,
   `MEL_CORO_STATE_DONE` = -1. The state is a true resume *index* (a counter), so integer constants
...
7. **[DEVIATION] The yield channel is an out-parameter and its type is inferred.** The spec sketches
   a generic `Mel_Coro_Yield*` and an optional 4th macro argument typing the channel. In reality the
...
10. **Reference offsets use the *spelling* location; call/structure offsets use the *expansion*
    location.** A macro-argument identifier (e.g. `acc` inside `mel_coro_yield(acc)`) collapses to
...
14. **[DEVIATION] `await` surface kept as `mel_coro_await(child_frame)`; the bootstrap is solved by
    iterative in-place injection.** The parent body names `Mel_Coro_Frame_child`, which does not
...
16. **[DEVIATION] The marked `*.coro.h` is both the codegen input and the includable interface; no
    ...
    (editor/normal mode), where `mel_coro(...) { … }` lowers to a discarded `static inline`
    ...
    anywhere). `cont.h` has exactly two modes: `MEL_CORO_CODEGEN` (clang-detectable sentinels +
...
18. **The codegen tool lives in-module (`codegen/`); the macros live in `<coro/coro.h>`, not
...
19. **Goldens are raw, unformatted output: the injected `.coro.h` plus the `.gen.c`.** Deterministic
