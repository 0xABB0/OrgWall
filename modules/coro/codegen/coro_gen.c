...
static enum CXChildVisitResult visit_body(CXCursor c, CXCursor parent, CXClientData d)
{
    ...
    else if (k == CXCursor_CallExpr && (is_marker(c, "__mel_coro_yield") || is_marker(c, "__mel_coro_await") || is_marker(c, "__mel_coro_return")))
    {
        g_f.susp = push(g_f.susp, &g_f.nsusp, sizeof(Suspend));
        Suspend* s = &g_f.susp[g_f.nsusp - 1];
        s->call_start = cursor_start(c);
        s->is_return = is_marker(c, "__mel_coro_return");
        s->is_await = is_marker(c, "__mel_coro_await");
        scan_call(s);
        if (s->has_value && clang_Cursor_getNumArguments(c) >= 2)
        {
            ...
            if (s->is_await)
            {
                ...
                char* p = strstr(ts, "Mel_Coro_Frame_");
                if (p)
                    s->child_name = dup_cstr(p + strlen("Mel_Coro_Frame_"));
                free(ts);
            }
            else
...
static void emit_func(Sb* hdr, Sb* impl)
{
    ...
    Sb frame = { 0 };
    sb_putf(&frame, "typedef struct Mel_Coro_Frame_%s\n{\n    i32 state;\n", f->name);
    ...
    sb_putf(&frame, "} Mel_Coro_Frame_%s;\n\n", f->name);

    sb_puts(hdr, frame.data);
    sb_putf(hdr, "#define MEL_CORO_LAYOUT_HASH_%s 0x%016llxull\n\n", f->name, (unsigned long long)layout_hash(fields.data));

    const char* yt = f->yield_type;
    if (yt)
    {
        sb_putf(hdr, "Mel_Coro_Suspended %s__resume(Mel_Coro_Frame_%s* __f, %s* __f_out);\n\n", f->name, f->name, yt);
    }
    else
    {
        sb_putf(hdr, "Mel_Coro_Suspended %s__resume(Mel_Coro_Frame_%s* __f);\n\n", f->name, f->name);
    }
    free(fields.data);
    free(frame.data);
    ...
    char* sig;
    if (yt)
        sig = heap_printf("Mel_Coro_Suspended %s__resume(Mel_Coro_Frame_%s* __f, %s* __f_out)\n{\n    switch (__f->state)\n    {\n    case MEL_CORO_STATE_START:;\n", f->name, f->name, yt);
    else
        sig = heap_printf("Mel_Coro_Suspended %s__resume(Mel_Coro_Frame_%s* __f)\n{\n    switch (__f->state)\n    {\n    case MEL_CORO_STATE_START:;\n", f->name, f->name);
    ...
    for (size_t i = 0; i < f->nsusp; i++)
    {
        ...
        if (s->is_await)
        {
            if (!s->child_name)
            {
                reject_at(s->call_start, "mel_coro_await argument is not a continuation frame");
                continue;
            }
            const char* cy = sig_yield(s->child_name);
            char*       fwd;
        ...
        else if (s->is_return)
        {
            if (s->has_value)
            {
                ...
                edit(s->close_paren, s->stmt_end, dup_cstr("); __f->state = MEL_CORO_STATE_DONE; return false; }"));
            }
            else
                edit(s->call_start, s->stmt_end, dup_cstr("{ __f->state = MEL_CORO_STATE_DONE; return false; }"));
        }
        else
        {
            if (s->has_value)
...
static enum CXChildVisitResult visit_yield_type(CXCursor c, CXCursor parent, CXClientData d)
{
    ...
    if (*out)
        return CXChildVisit_Break;
    if (clang_getCursorKind(c) == CXCursor_CallExpr && is_marker(c, "__mel_coro_yield") && clang_Cursor_getNumArguments(c) >= 2)
...
#define MARK_BEG "/* >>> mel_coro generated frames — managed region, do not edit >>> */"
#define MARK_END "/* <<< mel_coro generated frames <<< */"
...
static Sb inject_region(const char* src, const char* region)
{
    ...
    const char* mc = strstr(src, "mel_coro(");
    if (!mc)
    {
        sb_puts(&out, src);
        return out;
...
static int write_depfile(CXTranslationUnit tu, const char* target, const char* header_path)
{
    char* path = malloc(strlen(target) + 3);
    sprintf(path, "%s.d", target);
    FILE* f = fopen(path, "w");
    if (!f)
    {
        fprintf(stderr, "coro_gen: cannot open %s\n", path);
        free(path);
        return -1;
    }
    write_dep_path(f, target);
    fputc(':', f);
    fputc(' ', f);
    write_dep_path(f, header_path);
...
static int run_pass(CXIndex idx, const char* path, const char** args, int n, int quiet, const char* dep_target)
{
    free(g_src);
    g_src = read_file(path, &g_src_len);
    if (!g_src)
    {
        fprintf(stderr, "coro_gen: cannot read %s\n", path);
        return -1;
    }
    ...
    if (!tu)
    {
        fprintf(stderr, "coro_gen: failed to parse %s\n", path);
        return -1;
    }
    ...
    if (dep_target && write_depfile(tu, dep_target, path) < 0)
        parse_fatal = -1;
...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        fprintf(stderr, "usage: coro_gen <header.h> <out.gen.c> [clang-args...]\n");
        return 2;
    }
    ...
    {
        if (run_pass(idx, header_path, args, n, 1, NULL) < 0)
            return 1;
        Sb h = inject_region(g_src, g_hdr.data ? g_hdr.data : "");
        if (strcmp(h.data, g_src) == 0)
        {
            free(h.data);
            break;
        }
    ...
    int pf = run_pass(idx, header_path, args, n, 0, out_c);
    if (pf < 0 || pf)
        return 1;
    if (g_fatal)
        return 1;
    if (g_nfound == 0)
        fprintf(stderr, "coro_gen: no mel_coro definitions in %s\n", header_path);
