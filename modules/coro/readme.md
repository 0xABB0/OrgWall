...
A coro (formerly the `continuation` module) is a **stackless coroutine whose suspended state is a
reified delimited continuation**: a flat, owned data value holding a resume index and every local
that survives a suspension. The runtime object *is* the delimited continuation; the codegen that
mints it is defunctionalization of continuations over a CPS-explicit suspension set. Namespace
`<coro/...>`, symbol prefix `mel_coro_`.

The module owns its codegen tool (`coro-gen`, a `nob` host tool) and is built and tested through
the framework: `./nob test coro-test-<fixture>` runs the differential suite, `./nob build
coro-example` the runnable example. See [decisions.md](decisions.md) for every decision and
deviation from [spec.md](spec.md).
...
```c
typedef struct Mel_Coro_Frame_name {
...
#define MEL_CORO_LAYOUT_HASH_name 0x....ull

Mel_Coro_Suspended name__resume(Mel_Coro_Frame_name* __f, Y* __f_out);   // __f_out present iff Y exists
```

- `state` is an **integer index, never a code pointer** — this is what makes the frame relocatable
  and reload-safe. Sentinels live in `<coro/abi.h>`: `MEL_CORO_STATE_START` (0),
  `MEL_CORO_STATE_DONE` (-1).
- `Mel_Coro_Suspended` (a `bool`, from `abi.h`) is `true` while the continuation is still suspended
...
inside a managed `/* >>> mel_coro generated frames >>> */` region, so `sizeof` and `{0}` init work
...
Write an ordinary-looking C function in a header (`*.coro.h`) and mark it. The four macros are in
`<coro/coro.h>`. Codegen injects the frame struct + prototype into this same header; anyone
...
/*   (codegen injects Mel_Coro_Frame_sum_to + sum_to__resume here) */
/* <<< mel_coro generated frames <<< */
...
You write only the `mel_coro(...) { … }` body and (after the first build) keep the committed file as
...
- `mel_coro(name, params, ret)` — marks the function. `params` carries its own parentheses;
  `ret` is required (use `void`).
- `mel_coro_yield(v)` / `mel_coro_yield()` — a suspension point; surfaces `v` through the typed
...
- `mel_coro_await(child)` — a suspension point that drives a child continuation frame to completion,
...
- `mel_coro_return(v)` / `mel_coro_return()` — completion; writes `__ret` and sets the terminal
  state. Falling off the end is an implicit `mel_coro_return()`.

### Driving a frame

```c
Mel_Coro_Frame_sum_to f = {0};
...
1. parses the marked header (with `-DMEL_CORO_CODEGEN`), under which the macros expand to
   clang-detectable, error-free sentinel calls and an `annotate("mel:coro")` attribute;
...
`mel_coro_yield`, `mel_coro_await`, `mel_coro_return`, automatic lifting including loop induction
...
- **`mel_coro_await`.** Drives a child frame embedded in the parent frame. When the parent's and
...
- **Loop / async.** The `await` module drives a frame as a `Mel_Task`: it resumes on wake and, on a
  "waiting on X" yield (`Mel_Await_Step`), registers the task with X (future `then`, channel op,
  vat timer). The bridge lives entirely in `await`; this core carries no loop dependency.
...
- **Serialization.** Turning a frame into bytes belongs to the owning save/netcode module. This
  module owes only the flat, pointer-free layout and the per-continuation `MEL_CORO_LAYOUT_HASH_*`,
  so a consumer can refuse or migrate a mismatched frame.

## Build & test

Standard `nob` targets, from the repo root:

```sh
./nob test coro-test-sum_to        # one differential fixture (also: countdown, classify, relay, repeat_sum)
./nob build coro-gen               # just the codegen host tool
./nob build coro-example           # the runnable example
```

The codegen tool's contract:

```
coro_gen <header.coro.h> <out.gen.c> [clang-args...]
```
...
The build runs the tool on the committed fixtures in place, so the injected regions are real,
committed artifacts.

### The test suite (tested like a compiler)

- **Differential execution** (`test/driver/*_diff.c`, the `coro-test-*` nob targets): a
  hand-written straight-line oracle; the driven continuation must yield the identical sequence and
  return value. Catches liveness miscompute behaviorally. Fixtures cover `for` + induction lifting
  (`sum_to`), pure-yield `while` (`countdown`), `if`/`else` (`classify`), `do`/`while`
  (`repeat_sum`), and `await` with forwarding (`relay`).
- **Golden frame layout** (`test/golden/`): the injected `.coro.h` (frame region in context) and
  the repo-style-formatted `.gen.c` per fixture, committed so layout drift shows in review. Not
  currently diffed by a nob target (the original self-contained driver owned that check); owed.
- **Snapshot round-trip** (`test/driver/snapshot.c`): `memcpy` a mid-flight frame, resume the copy,
  assert identical continuation — proving self-containment. Not currently wired to a nob target.
- **Rejection fixtures** (`test/reject/`): each rejected construct asserts a hard build error at the
  right `file:line`. Not currently wired to a nob target.

## Layout

```
include/coro/abi.h           ABI: state sentinels, Mel_Coro_Suspended, Mel_Coro_Layout_Hash
include/coro/coro.h          the mel_coro* macro surface (codegen + editor modes)
codegen/coro_gen.c           the libclang source-to-source transform (the compiler)
build.c                      nob targets: coro lib, coro-gen tool, coro-example, coro-test-*
example/ticker.coro.h        a runnable continuation (the injected region is committed)
example/app.c                drives it — the canonical "how you use this"
test/fixtures/*.coro.h       marked continuations (codegen injects their regions in place)
test/driver/*.c              differential oracles + drivers + snapshot, and harness.h
test/golden/*.coro.h,*.gen.c committed golden output (injected header + resume impl)
test/reject/*.coro.c         fixtures that must be rejected with file:line
