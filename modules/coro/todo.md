...
- **`mel_coro_await` flattening.** A depth-N await chain resumes through N parent dispatchers per
...
- a `mel_build_generate_continuation(t, "path/to/marked.coro.c")` build API backed by a
...
The per-continuation `MEL_CORO_LAYOUT_HASH_*` is emitted but unused here. The owning save/netcode
module consumes it to refuse or migrate a frame whose layout drifted. A snapshot helper
(`mel_coro_frame_blit` or similar) and a versioned envelope belong there, not here.
