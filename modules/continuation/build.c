...
static void cont_test(Mel_Build* b, const char* name)
{
    ...
    mel_depends(t, "core");
    mel_codegen(t, "continuation-gen", fmt("%s.gen.c", name), fmt("$dir/test/fixtures/%s.cont.h", name), "$out", "-DMEL_CONT_CODEGEN", "$cflags", "$hostclang");
}
...
void build(Mel_Build* b)
{
    ...
    mel_depends(ex, "core");
    mel_codegen(ex, "continuation-gen", "ticker.gen.c", "$dir/example/ticker.cont.h", "$out", "-DMEL_CONT_CODEGEN", "$cflags", "$hostclang");

    static const char* fixtures[] = { "sum_to", "countdown", "classify", "relay", "repeat_sum" };
    for (int i = 0; i < 5; i++)
        cont_test(b, fixtures[i]);
}
