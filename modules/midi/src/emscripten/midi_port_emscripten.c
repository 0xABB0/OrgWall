...
EM_JS(int, mel_midi_web__count, (void), { return (globalThis.MelMidi && MelMidi.ready) ? MelMidi.inputs.length : 0; });
...
EM_JS(int, mel_midi_web__open, (int idx, void* port), {
    ...
    {
        const d = e.data;
        _mel_midi_web__on_message(port, d.length > 0 ? d[0] : 0, d.length > 1 ? d[1] : 0, d.length > 2 ? d[2] : 0, d.length, e.timeStamp);
    };
    return 1;
});
...
EM_JS(void, mel_midi_web__close, (void* port), {
    ...
    if (inp)
    {
        ...
        delete MelMidi.open[port];
    }
});

EMSCRIPTEN_KEEPALIVE void mel_midi_web__on_message(void* port_p, int b0, int b1, int b2, int len, double ts_ms)
{
    ...
    if (len > 0)
        chunk.data[0] = (uint8_t)b0;
    if (len > 1)
        chunk.data[1] = (uint8_t)b1;
    if (len > 2)
        chunk.data[2] = (uint8_t)b2;
    chunk.length = len;
...
int32_t mel_midi_port_platform_enumerate(Mel_Midi_Port_Info* out_infos, int32_t max_count)
{
    ...
    if (count > MEL_WEB_MIDI_MAX)
        count = MEL_WEB_MIDI_MAX;
