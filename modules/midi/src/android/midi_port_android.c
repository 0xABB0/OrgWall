...
{
    if (mel__midi_inited)
        return mel__midi_class != NULL;
    if (mel__midi_init_failed)
        return false;
...
int32_t mel_midi_port_platform_enumerate(Mel_Midi_Port_Info* out_infos, int32_t max_count)
{
    ...
    if (env == NULL || !mel__midi_resolve(env))
        return 0;
...
Mel_Midi_Port* mel_midi_port_platform_open_input(int32_t id, Mel_Midi_Port* port)
{
    ...
    jobject handle_local = (*env)->CallStaticObjectMethod(env, mel__midi_class, mel__midi_open, (jint)id, (jlong)(uintptr_t)port);
    if ((*env)->ExceptionCheck(env))
    {
        (*env)->ExceptionClear(env);
        return NULL;
...
{
    if (port == NULL || port->platform_handle == NULL)
        return;
    JNIEnv* env = mel_platform_android_env();
    if (env == NULL)
        return;
