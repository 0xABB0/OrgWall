...
bool mel_midi_parse(const Mel_Midi_Chunk* chunk, Mel_Midi_Msg* out_msg)
{
    ...
    if (status >= 0xF8)
    {
        ...
        {
        case 0xF8:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_CLOCK;
            return true;
        case 0xFA:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_START;
            return true;
        case 0xFB:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_CONTINUE;
            return true;
        case 0xFC:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_STOP;
            return true;
        case 0xFE:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_ACTIVE_SENSING;
            return true;
        case 0xFF:
            out_msg->kind = MEL_MIDI_MSG_SYSTEM_RESET;
            return true;
        default:
            return false;
        }
...
bool mel_midi_encode(const Mel_Midi_Msg* msg, Mel_Midi_Chunk* out_chunk)
{
    ...
    case MEL_MIDI_MSG_SYSTEM_CLOCK:
        out_chunk->data[0] = 0xF8;
        out_chunk->length = 1;
        return true;
    case MEL_MIDI_MSG_SYSTEM_START:
        out_chunk->data[0] = 0xFA;
        out_chunk->length = 1;
        return true;
    case MEL_MIDI_MSG_SYSTEM_CONTINUE:
        out_chunk->data[0] = 0xFB;
        out_chunk->length = 1;
        return true;
    case MEL_MIDI_MSG_SYSTEM_STOP:
        out_chunk->data[0] = 0xFC;
        out_chunk->length = 1;
        return true;
    case MEL_MIDI_MSG_SYSTEM_ACTIVE_SENSING:
        out_chunk->data[0] = 0xFE;
        out_chunk->length = 1;
        return true;
    case MEL_MIDI_MSG_SYSTEM_RESET:
        out_chunk->data[0] = 0xFF;
        out_chunk->length = 1;
        return true;
