...
static i32 reactor_priority_bucket(i32 priority)
{
    ...
    if (b < 0)
        b = 0;
    if (b >= MEL_REACTOR_PRIORITY_BUCKETS)
        b = MEL_REACTOR_PRIORITY_BUCKETS - 1;
    return (i32)b;
}
...
static void reactor_source_dispose(Mel_Reactor_Source* s)
{
    ...
    if (s->cb && s->cb->finalize)
        s->cb->finalize(s);
    reactor_source_unlink(s);
    if (s->polls)
        mel_dealloc(alloc, s->polls);
    if (!ext)
        mel_dealloc(alloc, s);
}
...
static bool reactor_iterate(Mel_Reactor* r, bool may_block)
{
    ...
    {
        if (s->destroyed || s->detach_pending)
            continue;
        bool ready = false;
        if (s->ready_time >= 0)
        {
    ...
    for (i32 bucket = 0; bucket < MEL_REACTOR_PRIORITY_BUCKETS; bucket++)
    {
        ...
        while (s)
        {
            ...
            if (reactor_priority_bucket(s->priority) != bucket)
            {
                s = next;
                continue;
            }
            bool ready = s->ready;
            if (!ready && s->cb && s->cb->check)
                ready = s->cb->check(s);
            if (ready)
            {
                s->ready = false;
...
bool mel_reactor_is_running(const Mel_Reactor* r) { return r && atomic_load((atomic_bool*)&r->running); }
...
{
    if (struct_size < sizeof(Mel_Reactor_Source))
        struct_size = sizeof(Mel_Reactor_Source);
...
i32 mel_reactor_source_get_priority(const Mel_Reactor_Source* s) { return s ? s->priority : MEL_REACTOR_PRIORITY_DEFAULT; }
...
void mel_reactor_source_set_ready_time(Mel_Reactor_Source* s, i64 ns)
{
    ...
    s->ready_time = ns;
    if (s->reactor)
        reactor_backend_wake(s->reactor);
}

i64 mel_reactor_source_get_ready_time(const Mel_Reactor_Source* s) { return s ? s->ready_time : MEL_REACTOR_READY_TIME_NEVER; }

Mel_Reactor* mel_reactor_source_reactor(const Mel_Reactor_Source* s) { return s ? s->reactor : NULL; }
...
void mel_reactor_source_add_poll(Mel_Reactor_Source* s, Mel_Reactor_Poll* p)
{
    ...
    if (s->poll_count == s->poll_cap)
    {
        ...
        Mel_Reactor_Poll** grown = s->polls ? mel_realloc(alloc, s->polls, cap * sizeof *grown) : mel_alloc(alloc, cap * sizeof *grown);
        if (!grown)
            return;
        s->polls = grown;
        s->poll_cap = (u16)cap;
...
static bool reactor_timer_dispatch(Mel_Reactor_Source* s, Mel_Reactor_Source_Proc cb, void* user)
{
    ...
    if (t->interval_ns > 0)
    {
        ...
        if (s->ready_time <= now)
            s->ready_time = now + t->interval_ns;
    }
    else
    {
        s->ready_time = now;
