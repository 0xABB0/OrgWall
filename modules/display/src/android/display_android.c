...
static bool resolve(JNIEnv* env)
{
    ...
    g.enumerate = (*env)->GetStaticMethodID(env, g.cls, "enumerate", "()[Lorgwall/melody/display/MelodyDisplay$Info;");
...
static Mel_Display_State map_state(jint s)
{
    switch (s)
    {
    case MEL_ANDROID_STATE_ON:
        return MEL_DISPLAY_STATE_ACTIVE;
    case MEL_ANDROID_STATE_OFF:
        return MEL_DISPLAY_STATE_POWERED_OFF;
    case MEL_ANDROID_STATE_DOZE:
    case MEL_ANDROID_STATE_DOZE_SUSPEND:
        return MEL_DISPLAY_STATE_DIMMED;
    case MEL_ANDROID_STATE_ON_SUSPEND:
        return MEL_DISPLAY_STATE_IDLE;
    default:
        return MEL_DISPLAY_STATE_ACTIVE;
    }
}
...
static void fill_modes(JNIEnv* env, jobject info, Mel_Display_Descriptor* d)
{
    ...
    if (n > MEL_DISPLAY_MAX_MODES)
        n = MEL_DISPLAY_MAX_MODES;
...
u32 mel_display__enumerate(const Mel_Alloc* alloc, Mel_Display_Raw* out, u32 cap)
{
    ...
    for (jsize i = 0; i < count && n < cap; i++)
    {
        ...
        d->connector = (id == 0) ? MEL_DISPLAY_CONNECTOR_INTERNAL : MEL_DISPLAY_CONNECTOR_UNKNOWN;
        ...
        hdr->mastering_primaries_support = is_hdr ? MEL_DISPLAY_MASTERING_STATIC : MEL_DISPLAY_MASTERING_NONE;
