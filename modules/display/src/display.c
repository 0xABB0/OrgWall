...
u32 mel_display_refresh(void)
{
    ...
    for (u32 i = 0; i < n; i++)
    {
        ...
        if (e)
        {
            ...
            mel_display_events__emit((Mel_Display_Event){ .kind = kind, .display = { e->handle }, .changed_fields = fields });
            continue;
        }
        ...
        seen[idx] = true;
        mel_display_events__emit((Mel_Display_Event){ .kind = MEL_DISPLAY_EVENT_ADDED, .display = { h } });
    }
    ...
    for (u32 i = 0; i < g_reg.entry_count;)
    {
        ...
        mel_display_events__emit((Mel_Display_Event){ .kind = MEL_DISPLAY_EVENT_REMOVED, .display = { h } });
