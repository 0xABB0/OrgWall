...
static bool is_hdr10_space(DXGI_COLOR_SPACE_TYPE cs) { return cs == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020; }
...
static void fill_modes(IDXGIOutput* output, Mel_Display_Descriptor* d)
{
    ...
    if (SUCCEEDED(IDXGIOutput_GetDisplayModeList(output, fmt, 0, &num, modes)))
    {
        ...
        {
            bool interlaced = modes[i].ScanlineOrdering == DXGI_MODE_SCANLINE_ORDER_UPPER_FIELD_FIRST || modes[i].ScanlineOrdering == DXGI_MODE_SCANLINE_ORDER_LOWER_FIELD_FIRST;
...
static void fill_from_desc(const DXGI_OUTPUT_DESC1* desc, Mel_Display_Descriptor* d)
{
    ...
    d->state = desc->AttachedToDesktop ? MEL_DISPLAY_STATE_ACTIVE : MEL_DISPLAY_STATE_DISCONNECTED;
    ...
    hdr->mastering_primaries_support = (hdr_space && desc->BitsPerColor >= 10) ? MEL_DISPLAY_MASTERING_STATIC : MEL_DISPLAY_MASTERING_NONE;
