...
static Mel_Display_Connector connector_from_name(const char* name)
{
    if (strncmp(name, "eDP", 3) == 0 || strncmp(name, "LVDS", 4) == 0)
        return MEL_DISPLAY_CONNECTOR_INTERNAL;
    if (strncmp(name, "HDMI", 4) == 0)
        return MEL_DISPLAY_CONNECTOR_HDMI;
    if (strncmp(name, "DP", 2) == 0 || strncmp(name, "DisplayPort", 11) == 0)
        return MEL_DISPLAY_CONNECTOR_DISPLAYPORT;
    if (strncmp(name, "VGA", 3) == 0)
        return MEL_DISPLAY_CONNECTOR_VGA;
...
u32 mel_display__enumerate(const Mel_Alloc* alloc, Mel_Display_Raw* out, u32 cap)
{
    ...
    for (int o = 0; o < res->noutput && n < cap; o++)
    {
        ...
        size_t nlen = (size_t)oi->nameLen < MEL_DISPLAY_NAME_CAP - 1 ? (size_t)oi->nameLen : MEL_DISPLAY_NAME_CAP - 1;
