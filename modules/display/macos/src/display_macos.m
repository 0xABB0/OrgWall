...
enum
{
    MEL_MACOS_INTERLACED_FLAG = 0x00000200u
};
...
static void fill_hdr(NSScreen* screen, Mel_Display_Descriptor* d)
{
    ...
    hdr->mastering_primaries_support = hdr->has_edr ? MEL_DISPLAY_MASTERING_STATIC : MEL_DISPLAY_MASTERING_NONE;
...
static Mel_Display_State display_state(CGDirectDisplayID id)
{
    ...
    if (CGDisplayIsInMirrorSet(id))
        return MEL_DISPLAY_STATE_MIRRORED;
...
u32 mel_display__enumerate(const Mel_Alloc* alloc, Mel_Display_Raw* out, u32 cap)
{
    ...
    for (NSScreen* screen in screens)
    {
        ...
        d->connector = CGDisplayIsBuiltin(id) ? MEL_DISPLAY_CONNECTOR_INTERNAL : MEL_DISPLAY_CONNECTOR_UNKNOWN;
...
{
    u64 id;
    if (!mel_display__stable_id(d, &id))
        return nil;
    for (NSScreen* screen in NSScreen.screens)
    {
        NSNumber* num = screen.deviceDescription[@"NSScreenNumber"];
        if (num && (u64)num.unsignedIntValue == id)
            return screen;
    }
    return nil;
}
