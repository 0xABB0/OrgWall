...
MEL_TEST(barcode_qr, ec_table_matches_total_codewords)
{
    for (i32 v = 0; v < 10; ++v)
    {
        for (i32 r = 0; r < 4; ++r)
        {
            ...
            MEL_REQUIRE_EQ(e[1] * e[2] + e[3] * e[4] + (e[1] + e[3]) * e[0], MEL__QR_TOTAL_T[v]);
        }
    }
}
...
MEL_TEST(barcode_qr, iso_numeric_example)
{
    ...
    usize count = 0;
    i32   version = 0;
    MEL_REQUIRE(mel_qr_codewords("01234567", mel_qr_ecc_m(), 0, mel_alloc_heap(), &cw, &count, &version));
...
MEL_TEST(barcode_qr, version_selection_and_capacity)
{
    ...
    usize count = 0;
    i32   version = 0;
    MEL_REQUIRE(mel_qr_codewords("HELLO WORLD", mel_qr_ecc_q(), 0, mel_alloc_heap(), &cw, &count, &version));
    MEL_REQUIRE_EQ(version, 1);
    mel_dealloc(mel_alloc_heap(), cw);
    ...
    big[2048] = '\0';
    MEL_REQUIRE(!mel_qr_codewords(big, mel_qr_ecc_h(), 0, mel_alloc_heap(), &cw, &count, &version));
}
...
MEL_TEST(barcode_qr, multiblock_interleave_roots)
{
    ...
    usize count = 0;
    i32   version = 0;
    MEL_REQUIRE(mel_qr_codewords(payload, mel_qr_ecc_q(), 0, mel_alloc_heap(), &cw, &count, &version));
    ...
    for (i32 b = 0; b < 2; ++b)
    {
        ...
        for (u32 i = 0; i < 18; ++i)
        {
            MEL_REQUIRE_EQ((i32)mel__poly_eval(&f, block, 35, mel_gf_pow(&f, 2, i)), 0);
        }
    }
    mel_gf_free(&f);
    mel_dealloc(mel_alloc_heap(), cw);
...
static bool mel__qr_isfunc(i32 size, i32 ver, i32 r, i32 c)
{
    ...
    for (i32 a = 1; a <= al[0]; ++a)
    {
        ...
        {
            bool corner = (a == 1 && b == 1) || (a == 1 && b == al[0]) || (a == al[0] && b == 1);
            if (corner)
            {
                continue;
            }
...
static void mel__qr_roundtrip(const char* data, mel_qr_opt opt, i32 expect_version)
{
...
MEL_TEST(barcode_qr, encode_roundtrip_v1) { mel__qr_roundtrip("01234567", (mel_qr_opt){ .ecc = mel_qr_ecc_m(), .version = 0, .mask = -1 }, 1); }
...
MEL_TEST(barcode_qr, encode_roundtrip_multiblock)
{
    ...
    payload[32] = '\0';
    mel__qr_roundtrip(payload, (mel_qr_opt){ .ecc = mel_qr_ecc_q(), .version = 0, .mask = -1 }, 3);
}
