...
MEL_TEST(barcode_itf, odd_length_policy) {
    ...
    MEL_REQUIRE(mel_itf_encode(&m, "123", 10, (mel_itf_opt){.pad_odd = true},
                               mel_alloc_heap()));
    MEL_REQUIRE_EQ(m.width, 4 + 18 * 2 + 5);
    mel_barcode_matrix_free(&m);
}
...
MEL_TEST(barcode_itf, checksum_then_pad) {
    mel_barcode_matrix m;
    MEL_REQUIRE(mel_itf_encode(&m, "1234", 10,
                               (mel_itf_opt){.checksum = true, .pad_odd = true},
                               mel_alloc_heap()));
