...
MEL_TEST(barcode_code39, single_char_pattern) {
    static const char* expected =
        "100010111011101" "0" "111010100010111" "0" "100010111011101";

    mel_barcode_matrix m;
    MEL_REQUIRE(mel_code39_encode(&m, "A", 30, (mel_code39_opt){0},
                                  mel_alloc_heap()));
...
MEL_TEST(barcode_code39, width_formula) {
    mel_barcode_matrix m;
    MEL_REQUIRE(mel_code39_encode(&m, "CODE39", 10, (mel_code39_opt){0},
                                  mel_alloc_heap()));
    MEL_REQUIRE_EQ(m.width, 15 * 8 + 7);
    mel_barcode_matrix_free(&m);

    MEL_REQUIRE(mel_code39_encode(&m, "CODE39", 10, (mel_code39_opt){.checksum = true},
                                  mel_alloc_heap()));
...
MEL_TEST(barcode_code39, rejects_invalid) {
    mel_barcode_matrix m;
    MEL_REQUIRE(!mel_code39_encode(&m, "abc", 10, (mel_code39_opt){0},
                                   mel_alloc_heap()));
    MEL_REQUIRE(!mel_code39_encode(&m, "A*B", 10, (mel_code39_opt){0},
                                   mel_alloc_heap()));
    MEL_REQUIRE(!mel_code39_encode(&m, "", 10, (mel_code39_opt){0},
                                   mel_alloc_heap()));
}
