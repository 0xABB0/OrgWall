...
void input_buffer_update(Input_Buffer* buf, bool U, bool D, bool L, bool R, bool a, bool b, bool c, bool x, bool y, bool z, bool s, bool d, bool w, bool m)
{
    socd_resolve(&U, &D, &buf->socd_first[0], &buf->socd_first[1], buf->socd_mode);

    if (buf->socd_mode == SOCD_UP_PRIORITY && U && D)
        D = false;
...
static i32 state_dir_hold_no_dollar(Input_Buffer* buf, Cmd_Key_Id key)
{
    ...
    {
    case CK_U:
        conflict = -max3(buf->Bb, buf->Db, buf->Fb);
        intended = buf->Ub;
        break;
    case CK_D:
        conflict = -max3(buf->Bb, buf->Ub, buf->Fb);
        intended = buf->Db;
        break;
    case CK_B:
        conflict = -max3(buf->Db, buf->Ub, buf->Fb);
        intended = buf->Bb;
        break;
    case CK_F:
        conflict = -max3(buf->Db, buf->Ub, buf->Bb);
        intended = buf->Fb;
        break;
    case CK_L:
        conflict = -max3(buf->Db, buf->Ub, buf->Rb);
        intended = buf->Lb;
        break;
    case CK_R:
        conflict = -max3(buf->Db, buf->Ub, buf->Lb);
        intended = buf->Rb;
        break;
    case CK_N:
        return buf->Nb;
    ...
    case CK_UB:
        conflict = -max2(buf->Db, buf->Fb);
        intended = min2(buf->Ub, buf->Bb);
        break;
    case CK_UF:
        conflict = -max2(buf->Db, buf->Bb);
        intended = min2(buf->Ub, buf->Fb);
        break;
    case CK_DB:
        conflict = -max2(buf->Ub, buf->Fb);
        intended = min2(buf->Db, buf->Bb);
        break;
    case CK_DF:
        conflict = -max2(buf->Ub, buf->Bb);
        intended = min2(buf->Db, buf->Fb);
        break;
    case CK_UL:
        conflict = -max2(buf->Db, buf->Rb);
        intended = min2(buf->Ub, buf->Lb);
        break;
    case CK_UR:
        conflict = -max2(buf->Db, buf->Lb);
        intended = min2(buf->Ub, buf->Rb);
        break;
    case CK_DL:
        conflict = -max2(buf->Ub, buf->Rb);
        intended = min2(buf->Db, buf->Lb);
        break;
    case CK_DR:
        conflict = -max2(buf->Ub, buf->Lb);
        intended = min2(buf->Db, buf->Rb);
        break;
...
static i32 state_dir_hold_dollar(Input_Buffer* buf, Cmd_Key_Id key)
{
    ...
    {
    case CK_U:
        return buf->Ub > 0 ? all_min : 0;
    case CK_D:
        return buf->Db > 0 ? all_min : 0;
    case CK_B:
        return buf->Bb > 0 ? all_min : 0;
    case CK_F:
        return buf->Fb > 0 ? all_min : 0;
    case CK_L:
        return buf->Lb > 0 ? all_min : 0;
    case CK_R:
        return buf->Rb > 0 ? all_min : 0;

    case CK_UB:
        return (buf->Ub > 0 && buf->Bb > 0) ? all_min : 0;
    case CK_UF:
        return (buf->Ub > 0 && buf->Fb > 0) ? all_min : 0;
    case CK_DB:
        return (buf->Db > 0 && buf->Bb > 0) ? all_min : 0;
    case CK_DF:
        return (buf->Db > 0 && buf->Fb > 0) ? all_min : 0;
    case CK_UL:
        return (buf->Ub > 0 && buf->Lb > 0) ? all_min : 0;
    case CK_UR:
        return (buf->Ub > 0 && buf->Rb > 0) ? all_min : 0;
    case CK_DL:
        return (buf->Db > 0 && buf->Lb > 0) ? all_min : 0;
    case CK_DR:
        return (buf->Db > 0 && buf->Rb > 0) ? all_min : 0;

    case CK_N:
    {
        i32 r = all_min;
        r = min2(r, absi(buf->ab));
        r = min2(r, absi(buf->bb));
        r = min2(r, absi(buf->cb));
        r = min2(r, absi(buf->xb));
        r = min2(r, absi(buf->yb));
        r = min2(r, absi(buf->zb));
        r = min2(r, absi(buf->sb));
        r = min2(r, absi(buf->db));
        r = min2(r, absi(buf->wb));
        return r;
    }
...
i32 input_buffer_state(Input_Buffer* buf, Command_Key key)
{
    ...
    if (!key.tilde && !key.dollar)
        return state_dir_hold_no_dollar(buf, key.key);
    if (!key.tilde && key.dollar)
        return state_dir_hold_dollar(buf, key.key);
    if (key.tilde && !key.dollar)
        return state_dir_release_no_dollar(buf, key.key);
...
static i32 charge_dir_hold_dollar(Input_Buffer* buf, Cmd_Key_Id key)
{
    ...
    case CK_UB:
        return min2(buf->Ub, buf->Bb);
    case CK_UF:
        return min2(buf->Ub, buf->Fb);
    case CK_DB:
        return min2(buf->Db, buf->Bb);
    case CK_DF:
        return min2(buf->Db, buf->Fb);
    case CK_UL:
        return min2(buf->Ub, buf->Lb);
    case CK_UR:
        return min2(buf->Ub, buf->Rb);
    case CK_DL:
        return min2(buf->Db, buf->Lb);
    case CK_DR:
        return min2(buf->Db, buf->Rb);
...
static i32 charge_dir_release_dollar(Input_Buffer* buf, Cmd_Key_Id key)
{
    ...
    case CK_UB:
        return min2(buf->Up, buf->Bp);
    case CK_UF:
        return min2(buf->Up, buf->Fp);
    case CK_DB:
        return min2(buf->Dp, buf->Bp);
    case CK_DF:
        return min2(buf->Dp, buf->Fp);
    case CK_UL:
        return min2(buf->Up, buf->Lp);
    case CK_UR:
        return min2(buf->Up, buf->Rp);
    case CK_DL:
        return min2(buf->Dp, buf->Lp);
    case CK_DR:
        return min2(buf->Dp, buf->Rp);
...
static i32 charge_dir_hold_no_dollar(Input_Buffer* buf, Cmd_Key_Id key)
{
    ...
    case CK_U:
        conflict = -max3(buf->Bb, buf->Db, buf->Fb);
        strict = min2(conflict, buf->Ub);
        break;
    case CK_D:
        conflict = -max3(buf->Bb, buf->Ub, buf->Fb);
        strict = min2(conflict, buf->Db);
        break;
    case CK_B:
        conflict = -max3(buf->Ub, buf->Db, buf->Fb);
        strict = min2(conflict, buf->Bb);
        break;
    case CK_F:
        conflict = -max3(buf->Ub, buf->Db, buf->Bb);
        strict = min2(conflict, buf->Fb);
        break;
    case CK_L:
        conflict = -max3(buf->Ub, buf->Db, buf->Rb);
        strict = min2(conflict, buf->Lb);
        break;
    case CK_R:
        conflict = -max3(buf->Ub, buf->Db, buf->Lb);
        strict = min2(conflict, buf->Rb);
        break;
    case CK_N:
        return buf->Nb;
    ...
    case CK_UB:
        conflict = -max2(buf->Db, buf->Fb);
        strict = min2(conflict, min2(buf->Ub, buf->Bb));
        break;
    case CK_UF:
        conflict = -max2(buf->Db, buf->Bb);
        strict = min2(conflict, min2(buf->Ub, buf->Fb));
        break;
    case CK_DB:
        conflict = -max2(buf->Ub, buf->Fb);
        strict = min2(conflict, min2(buf->Db, buf->Bb));
        break;
    case CK_DF:
        conflict = -max2(buf->Ub, buf->Bb);
        strict = min2(conflict, min2(buf->Db, buf->Fb));
        break;
    case CK_UL:
        conflict = -max2(buf->Db, buf->Rb);
        strict = min2(conflict, min2(buf->Ub, buf->Lb));
        break;
    case CK_UR:
        conflict = -max2(buf->Db, buf->Lb);
        strict = min2(conflict, min2(buf->Ub, buf->Rb));
        break;
    case CK_DL:
        conflict = -max2(buf->Ub, buf->Rb);
        strict = min2(conflict, min2(buf->Db, buf->Lb));
        break;
    case CK_DR:
        conflict = -max2(buf->Ub, buf->Lb);
        strict = min2(conflict, min2(buf->Db, buf->Rb));
        break;
...
i32 input_buffer_state_charge(Input_Buffer* buf, Command_Key key)
{
    ...
    {
        if (!key.tilde)
            return *buf_counter(buf, key.key);
        return *buf_prev(buf, key.key);
    }

    if (!key.tilde && key.dollar)
        return charge_dir_hold_dollar(buf, key.key);
    if (key.tilde && key.dollar)
        return charge_dir_release_dollar(buf, key.key);
    if (!key.tilde && !key.dollar)
        return charge_dir_hold_no_dollar(buf, key.key);
