...
static u8 parse_statetype_char(str8 s)
{
    ...
    {
    case 'S':
        return MUGEN_PHYSICS_S;
    case 'C':
        return MUGEN_PHYSICS_C;
    case 'A':
        return MUGEN_PHYSICS_A;
    case 'N':
        return MUGEN_PHYSICS_N;
    case 'L':
        return MUGEN_PHYSICS_L;
    case 'U':
        return MUGEN_STATETYPE_U;
    }
    return 0;
}
...
static u8 parse_movetype_char(str8 s)
{
    ...
    case 'I':
        return MUGEN_MOVETYPE_I;
    case 'A':
        return MUGEN_MOVETYPE_A;
    case 'H':
        return MUGEN_MOVETYPE_H;
    case 'U':
        return MUGEN_MOVETYPE_U;
    }
    return MUGEN_MOVETYPE_I;
}
...
static u8 parse_controller_type(str8 s) { return mugen_sc_lookup_name(trim(s)); }

static i32 parse_trigger_level(str8 key)
{
    if (str8_ieq_cstr(key, "triggerall"))
        return 0;
...
static void finalize_controller(Mugen_State_Controller* sc, Ptr_List* triggerall_list, Ptr_List* trigger_groups, u32* group_sizes, u32 max_group, const Mel_Alloc* alloc)
{
    ...
    if (group_count > 0)
    {
        ...
        for (u32 i = 0; i < group_count; i++)
        {
            ...
            if (count > 0)
            {
                ...
                memcpy(sc->trigger_groups[i].conditions, (Mugen_Expr**)trigger_groups->items + offset, count * sizeof(Mugen_Expr*));
            }
            offset += count;
        }
    }
...
static void parse_constants_line(Mugen_Char_Constants* c, u8 section, str8 key, str8 val)
{
    ...
    case 1:
    {
        if (str8_ieq_cstr(key, "life"))
            c->life = parse_int(val);
        else if (str8_ieq_cstr(key, "attack"))
            c->attack = parse_int(val);
        else if (str8_ieq_cstr(key, "defence"))
            c->defence = parse_int(val);
        else if (str8_ieq_cstr(key, "fall.defence_up"))
            c->fall_defence_up = parse_int(val);
        else if (str8_ieq_cstr(key, "liedown.time"))
            c->liedown_time = parse_int(val);
        else if (str8_ieq_cstr(key, "airjuggle"))
            c->airjuggle = parse_int(val);
        else if (str8_ieq_cstr(key, "sparkno"))
            c->sparkno = parse_int(val);
        else if (str8_ieq_cstr(key, "guard.sparkno"))
            c->guard_sparkno = parse_int(val);
        else if (str8_ieq_cstr(key, "power.max"))
            c->power_max = parse_int(val);
        break;
    }
    case 2:
    {
        if (str8_ieq_cstr(key, "xscale"))
            c->xscale = parse_float(val);
        else if (str8_ieq_cstr(key, "yscale"))
            c->yscale = parse_float(val);
        else if (str8_ieq_cstr(key, "ground.back"))
            c->ground_back = parse_float(val);
        else if (str8_ieq_cstr(key, "ground.front"))
            c->ground_front = parse_float(val);
        else if (str8_ieq_cstr(key, "air.back"))
            c->air_back = parse_float(val);
        else if (str8_ieq_cstr(key, "air.front"))
            c->air_front = parse_float(val);
        else if (str8_ieq_cstr(key, "height"))
            c->height = parse_float(val);
        else if (str8_ieq_cstr(key, "attack.dist"))
            c->attack_dist = parse_float(val);
        break;
    }
    case 3:
    {
        ...
        if (str8_ieq_cstr(key, "walk.fwd"))
            c->walk_fwd_x = parse_float(first);
        else if (str8_ieq_cstr(key, "walk.back"))
            c->walk_back_x = parse_float(first);
        else if (str8_ieq_cstr(key, "run.fwd"))
        {
            c->run_fwd_x = parse_float(first);
            if (rest.len > 0)
                c->run_fwd_y = -parse_float(rest);
        }
        else if (str8_ieq_cstr(key, "run.back"))
        {
            c->run_back_x = parse_float(first);
            if (rest.len > 0)
                c->run_back_y = -parse_float(rest);
        }
        else if (str8_ieq_cstr(key, "jump.neu"))
        {
            c->jump_neu_x = parse_float(first);
            if (rest.len > 0)
                c->jump_y = -parse_float(rest);
        }
        else if (str8_ieq_cstr(key, "jump.back"))
            c->jump_back_x = parse_float(first);
        else if (str8_ieq_cstr(key, "jump.fwd"))
            c->jump_fwd_x = parse_float(first);
        else if (str8_ieq_cstr(key, "runjump.back"))
        {
            c->runjump_back_x = parse_float(first);
            if (rest.len > 0)
                c->runjump_y = -parse_float(rest);
        }
        ...
        else if (str8_ieq_cstr(key, "airjump.neu"))
        {
            ...
            if (rest.len > 0)
                c->airjump_y = -parse_float(rest);
        }
        else if (str8_ieq_cstr(key, "airjump.back"))
            c->airjump_back_x = parse_float(first);
        else if (str8_ieq_cstr(key, "airjump.fwd"))
            c->airjump_fwd_x = parse_float(first);
        break;
    }
    case 4:
    {
        if (str8_ieq_cstr(key, "airjump.num"))
            c->airjump_num = parse_int(val);
        else if (str8_ieq_cstr(key, "airjump.height"))
            c->airjump_height = parse_int(val);
        else if (str8_ieq_cstr(key, "yaccel"))
            c->yaccel = parse_float(val);
        else if (str8_ieq_cstr(key, "stand.friction"))
            c->stand_friction = parse_float(val);
        else if (str8_ieq_cstr(key, "crouch.friction"))
            c->crouch_friction = parse_float(val);
        else if (str8_ieq_cstr(key, "stand.friction.threshold"))
            c->stand_friction_threshold = parse_float(val);
        else if (str8_ieq_cstr(key, "crouch.friction.threshold"))
            c->crouch_friction_threshold = parse_float(val);
        else if (str8_ieq_cstr(key, "down.bounce.offset"))
        {
            ...
            if (brest.len > 0)
                c->down_bounce_offset_y = parse_float(brest);
        }
        else if (str8_ieq_cstr(key, "down.bounce.yaccel"))
            c->down_bounce_yaccel = parse_float(val);
        else if (str8_ieq_cstr(key, "down.bounce.groundlevel"))
            c->down_bounce_groundlevel = parse_float(val);
        else if (str8_ieq_cstr(key, "down.friction.threshold"))
            c->down_friction_threshold = parse_float(val);
        break;
    }
    }
}
    ...
    *out = (Mugen_Cns){ 0 };
    ...
    while (lp.pos < (usize)lp.data.len)
    {
        ...
        if (line.data[0] == '[')
        {
            if (in_controller)
            {
                finalize_controller(&current_ctrl, &triggerall_list, &trigger_exprs, trigger_group_sizes, max_trigger_group, alloc);
                ctrl_push(&controllers, current_ctrl, alloc);
                in_controller = false;
            }

            header_section = 0;
            if (starts_with_i(line, "[data]"))
            {
                header_section = 1;
                out->has_constants = true;
                continue;
            }
            else if (starts_with_i(line, "[size]"))
            {
                header_section = 2;
                out->has_constants = true;
                continue;
            }
            else if (starts_with_i(line, "[velocity]"))
            {
                header_section = 3;
                out->has_constants = true;
                continue;
            }
            else if (starts_with_i(line, "[movement]"))
            {
                header_section = 4;
                out->has_constants = true;
                continue;
        ...
        if (!in_controller)
        {
            if (str8_ieq_cstr(key, "type"))
                current_def.statetype = parse_statetype_char(val);
            else if (str8_ieq_cstr(key, "movetype"))
                current_def.movetype = parse_movetype_char(val);
            else if (str8_ieq_cstr(key, "physics"))
                current_def.physics = parse_statetype_char(val);
            else if (str8_ieq_cstr(key, "anim"))
                current_def.anim = parse_int(val);
            else if (str8_ieq_cstr(key, "velset"))
            {
                ...
                if (rest.len > 0)
                    current_def.velset_y = parse_float(rest);
                current_def.has_velset = true;
            }
            else if (str8_ieq_cstr(key, "ctrl"))
                current_def.ctrl = parse_int(val);
            else if (str8_ieq_cstr(key, "juggle"))
                current_def.juggle = parse_int(val);
            else if (str8_ieq_cstr(key, "poweradd"))
                current_def.poweradd = parse_int(val);
            else if (str8_ieq_cstr(key, "sprpriority"))
                current_def.sprpriority = parse_int(val);
            else if (str8_ieq_cstr(key, "hitdefpersist"))
                current_def.hitdefpersist = parse_int(val) != 0;
            else if (str8_ieq_cstr(key, "movehitpersist"))
                current_def.movehitpersist = parse_int(val) != 0;
            else if (str8_ieq_cstr(key, "hitcountpersist"))
                current_def.hitcountpersist = parse_int(val) != 0;
        }
        else
        {
            if (str8_ieq_cstr(key, "type"))
            ...
            else
            {
                ...
                else if (trig_level > 0)
                {
                    ...
                    if ((u32)trig_level < 64)
                    {
                        ...
                        if ((u32)trig_level > max_trigger_group)
                            max_trigger_group = (u32)trig_level;
                    }
                }
                else
                {
