...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Helper_Params));
    Helper_Params* p = sc->params;
    if (str8_ieq_cstr(key, "helperid") || str8_ieq_cstr(key, "id"))
        p->id = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "stateno"))
        p->stateno = mugen_expr_parse(val, alloc);
    ...
    else if (str8_ieq_cstr(key, "postype"))
    {
        ...
        if (str8_ieq_cstr(v, "p1"))
            p->postype = MUGEN_POSTYPE_P1;
        else if (str8_ieq_cstr(v, "left"))
            p->postype = MUGEN_POSTYPE_LEFT;
        else if (str8_ieq_cstr(v, "right"))
            p->postype = MUGEN_POSTYPE_RIGHT;
        else if (str8_ieq_cstr(v, "back"))
            p->postype = MUGEN_POSTYPE_BACK;
        else if (str8_ieq_cstr(v, "front"))
            p->postype = MUGEN_POSTYPE_FRONT;
    }
    else if (str8_ieq_cstr(key, "facing"))
        p->facing = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "ownpal"))
        p->ownpal = mugen_expr_parse(val, alloc);
}
...
__attribute__((constructor)) static void register_helper(void)
{
