...
static bool parse_file_header(const u8* data, usize size, Sff_File_Header* out)
{
    if (size < 36)
        return false;
    if (memcmp(data, "ElecbyteSpr\0", 12) != 0)
        return false;
...
{
    if (block_end < 769 || block_end > file_size)
        return -1;
...
static void pack_atlas(Decoded_Sprite* sprites, u32 count, u32* out_width, u32* out_height, Pack_Pos* positions)
{
    u32 max_w = 2048;
    u32 x = 0;
    u32 y = 0;
...
static void blit_indexed_to_atlas(u8* atlas, u32 atlas_w, const u8* indexed_pixels, u16 sprite_w, u16 sprite_h, u32 dst_x, u32 dst_y, const u32 palette[256])
{
...
static void blit_rgba_to_atlas(u8* atlas, u32 atlas_w, const u8* rgba_pixels, u16 sprite_w, u16 sprite_h, u32 dst_x, u32 dst_y)
{
    for (u32 row = 0; row < sprite_h; row++)
    {
...
static bool load_v1(Mugen_Sff* sff, const u8* data, usize file_size, Sff_File_Header* fh, const Mel_Alloc* alloc)
{
    ...
    for (u32 i = 0; i < fh->sprite_count; i++)
    {
        ...
        u32 block_end = (sh.next_header_offset > header_offset) ? sh.next_header_offset : pcx_offset + sh.data_size;
        ...
        {
            i64 pal_marker = find_palette_marker(data, file_size, pcx_data_start, block_end);
            rle_size = (pal_marker >= 0) ? (usize)(pal_marker - pcx_data_start) : (usize)(block_end - pcx_data_start);
        }
    ...
    {
        mel_sprite_sheet_push_frame(&sff->sheet, mel_rect((f32)positions[i].x * inv_w, (f32)positions[i].y * inv_h, (f32)decoded[i].width * inv_w, (f32)decoded[i].height * inv_h));
    }
    ...
    for (u32 i = 0; i < valid_count; i++)
    {
        blit_indexed_to_atlas(sff->atlas_pixels, sff->atlas_width, decoded[i].pixels, decoded[i].width, decoded[i].height, positions[i].x, positions[i].y, sprite_palettes[i]);
    }
...
static bool load_v2(Mugen_Sff* sff, const u8* data, usize file_size, Sff_File_Header* fh, const Mel_Alloc* alloc)
{
    ...
    for (u32 i = 0; i < valid_count; i++)
    {
        if (is_rgba[i])
        {
            blit_rgba_to_atlas(sff->atlas_pixels, sff->atlas_width, decoded[i].pixels, decoded[i].width, decoded[i].height, positions[i].x, positions[i].y);
        }
        else
        {
            u16  pal_idx = pal_indices[i];
            u32* pal = (pal_idx < MAX_PALETTES && palettes[pal_idx].loaded) ? palettes[pal_idx].colors : palettes[0].colors;
            blit_indexed_to_atlas(sff->atlas_pixels, sff->atlas_width, decoded[i].pixels, decoded[i].width, decoded[i].height, positions[i].x, positions[i].y, pal);
        }
    }
