...
static str8 def_before_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return def_trim(str8_from_parts(line.data, i));
...
static str8 def_after_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return def_trim(str8_from_parts(line.data + i + 1, line.len - i - 1));
    return (str8){ 0 };
}
...
static bool mugen_def_parse(Mugen_Def* def, str8 data)
{
    *def = (Mugen_Def){ 0 };
    ...
    while (pos < (usize)data.len)
    {
        ...
        if (def_ieq(key, "cmd"))
            def->cmd = val;
        else if (def_ieq(key, "cns"))
            def->cns = val;
        else if (def_ieq(key, "sprite"))
            def->sprite = val;
        else if (def_ieq(key, "anim"))
            def->anim = val;
        else if (def_ieq(key, "stcommon"))
            def->stcommon = val;
        else if (def_ieq(key, "st") || def_starts_with_i(key, "st"))
        {
            if (def_ieq(key, "stcommon") || def_ieq(key, "sprite"))
                continue;

            bool is_st = def_ieq(key, "st");
            bool is_stN = !is_st && key.len >= 3 && key.data[0] == 's' && key.data[1] == 't' && key.data[2] >= '0' && key.data[2] <= '9';
            if (!is_st && !is_stN)
                continue;
...
{
    if (char_dir.len + 1 + filename.len >= buf_size)
        return (str8){ 0 };
