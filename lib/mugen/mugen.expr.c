...
static bool at_end(Expr_Parser* p) { return p->pos >= (usize)p->text.len; }
...
static bool ident_char(u8 c) { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_' || (c >= '0' && c <= '9') || c == '.'; }
...
static u8 lookup_query(str8 name) { return mugen_query_lookup_name(name); }
...
static const Var_Entry s_vars[] = { { "var", MUGEN_VAR_INT }, { "fvar", MUGEN_VAR_FLOAT }, { "sysvar", MUGEN_VAR_SYSINT }, { "sysfvar", MUGEN_VAR_SYSFLOAT }, { NULL, 0 } };
...
static Mugen_Expr* parse_add(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos++;
            lhs = make_binary(p, MUGEN_OP_ADD, lhs, parse_mul(p));
        }
        else if (c == '-')
        {
            p->pos++;
            lhs = make_binary(p, MUGEN_OP_SUB, lhs, parse_mul(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_cmp(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        if (c == '<')
        {
            ...
            {
                p->pos++;
                lhs = make_binary(p, MUGEN_OP_LE, lhs, parse_add(p));
            }
            else
                lhs = make_binary(p, MUGEN_OP_LT, lhs, parse_add(p));
        }
        else if (c == '>')
        {
            p->pos++;
            ...
            {
                p->pos++;
                lhs = make_binary(p, MUGEN_OP_GE, lhs, parse_add(p));
            }
            else
                lhs = make_binary(p, MUGEN_OP_GT, lhs, parse_add(p));
        }
        else
            break;
    }
...
{
    if (eq_node->type != MUGEN_EXPR_BINARY)
        return eq_node;
    if (eq_node->binary.op != MUGEN_OP_EQ)
        return eq_node;
    if (!eq_node->binary.lhs || eq_node->binary.lhs->type != MUGEN_EXPR_QUERY)
        return eq_node;
    if (eq_node->binary.lhs->query.id != MUGEN_QUERY_ANIMELEM)
        return eq_node;
...
static Mugen_Expr* parse_eq(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        if (c == '=' && (p->pos + 1 >= (usize)p->text.len || p->text.data[p->pos + 1] != '='))
        {
            ...
            bool is_hitdefattr = (lhs && lhs->type == MUGEN_EXPR_QUERY && lhs->query.id == MUGEN_QUERY_HITDEFATTR);
            if (is_hitdefattr)
            {
...
static Mugen_Expr* parse_band(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos++;
            lhs = make_binary(p, MUGEN_OP_BAND, lhs, parse_eq(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_bxor(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos++;
            lhs = make_binary(p, MUGEN_OP_BXOR, lhs, parse_band(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_bor(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos++;
            lhs = make_binary(p, MUGEN_OP_BOR, lhs, parse_bxor(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_and(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos += 2;
            lhs = make_binary(p, MUGEN_OP_AND, lhs, parse_bor(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_xor(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos += 2;
            lhs = make_binary(p, MUGEN_OP_XOR, lhs, parse_and(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_or(Expr_Parser* p)
{
    ...
    for (;;)
    {
        ...
        {
            p->pos += 2;
            lhs = make_binary(p, MUGEN_OP_OR, lhs, parse_xor(p));
        }
        else
            break;
    }
...
static Mugen_Expr* parse_expr(Expr_Parser* p) { return parse_or(p); }
...
f32 mugen_expr_eval(Mugen_Expr* expr, Mugen_Char_State* state)
{
    ...
    switch (expr->type)
    {
    case MUGEN_EXPR_LIT_INT:
        return (f32)expr->lit_int;
    case MUGEN_EXPR_LIT_FLOAT:
        return expr->lit_float;
    case MUGEN_EXPR_LIT_STRING:
        return 0.0f;
    ...
    case MUGEN_EXPR_BINARY:
    {
        ...
        {
            bool is_command = (expr->binary.lhs && expr->binary.lhs->type == MUGEN_EXPR_QUERY && expr->binary.lhs->query.id == MUGEN_QUERY_COMMAND);
            ...
            bool is_statetype = (expr->binary.lhs && expr->binary.lhs->type == MUGEN_EXPR_QUERY &&
                                 (expr->binary.lhs->query.id == MUGEN_QUERY_STATETYPE || expr->binary.lhs->query.id == MUGEN_QUERY_P2STATETYPE || expr->binary.lhs->query.id == MUGEN_QUERY_PHYSICS ||
            ...
            if (is_statetype && expr->binary.rhs && expr->binary.rhs->type == MUGEN_EXPR_LIT_INT)
            {
                ...
                if (val == 'S')
                    expected = MUGEN_PHYSICS_S;
                else if (val == 'C')
                    expected = MUGEN_PHYSICS_C;
                else if (val == 'A')
                    expected = MUGEN_PHYSICS_A;
                else if (val == 'L')
                    expected = MUGEN_PHYSICS_L;
                else if (val == 'N')
                    expected = MUGEN_PHYSICS_N;
                bool eq = (st == expected);
                return (expr->binary.op == MUGEN_OP_EQ) ? (eq ? 1.0f : 0.0f) : (eq ? 0.0f : 1.0f);
            }

            bool is_hitdefattr = (expr->binary.lhs && expr->binary.lhs->type == MUGEN_EXPR_QUERY && expr->binary.lhs->query.id == MUGEN_QUERY_HITDEFATTR);
            ...
            bool is_movetype = (expr->binary.lhs && expr->binary.lhs->type == MUGEN_EXPR_QUERY &&
                                (expr->binary.lhs->query.id == MUGEN_QUERY_MOVETYPE || expr->binary.lhs->query.id == MUGEN_QUERY_P2MOVETYPE || expr->binary.lhs->query.id == MUGEN_QUERY_PREVMOVETYPE));
            if (is_movetype && expr->binary.rhs && expr->binary.rhs->type == MUGEN_EXPR_LIT_INT)
            {
                ...
                i32 val = expr->binary.rhs->lit_int;
                if (val == 'I')
                    expected = MUGEN_MOVETYPE_I;
                else if (val == 'A')
                    expected = MUGEN_MOVETYPE_A;
                else if (val == 'H')
                    expected = MUGEN_MOVETYPE_H;
                bool eq = (mt == expected);
                return (expr->binary.op == MUGEN_OP_EQ) ? (eq ? 1.0f : 0.0f) : (eq ? 0.0f : 1.0f);
            }
        }
        ...
        case MUGEN_OP_GE:
            return l >= r ? 1.0f : 0.0f;
        case MUGEN_OP_AND:
            return (l != 0.0f && r != 0.0f) ? 1.0f : 0.0f;
        case MUGEN_OP_OR:
            return (l != 0.0f || r != 0.0f) ? 1.0f : 0.0f;
        case MUGEN_OP_XOR:
            return ((l != 0.0f) != (r != 0.0f)) ? 1.0f : 0.0f;
        case MUGEN_OP_BAND:
            return (f32)((i32)l & (i32)r);
        case MUGEN_OP_BOR:
            return (f32)((i32)l | (i32)r);
        case MUGEN_OP_BXOR:
            return (f32)((i32)l ^ (i32)r);
    ...
    case MUGEN_EXPR_FUNC:
    {
        ...
        case MUGEN_FUNC_COND:
            ...
            {
                f32 cond = mugen_expr_eval(expr->func.args[0], state);
                return cond != 0.0f ? mugen_expr_eval(expr->func.args[1], state) : mugen_expr_eval(expr->func.args[2], state);
            }
            return 0.0f;
        }
        return 0.0f;
    ...
    case MUGEN_EXPR_ASSIGN:
    {
        ...
        case MUGEN_VAR_SYSINT:
            if (idx >= 0 && idx < 5)
                state->sysvar[idx] = (i32)val;
            break;
        case MUGEN_VAR_SYSFLOAT:
            if (idx >= 0 && idx < 5)
                state->sysfvar[idx] = val;
            break;
        }
        return val;
    }
...
bool mugen_expr_eval_bool(Mugen_Expr* expr, Mugen_Char_State* state) { return mugen_expr_eval(expr, state) != 0.0f; }
