...
static str8 stage_before_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return stage_trim(str8_from_parts(line.data, i));
...
static str8 stage_after_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return stage_trim(str8_from_parts(line.data + i + 1, line.len - i - 1));
    return (str8){ 0 };
}
...
{
    if (stage_ieq(val, "add"))
        return MUGEN_TRANS_ADD;
    if (stage_ieq(val, "add1"))
        return MUGEN_TRANS_ADD1;
    if (stage_ieq(val, "addalpha"))
        return MUGEN_TRANS_ADDALPHA;
    if (stage_ieq(val, "sub"))
        return MUGEN_TRANS_SUB;
    return MUGEN_TRANS_NONE;
}

static u8 stage_parse_bg_type(str8 val)
{
    if (stage_ieq(val, "parallax"))
        return MUGEN_BG_PARALLAX;
    ...
    *out = (Mugen_Stage){
    ...
    while (pos < (usize)data.len)
    {
        ...
        case SECTION_PLAYERINFO:
            if (stage_ieq(key, "p1startx"))
                out->p1startx = stage_parse_float(val);
            else if (stage_ieq(key, "p2startx"))
                out->p2startx = stage_parse_float(val);
            else if (stage_ieq(key, "p1facing"))
                out->p1facing = (i8)stage_parse_int(val);
            else if (stage_ieq(key, "p2facing"))
                out->p2facing = (i8)stage_parse_int(val);
            break;

        case SECTION_BOUND:
            if (stage_ieq(key, "screenleft"))
                out->screenleft = stage_parse_float(val);
            else if (stage_ieq(key, "screenright"))
                out->screenright = stage_parse_float(val);
            break;

        case SECTION_CAMERA:
            if (stage_ieq(key, "boundleft"))
                out->left_bound = stage_parse_float(val);
            else if (stage_ieq(key, "boundright"))
                out->right_bound = stage_parse_float(val);
            else if (stage_ieq(key, "boundhigh"))
                out->top_bound = stage_parse_float(val);
            else if (stage_ieq(key, "boundlow"))
                out->bottom_bound = stage_parse_float(val);
            else if (stage_ieq(key, "startx"))
                out->camera_startx = stage_parse_float(val);
            else if (stage_ieq(key, "starty"))
                out->camera_starty = stage_parse_float(val);
            else if (stage_ieq(key, "tension"))
                out->tension = stage_parse_float(val);
            else if (stage_ieq(key, "verticalfollow"))
                out->verticalfollow = stage_parse_float(val);
            else if (stage_ieq(key, "floortension"))
                out->floortension = stage_parse_float(val);
            break;

        case SECTION_STAGEINFO:
            if (stage_ieq(key, "zoffset"))
                out->zoffset = stage_parse_float(val);
        ...
        case SECTION_BG:
            if (stage_ieq(key, "type"))
                cur_bg.type = stage_parse_bg_type(val);
            else if (stage_ieq(key, "layerno"))
                cur_bg.layerno = (u8)stage_parse_int(val);
            else if (stage_ieq(key, "trans"))
                cur_bg.trans = stage_parse_trans(val);
            else if (stage_ieq(key, "mask"))
                cur_bg.mask = stage_parse_int(val) != 0;
            else if (stage_ieq(key, "spriteno"))
                stage_parse_pair_u16(val, &cur_bg.sprite_group, &cur_bg.sprite_number);
            else if (stage_ieq(key, "start"))
                stage_parse_pair_f32(val, &cur_bg.start_x, &cur_bg.start_y);
            else if (stage_ieq(key, "delta"))
                stage_parse_pair_f32(val, &cur_bg.delta_x, &cur_bg.delta_y);
            else if (stage_ieq(key, "velocity"))
                stage_parse_pair_f32(val, &cur_bg.velocity_x, &cur_bg.velocity_y);
            ...
            else if (stage_ieq(key, "alpha"))
                stage_parse_pair_u16(val, &cur_bg.alpha_src, &cur_bg.alpha_dst);
            else if (stage_ieq(key, "xscale"))
                stage_parse_pair_f32(val, &cur_bg.xscale_top, &cur_bg.xscale_bot);
            else if (stage_ieq(key, "yscalestart"))
                cur_bg.yscalestart = stage_parse_float(val);
            else if (stage_ieq(key, "yscaledelta"))
                cur_bg.yscaledelta = stage_parse_float(val);
            break;
        }
    }
