...
void mugen_targets_clear(Mugen_Char_State* state) { state->target_count = 0; }
...
void mugen_cns_tick(Mugen_Cns* cns, Mugen_Char_State* state)
{
    ...
    else if (!state->state_changed)
    {
        ...
        if (state->movehit > 0)
            state->movehit++;
        if (state->moveguarded > 0)
            state->moveguarded++;

        if (state->ghv.hitshaketime > 0)
            state->ghv.hitshaketime--;
        if (state->ghv.hittime > 0)
            state->ghv.hittime--;
        ...
        if (state->envshake_time > 0)
            state->envshake_time--;
        if (state->palfx_time > 0)
            state->palfx_time--;
