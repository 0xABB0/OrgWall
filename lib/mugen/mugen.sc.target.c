...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(TargetBind_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(TargetState_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(TargetLifeAdd_Params));
    TargetLifeAdd_Params* p = sc->params;
    if (str8_ieq_cstr(key, "value"))
        p->value = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "kill"))
        p->kill = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "absolute"))
        p->absolute = mugen_expr_parse(val, alloc);
}

static void targetfacing_parse(Mugen_State_Controller* sc, str8 key, str8 val, const Mel_Alloc* alloc)
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(TargetFacing_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(TargetPowerAdd_Params));
...
{
    TargetState_Params* p = sc->params;
    if (!p || !p->value || state->target_count == 0)
        return;
...
__attribute__((constructor)) static void register_target(void)
{
