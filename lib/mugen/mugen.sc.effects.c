...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(EnvShake_Params));
    EnvShake_Params* p = sc->params;
    if (str8_ieq_cstr(key, "time"))
        p->time = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "ampl"))
        p->ampl = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "freq"))
        p->freq = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "phase"))
        p->phase = mugen_expr_parse(val, alloc);
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(ScreenBound_Params));
    ...
    else if (str8_ieq_cstr(key, "movecamera"))
    {
        ...
        if (rest.len > 0)
            p->movecamera_y = mugen_expr_parse(rest, alloc);
    }
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(PalFX_Params));
    ...
    else if (str8_ieq_cstr(key, "add"))
    {
        ...
        for (i32 i = 0; i < 3; i++)
        {
            ...
            if (part.len > 0)
                p->add[i] = mugen_expr_parse(part, alloc);
        }
    }
    else if (str8_ieq_cstr(key, "mul"))
    {
        ...
        for (i32 i = 0; i < 3; i++)
        {
            str8 part = mcns_split_comma_first(rest, &rest);
            if (part.len > 0)
                p->mul[i] = mugen_expr_parse(part, alloc);
        }
    }
    else if (str8_ieq_cstr(key, "sinadd"))
    {
        ...
        for (i32 i = 0; i < 3; i++)
        {
            str8 part = mcns_split_comma_first(rest, &rest);
            if (part.len > 0)
                p->sinadd[i] = mugen_expr_parse(part, alloc);
        }
        if (rest.len > 0)
            p->sinadd_period = mugen_expr_parse(rest, alloc);
    }
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Pause_Params));
    Pause_Params* p = sc->params;
    if (str8_ieq_cstr(key, "time"))
        p->time = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "endcmdbuftime"))
        p->endcmdbuftime = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "pausebg"))
        p->pausebg = mugen_expr_parse(val, alloc);
}
...
__attribute__((constructor)) static void register_effects(void)
{
