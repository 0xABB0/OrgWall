...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(ChangeAnim_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Ctrl_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(StateTypeSet_Params));
    ...
    {
        if (c == 'S')
            p->statetype = MUGEN_PHYSICS_S;
        else if (c == 'C')
            p->statetype = MUGEN_PHYSICS_C;
        else if (c == 'A')
            p->statetype = MUGEN_PHYSICS_A;
    }
    else if (str8_ieq_cstr(key, "movetype"))
    {
        if (c == 'I')
            p->movetype = MUGEN_MOVETYPE_I;
        else if (c == 'A')
            p->movetype = MUGEN_MOVETYPE_A;
        else if (c == 'H')
            p->movetype = MUGEN_MOVETYPE_H;
    }
    else if (str8_ieq_cstr(key, "physics"))
    {
        if (c == 'S')
            p->physics = MUGEN_PHYSICS_S;
        else if (c == 'C')
            p->physics = MUGEN_PHYSICS_C;
        else if (c == 'A')
            p->physics = MUGEN_PHYSICS_A;
        else if (c == 'N')
            p->physics = MUGEN_PHYSICS_N;
    }
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(SelfState_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(ChangeAnim2_Params));
...
static void changestate_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (p->ctrl)
        state->pending_ctrl = (i32)mugen_expr_eval(p->ctrl, state);
    if (p->anim)
        state->pending_anim = (i32)mugen_expr_eval(p->anim, state);
...
static void statetypeset_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (!p)
        return;
    if (p->statetype >= 0)
        state->statetype = (u8)p->statetype;
    if (p->movetype >= 0)
        state->movetype = (u8)p->movetype;
    if (p->physics >= 0)
        state->physics = (u8)p->physics;
}
...
__attribute__((constructor)) static void register_state(void)
{
