...
void mugen_roster_init_opt(Mugen_Roster* r, Mugen_Roster_Init_Opt opt)
{
    ...
    mel_slotmap_init(&r->entries, opt.alloc, .item_size = sizeof(Mugen_Roster_Entry));
}
...
static bool roster_scan_char_dir(str8 path, const Mel_Vfs_Stat* stat, void* user)
{
    ...
    if (!mel_vfs_exists(def_path))
        return true;

    Mugen_Char ch;
    bool       ok = mugen_char_load(&ch, .def_path = def_path, .stcommon_path = ctx->roster->stcommon_path, .alloc = ctx->roster->alloc);
...
u32 mugen_roster_count(Mugen_Roster* r) { return mel_slotmap_count(&r->entries); }
