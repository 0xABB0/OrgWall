...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(VarSet_Params));
    VarSet_Params* p = sc->params;
    if (str8_ieq_cstr(key, "value"))
        p->value = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "v"))
    {
        p->var_type = MUGEN_VAR_INT;
        p->index = mugen_expr_parse(val, alloc);
    }
    else if (str8_ieq_cstr(key, "fv"))
    {
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(VarRandom_Params));
    VarRandom_Params* p = sc->params;
    if (str8_ieq_cstr(key, "v"))
    {
        p->var_type = MUGEN_VAR_INT;
        p->index = mugen_expr_parse(val, alloc);
    }
    else if (str8_ieq_cstr(key, "range"))
    {
        ...
        if (rest.len > 0)
            p->range_high = mugen_expr_parse(rest, alloc);
    }
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(VarRangeSet_Params));
    VarRangeSet_Params* p = sc->params;
    if (str8_ieq_cstr(key, "value"))
        p->value = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "fvalue"))
        p->fvalue = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "first"))
        p->first = mugen_expr_parse(val, alloc);
    else if (str8_ieq_cstr(key, "last"))
        p->last = mugen_expr_parse(val, alloc);
}
...
__attribute__((constructor)) static void register_var(void)
{
