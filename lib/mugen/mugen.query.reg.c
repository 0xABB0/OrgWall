...
{
    if (eval)
        query_table[id].eval = eval;
    if (!query_table[id].name && name)
        query_table[id].name = name;
...
{
    if (id >= MUGEN_QUERY_MAX)
        return NULL;
    return &query_table[id];
}

u32 mugen_query_name_count(void) { return name_count; }

const Mugen_Query_Name_Entry* mugen_query_name_table(void) { return name_table; }
