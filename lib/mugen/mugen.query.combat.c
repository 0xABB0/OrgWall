...
static f32 eval_gethitvar(Mugen_Expr* arg, Mugen_Char_State* state)
{
    ...
    if (str8_ieq_cstr(name, "animtype"))
        return (f32)ghv->animtype;
    if (str8_ieq_cstr(name, "air.animtype"))
        return (f32)ghv->air_animtype;
    if (str8_ieq_cstr(name, "ground.animtype"))
        return (f32)ghv->ground_animtype;
    if (str8_ieq_cstr(name, "groundtype"))
        return (f32)ghv->groundtype;
    if (str8_ieq_cstr(name, "airtype"))
        return (f32)ghv->airtype;
    if (str8_ieq_cstr(name, "damage"))
        return (f32)ghv->damage;
    if (str8_ieq_cstr(name, "hitcount"))
        return (f32)ghv->hitcount;
    if (str8_ieq_cstr(name, "guardcount"))
        return (f32)ghv->guardcount;
    if (str8_ieq_cstr(name, "hitshaketime"))
        return (f32)ghv->hitshaketime;
    if (str8_ieq_cstr(name, "hittime"))
        return (f32)ghv->hittime;
    if (str8_ieq_cstr(name, "slidetime"))
        return (f32)ghv->slidetime;
    if (str8_ieq_cstr(name, "ctrltime"))
        return (f32)ghv->ctrltime;
    if (str8_ieq_cstr(name, "xvel"))
        return ghv->xvel;
    if (str8_ieq_cstr(name, "yvel"))
        return ghv->yvel;
    if (str8_ieq_cstr(name, "xaccel"))
        return ghv->xaccel;
    if (str8_ieq_cstr(name, "yaccel"))
        return ghv->yaccel;
    if (str8_ieq_cstr(name, "xoff"))
        return ghv->xoff;
    if (str8_ieq_cstr(name, "yoff"))
        return ghv->yoff;
    if (str8_ieq_cstr(name, "isbound"))
        return ghv->isbound ? 1.0f : 0.0f;
    if (str8_ieq_cstr(name, "guarded"))
        return ghv->guarded ? 1.0f : 0.0f;
    if (str8_ieq_cstr(name, "fall"))
        return ghv->fallflag ? 1.0f : 0.0f;
    if (str8_ieq_cstr(name, "fall.recover"))
        return ghv->fall_recover ? 1.0f : 0.0f;
    if (str8_ieq_cstr(name, "fall.recovertime"))
        return (f32)ghv->fall_recovertime;
    if (str8_ieq_cstr(name, "fall.xvel"))
        return ghv->fall_xvel;
    if (str8_ieq_cstr(name, "fall.yvel"))
        return ghv->fall_yvel;
    if (str8_ieq_cstr(name, "fall.damage"))
        return (f32)ghv->fall_damage;
    if (str8_ieq_cstr(name, "fall.kill"))
        return ghv->fall_kill ? 1.0f : 0.0f;
    if (str8_ieq_cstr(name, "attr"))
        return (f32)ghv->attr;
    if (str8_ieq_cstr(name, "priority"))
        return (f32)ghv->priority;
    if (str8_ieq_cstr(name, "forcestand"))
        return ghv->forcestand ? 1.0f : 0.0f;
    return 0.0f;
}

__attribute__((constructor)) static void register_combat_queries(void)
{
