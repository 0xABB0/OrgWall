...
static bool mugen__parse_clsn_box(str8 line, Mugen_Clsn_Box* out)
{
    ...
    if (!mugen__parse_i16(sx1, &out->x1))
        return false;
    if (!mugen__parse_i16(sy1, &out->y1))
        return false;
    if (!mugen__parse_i16(sx2, &out->x2))
        return false;
    if (!mugen__parse_i16(sy2, &out->y2))
        return false;
...
static bool mugen__parse_frame_line(str8 line, Mugen_Air_Frame* out)
{
    ...
    if (!mugen__parse_i32(sgroup, &group))
        return false;
    if (!mugen__parse_i32(snumber, &number))
        return false;
    if (!mugen__parse_i32(sxoff, &xoff))
        return false;
    if (!mugen__parse_i32(syoff, &yoff))
        return false;
    if (!mugen__parse_i32(stime, &time_val))
        return false;
    ...
    if (rem.len > 0)
    {
        ...
        {
            if (flags.data[i] == 'H' || flags.data[i] == 'h')
                out->flip_h = true;
            if (flags.data[i] == 'V' || flags.data[i] == 'v')
                out->flip_v = true;
        }
    }
...
static bool mugen__is_digit_or_minus(u8 c) { return (c >= '0' && c <= '9') || c == '-'; }
    ...
    *out = (Mugen_Air){ 0 };
    ...
    while (remaining.len > 0)
    {
        ...
        if (line.len > 0 && mugen__is_digit_or_minus(line.data[0]))
        {
            ...
            if (!mugen__parse_frame_line(line, &frame))
                continue;
    ...
    if (pending_clsn1.boxes)
        mel_dealloc(alloc, pending_clsn1.boxes);
    if (pending_clsn2.boxes)
        mel_dealloc(alloc, pending_clsn2.boxes);
    if (default_clsn1.boxes)
        mel_dealloc(alloc, default_clsn1.boxes);
    if (default_clsn2.boxes)
        mel_dealloc(alloc, default_clsn2.boxes);
...
void mugen_air_shutdown(Mugen_Air* air, const Mel_Alloc* alloc)
{
    ...
    for (u32 a = 0; a < air->action_count; a++)
    {
        ...
        {
            if (act->frames[f].clsn1)
                mel_dealloc(alloc, act->frames[f].clsn1);
            if (act->frames[f].clsn2)
                mel_dealloc(alloc, act->frames[f].clsn2);
        }
        mel_dealloc(alloc, act->frames);
    }
    if (air->actions)
        mel_dealloc(alloc, air->actions);
    *air = (Mugen_Air){ 0 };
}
