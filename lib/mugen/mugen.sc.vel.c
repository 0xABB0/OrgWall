...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Vel_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Pos_Params));
...
static void velset_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (!p)
        return;
    if (p->x)
        state->vel_x = mugen_expr_eval(p->x, state);
    if (p->y)
        state->vel_y = mugen_expr_eval(p->y, state);
}
...
static void posset_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (!p)
        return;
    if (p->x)
        state->pos_x = mugen_expr_eval(p->x, state);
    if (p->y)
        state->pos_y = mugen_expr_eval(p->y, state);
}
...
static void posadd_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (!p)
        return;
    if (p->x)
        state->pos_x += mugen_expr_eval(p->x, state) * state->facing;
    if (p->y)
        state->pos_y += mugen_expr_eval(p->y, state);
}
...
__attribute__((constructor)) static void register_vel(void)
{
