...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(NotHitBy_Params));
    NotHitBy_Params* p = sc->params;
    if (str8_ieq_cstr(key, "value"))
        p->attr_flags = mcns_parse_attr(val);
    else if (str8_ieq_cstr(key, "time"))
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(Width_Params));
    ...
    if (str8_ieq_cstr(key, "value"))
    {
        ...
        if (rest.len > 0)
            p->back = mugen_expr_parse(rest, alloc);
    }
}
...
static void assertspecial_parse(Mugen_State_Controller* sc, str8 key, str8 val, const Mel_Alloc* alloc)
{
    (void)alloc;
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(AssertSpecial_Params));
    AssertSpecial_Params* p = sc->params;
    if (str8_ieq_cstr(key, "flag") || str8_ieq_cstr(key, "flag2") || str8_ieq_cstr(key, "flag3"))
    {
        if (str8_ieq_cstr(val, "nowalk"))
            p->flags |= MUGEN_ASSERT_NOWALK;
        else if (str8_ieq_cstr(val, "noautoturn"))
            p->flags |= MUGEN_ASSERT_NOAUTOTURN;
        else if (str8_ieq_cstr(val, "nostandguard"))
            p->flags |= MUGEN_ASSERT_NOSTANDGUARD;
        else if (str8_ieq_cstr(val, "nocrouchguard"))
            p->flags |= MUGEN_ASSERT_NOCROUCHGUARD;
        else if (str8_ieq_cstr(val, "noairguard"))
            p->flags |= MUGEN_ASSERT_NOAIRGUARD;
        else if (str8_ieq_cstr(val, "unguardable"))
            p->flags |= MUGEN_ASSERT_UNGUARDABLE;
        else if (str8_ieq_cstr(val, "nojugglecheck"))
            p->flags |= MUGEN_ASSERT_NOJUGGLECHECK;
        else if (str8_ieq_cstr(val, "intro"))
            p->flags |= MUGEN_ASSERT_INTRO;
        else if (str8_ieq_cstr(val, "nocornerpush"))
            p->flags |= MUGEN_ASSERT_NOCORNERPUSH;
    }
}
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(DefenceMulSet_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(AttackMulSet_Params));
...
{
    if (!sc->params)
        sc->params = mcns_alloc_params(alloc, sizeof(HitBy_Params));
...
static void width_exec(Mugen_State_Controller* sc, Mugen_Char_State* state)
{
    ...
    if (!p)
        return;
    if (p->front)
        state->ground_front = mugen_expr_eval(p->front, state);
    if (p->back)
        state->ground_back = mugen_expr_eval(p->back, state);
}
...
__attribute__((constructor)) static void register_combat(void)
{
