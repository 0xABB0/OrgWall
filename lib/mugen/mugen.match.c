...
static void init_fighter(Mugen_Match* m, Fighter* f, Mugen_Char* ch, f32 start_x, bool facing_right)
{
    fighter_init(f, m->alloc, .start_x = start_x, .facing_right = facing_right, .air = &ch->air);
...
Mugen_Match* mugen_match_create_opt(Mugen_Match_Create_Opt opt)
{
    ...
    mel_sim_init(&m->sim, .event_buffer = m->event_buf, .event_buffer_size = sizeof(m->event_buf), .time_scale = 0.0f, .alloc = opt.alloc);
    ...
    round_init(&m->round, .p1 = &m->p1, .p2 = &m->p2, .stage_left = stage_left, .stage_right = stage_right);
...
void mugen_match_start(Mugen_Match* m) { m->sim.time_scale = 1.0f; }

void mugen_match_pause(Mugen_Match* m) { m->sim.time_scale = 0.0f; }
...
void mugen_match_update(Mugen_Match* m, f32 frame_dt) { mel_sim_tick(&m->sim, frame_dt); }
