...
static str8 fightdef_before_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return fightdef_trim(str8_from_parts(line.data, i));
...
static str8 fightdef_after_eq(str8 line)
{
    for (size i = 0; i < line.len; i++)
        if (line.data[i] == '=')
            return fightdef_trim(str8_from_parts(line.data + i + 1, line.len - i - 1));
    return (str8){ 0 };
}
...
{
    if (fightdef_ieq(key, "bg0.anim"))
        bar->bg0_anim = (u32)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "bg0.facing"))
    { /* bg0 is anim, facing handled by anim */
    }
    else if (fightdef_ieq(key, "bg1.spr"))
        bar->bg1_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "bg1.facing"))
        bar->bg1_facing = (i8)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "mid.spr"))
        bar->mid_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "mid.facing"))
        bar->mid_facing = (i8)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "mid.offset"))
        fightdef_parse_pair_i32(val, &bar->mid_offset_x, &bar->mid_offset_y);
    else if (fightdef_ieq(key, "front.spr"))
        bar->front_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "front.facing"))
        bar->front_facing = (i8)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "front.offset"))
    { /* same struct could be extended */
    }
    else if (fightdef_ieq(key, "range.x"))
        fightdef_parse_pair_i32(val, &bar->range_x_start, &bar->range_x_end);
}
...
{
    if (fightdef_ieq(key, "match.wins"))
        r->match_wins = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "match.maxdrawgames"))
        r->match_maxdrawgames = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "start.waittime"))
        r->start_waittime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "pos"))
        fightdef_parse_pair_i32(val, &r->pos_x, &r->pos_y);

    else if (fightdef_ieq(key, "round.time"))
        r->round_time = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "round.default.offset"))
        fightdef_parse_pair_i32(val, &r->round_default_offset_x, &r->round_default_offset_y);
    else if (fightdef_ieq(key, "round.default.font"))
        r->round_default_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "round.default.text"))
    {
        if (alloc)
            r->round_default_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "round.default.displaytime"))
        r->round_default_displaytime = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "round1.snd"))
        r->round_snd[0] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round2.snd"))
        r->round_snd[1] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round3.snd"))
        r->round_snd[2] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round4.snd"))
        r->round_snd[3] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round5.snd"))
        r->round_snd[4] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round6.snd"))
        r->round_snd[5] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round7.snd"))
        r->round_snd[6] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round8.snd"))
        r->round_snd[7] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round9.snd"))
        r->round_snd[8] = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "round.sndtime"))
        r->round_sndtime = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "fight.time"))
        r->fight_time = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "fight.offset"))
        fightdef_parse_pair_i32(val, &r->fight_offset_x, &r->fight_offset_y);
    else if (fightdef_ieq(key, "fight.anim"))
        r->fight_anim = (u32)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "fight.font"))
        r->fight_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "fight.text"))
    {
        if (alloc)
            r->fight_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "fight.displaytime"))
        r->fight_displaytime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "fight.snd"))
        r->fight_snd = fightdef_parse_snd_ref(val);
    else if (fightdef_ieq(key, "fight.sndtime"))
        r->fight_sndtime = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "ctrl.time"))
        r->ctrl_time = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "ko.time"))
        r->ko_time = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "ko.offset"))
        fightdef_parse_pair_i32(val, &r->ko_offset_x, &r->ko_offset_y);
    else if (fightdef_ieq(key, "ko.anim"))
        r->ko_anim = (u32)fightdef_parse_int(val);
    else if (fightdef_ieq(key, "ko.font"))
        r->ko_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "ko.text"))
    {
        if (alloc)
            r->ko_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "ko.displaytime"))
        r->ko_displaytime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "ko.snd"))
        r->ko_snd = fightdef_parse_snd_ref(val);

    else if (fightdef_ieq(key, "dko.offset"))
        fightdef_parse_pair_i32(val, &r->dko_offset_x, &r->dko_offset_y);
    else if (fightdef_ieq(key, "dko.font"))
        r->dko_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "dko.text"))
    {
        if (alloc)
            r->dko_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "dko.displaytime"))
        r->dko_displaytime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "dko.snd"))
        r->dko_snd = fightdef_parse_snd_ref(val);

    else if (fightdef_ieq(key, "to.offset"))
        fightdef_parse_pair_i32(val, &r->to_offset_x, &r->to_offset_y);
    else if (fightdef_ieq(key, "to.font"))
        r->to_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "to.text"))
    {
        if (alloc)
            r->to_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "to.displaytime"))
        r->to_displaytime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "to.snd"))
        r->to_snd = fightdef_parse_snd_ref(val);

    else if (fightdef_ieq(key, "ko.sndtime"))
        r->ko_sndtime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "slow.time"))
        r->slow_time = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "over.waittime"))
        r->over_waittime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "over.hittime"))
        r->over_hittime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "over.wintime"))
        r->over_wintime = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "over.time"))
        r->over_time = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "win.time"))
        r->win_time = fightdef_parse_int(val);
    else if (fightdef_ieq(key, "win.offset"))
        fightdef_parse_pair_i32(val, &r->win_offset_x, &r->win_offset_y);
    else if (fightdef_ieq(key, "win.font"))
        r->win_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "win.text"))
    {
        if (alloc)
            r->win_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "win.displaytime"))
        r->win_displaytime = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "win2.offset"))
        fightdef_parse_pair_i32(val, &r->win2_offset_x, &r->win2_offset_y);
    else if (fightdef_ieq(key, "win2.font"))
        r->win2_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "win2.text"))
    {
        if (alloc)
            r->win2_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "win2.displaytime"))
        r->win2_displaytime = fightdef_parse_int(val);

    else if (fightdef_ieq(key, "draw.offset"))
        fightdef_parse_pair_i32(val, &r->draw_offset_x, &r->draw_offset_y);
    else if (fightdef_ieq(key, "draw.font"))
        r->draw_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "draw.text"))
    {
        if (alloc)
            r->draw_text = str8_dup_alloc(val, alloc);
    }
    else if (fightdef_ieq(key, "draw.displaytime"))
        r->draw_displaytime = fightdef_parse_int(val);
}

static void parse_winicon_player(Mugen_Fightdef_Winicon_Player* wp, str8 key, str8 val)
{
    if (fightdef_ieq(key, "pos"))
        fightdef_parse_pair_i32(val, &wp->pos_x, &wp->pos_y);
    else if (fightdef_ieq(key, "iconoffset"))
        fightdef_parse_pair_i32(val, &wp->iconoffset_x, &wp->iconoffset_y);
    else if (fightdef_ieq(key, "counter.offset"))
        fightdef_parse_pair_i32(val, &wp->counter_offset_x, &wp->counter_offset_y);
    else if (fightdef_ieq(key, "counter.font"))
        wp->counter_font = fightdef_parse_font_ref(val);
    else if (fightdef_ieq(key, "n.spr"))
        wp->n_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "s.spr"))
        wp->s_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "h.spr"))
        wp->h_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "throw.spr"))
        wp->throw_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "c.spr"))
        wp->c_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "t.spr"))
        wp->t_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "suicide.spr"))
        wp->suicide_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "teammate.spr"))
        wp->teammate_spr = fightdef_parse_spr_ref(val);
    else if (fightdef_ieq(key, "perfect.spr"))
        wp->perfect_spr = fightdef_parse_spr_ref(val);
}
...
bool mugen_fightdef_load(Mugen_Fightdef* out, str8 data, const Mel_Alloc* alloc)
{
    *out = (Mugen_Fightdef){ 0 };
    ...
    while (pos < (usize)data.len)
    {
        ...
        if (line.data[0] == '[')
        {
            if (fightdef_starts_with_i(line, "[files]"))
                section = SEC_FILES;
            else if (fightdef_starts_with_i(line, "[fightfx]"))
                section = SEC_FIGHTFX;
            else if (fightdef_ieq(line, "[lifebar]"))
                section = SEC_LIFEBAR;
            else if (fightdef_ieq(line, "[powerbar]"))
                section = SEC_POWERBAR;
            else if (fightdef_ieq(line, "[face]"))
                section = SEC_FACE;
            else if (fightdef_ieq(line, "[name]"))
                section = SEC_NAME;
            else if (fightdef_ieq(line, "[time]"))
                section = SEC_TIME;
            else if (fightdef_ieq(line, "[combo]"))
                section = SEC_COMBO;
            else if (fightdef_ieq(line, "[round]"))
                section = SEC_ROUND;
            else if (fightdef_ieq(line, "[winicon]"))
                section = SEC_WINICON;
            else if (fightdef_starts_with_i(line, "[begin action"))
                section = SEC_SKIP;
            else if (fightdef_starts_with_i(line, "[simul "))
                section = SEC_SKIP;
            else if (fightdef_starts_with_i(line, "[turns "))
                section = SEC_SKIP;
            else
                section = SEC_NONE;
            continue;
        ...
        if (section == SEC_NONE || section == SEC_SKIP)
            continue;
        ...
        {
        case SEC_FILES:
            if (fightdef_ieq(key, "sff"))
            {
                if (alloc)
                    out->files.sff = str8_dup_alloc(val, alloc);
            }
            else if (fightdef_ieq(key, "snd"))
            {
                if (alloc)
                    out->files.snd = str8_dup_alloc(val, alloc);
            }
            else if (fightdef_ieq(key, "fightfx.sff"))
            {
                if (alloc)
                    out->files.fightfx_sff = str8_dup_alloc(val, alloc);
            }
            else if (fightdef_ieq(key, "fightfx.air"))
            {
                if (alloc)
                    out->files.fightfx_air = str8_dup_alloc(val, alloc);
            }
            else if (fightdef_ieq(key, "common.snd"))
            {
                if (alloc)
                    out->files.common_snd = str8_dup_alloc(val, alloc);
            }
        ...
        case SEC_LIFEBAR:
            parse_lifebar_line(&out->lifebar, key, val);
            break;
        case SEC_POWERBAR:
            parse_powerbar_line(&out->powerbar, key, val);
            break;
        case SEC_FACE:
            parse_face_line(&out->face, key, val);
            break;
        case SEC_NAME:
            parse_name_line(&out->name, key, val);
            break;
        case SEC_TIME:
            parse_time_line(&out->time, key, val);
            break;
        case SEC_COMBO:
            parse_combo_line(&out->combo, key, val, alloc);
            break;
        case SEC_ROUND:
            parse_round_line(&out->round, key, val, alloc);
            break;
        case SEC_WINICON:
            parse_winicon_line(&out->winicon, key, val);
            break;
        }
    }
