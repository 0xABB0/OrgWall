...
static void draw_tile_visual_panel(Mel_EdTiles* ed);
...
void mel_ed_tiles_init(Mel_EdTiles* ed, const Mel_Alloc* alloc)
{
    ...
    *ed = (Mel_EdTiles){0};
    ...
    ed->import_padding = 0;
    ed->import_margin = 0;
    ed->import_preview_texture = nullptr;
...
void mel_ed_tiles_set_tile_visual(Mel_EdTiles* ed, Mel_TileVisual* visual, const char* path)
{
    ...
    ed->tile_visual = visual;
    if (path)
    {
        strncpy(ed->tile_visual_path, path, sizeof(ed->tile_visual_path) - 1);
    }
    else
    {
        ed->tile_visual_path[0] = '\0';
    }
}
...
void mel_ed_tiles_set_tilemap(Mel_EdTiles* ed, Mel_Tilemap* tilemap, const char* path)
{
    ...
    if (path)
    {
        strncpy(ed->tilemap_path, path, sizeof(ed->tilemap_path) - 1);
    }
    else
    {
        ed->tilemap_path[0] = '\0';
    ...
    if (tilemap && tilemap->tile_visual)
    {
        ed->tile_visual = tilemap->tile_visual;
        if (tilemap->tile_visual_path)
        {
            strncpy(ed->tile_visual_path, tilemap->tile_visual_path, sizeof(ed->tile_visual_path) - 1);
        }
    }
}
...
void mel_ed_tiles_draw(Mel_EdTiles* ed)
{
    ...
    {
        if (ed->tilemap && ed->registry && strlen(ed->tilemap_path) > 0)
        {
            if (mel_asset_registry_save_tilemap(ed->registry, ed->tilemap, ed->tilemap_path))
            {
                ed->dirty = false;
                SDL_Log("Saved tilemap: %s", ed->tilemap_path);
            }
        }
        if (ed->tile_visual && ed->registry && strlen(ed->tile_visual_path) > 0)
        {
            mel_asset_registry_save_tile_visual(ed->registry, ed->tile_visual, ed->tile_visual_path);
            SDL_Log("Saved tile visual: %s", ed->tile_visual_path);
        }
    }
}
...
static void draw_tile_visual_panel(Mel_EdTiles* ed)
{
    ...
    const char* visual_label = ed->tile_visual && ed->tile_visual->name ? ed->tile_visual->name : "(none)";
    if (igBeginCombo("##tilevisualcombo", visual_label, 0))
    {
        if (ed->registry)
        {
            u32 count = mel_asset_registry_tile_visual_count(ed->registry);
            for (u32 i = 0; i < count; i++)
            {
                Mel_TileVisual* visual = mel_asset_registry_tile_visual_at(ed->registry, i);
                const char* path = mel_asset_registry_tile_visual_path(ed->registry, i);
                const char* name = visual && visual->name ? visual->name : path;
                bool selected = (visual == ed->tile_visual);

                if (igSelectable_Bool(name, selected, 0, (ImVec2){0, 0}))
                {
                    mel_ed_tiles_set_tile_visual(ed, visual, path);
                }
            }
            if (count == 0)
            {
    ...
    if (ed->tile_visual)
    {
        igText("%u sources, %u tiles", ed->tile_visual->source_count, ed->tile_visual->tile_count);
    }

    if (igButton("New##tilevisual", (ImVec2){60, 0}))
    {
        igOpenPopup_Str("NewTileVisual", 0);
    }
    igSameLine(0, 5);
    if (igButton("Import##tilevisual", (ImVec2){60, 0}))
    {
        ed->show_import_dialog = true;
    }

    if (igBeginPopup("NewTileVisual", 0))
    {
        igText("Create New Tile Visual");
        igInputText("Name##newvisual", ed->new_tile_visual_name, sizeof(ed->new_tile_visual_name), 0, nullptr, nullptr);

        if (igButton("Create##newvisual", (ImVec2){100, 0}))
        {
            if (ed->registry && strlen(ed->new_tile_visual_name) > 0)
            {
                Mel_TileVisual* visual = mel_asset_registry_create_tile_visual(ed->registry, ed->new_tile_visual_name);
                if (visual)
                {
                    char path[256];
...
static void draw_tile_palette(Mel_EdTiles* ed)
{
    ...
    if (!ed->tile_visual || ed->tile_visual->tile_count == 0)
    {
    ...
    if (igBeginChild_Str("TilePalette", (ImVec2){0, 200}, ImGuiChildFlags_Borders, ImGuiWindowFlags_HorizontalScrollbar))
    {
        ...
        for (u32 i = 0; i < ed->tile_visual->tile_count; i++)
        {
            u32 tx = i % cols;
            u32 ty = i / cols;
            ...
            Mel_TileEntry* entry = &ed->tile_visual->tiles[i];
            ...
            if (entry->source_idx < ed->tile_visual->source_count)
            {
                Mel_TileSource* source = &ed->tile_visual->sources[entry->source_idx];
                if (source->texture && source->texture->descriptor)
                {
                    f32 tex_w = (f32)source->texture->image.width;
                    f32 tex_h = (f32)source->texture->image.height;
                    ...
                    ImTextureRef_c tex_ref = {._TexData = nullptr, ._TexID = (ImTextureID)source->texture->descriptor};
        ...
        if (igIsWindowHovered(0) && io->MouseClicked[0])
        {
            ...
            if (rel_x >= 0 && rel_y >= 0)
            {
                ...
                if (idx < ed->tile_visual->tile_count)
                {
                    mel_ed_tiles_brush_set_single(ed, (i32)ed->tile_visual->tiles[idx].id);
                }
            }
        }

        u32 rows = (ed->tile_visual->tile_count + cols - 1) / cols;
...
{
    if (!brush || !brush->tiles || brush->width == 0 || brush->height == 0 || !ed->tile_visual)
    {
    ...
    for (u32 by = 0; by < brush->height; by++)
    {
        for (u32 bx = 0; bx < brush->width; bx++)
        {
            ...
            Mel_TileEntry* entry = mel_tile_visual_get_tile(ed->tile_visual, (u32)tile_id);
            if (entry && entry->source_idx < ed->tile_visual->source_count)
            {
                Mel_TileSource* source = &ed->tile_visual->sources[entry->source_idx];
                if (source->texture && source->texture->descriptor)
                {
...
static void draw_brush_panel(Mel_EdTiles* ed)
{
    ...
    if (ed->brush.history_count > 0)
    {
        ...
        for (u32 i = 0; i < ed->brush.history_count && i < 6; i++)
        {
            ...
            if (hist->tiles && hist->width > 0 && hist->height > 0 && ed->tile_visual)
            {
...
static void draw_tilemap_panel(Mel_EdTiles* ed)
{
    ...
    const char* tilemap_label = ed->tilemap && ed->tilemap->name ? ed->tilemap->name : "(none)";
    if (igBeginCombo("##tilemapcombo", tilemap_label, 0))
    {
        if (ed->registry)
        {
            u32 count = mel_asset_registry_tilemap_count(ed->registry);
            for (u32 i = 0; i < count; i++)
            {
                Mel_Tilemap* tilemap = mel_asset_registry_tilemap_at(ed->registry, i);
                const char* path = mel_asset_registry_tilemap_path(ed->registry, i);
                const char* name = tilemap && tilemap->name ? tilemap->name : path;
                bool selected = (tilemap == ed->tilemap);

                if (igSelectable_Bool(name, selected, 0, (ImVec2){0, 0}))
                {
                    mel_ed_tiles_set_tilemap(ed, tilemap, path);
                }
            }
            if (count == 0)
            {
    ...
    if (igBeginPopup("NewTilemap", 0))
    {
        ...
        {
            if (ed->registry && strlen(ed->new_tilemap_name) > 0)
            {
                Mel_Tilemap* tilemap = mel_asset_registry_create_tilemap(ed->registry,
                    ed->new_tilemap_name,
                ...
                if (tilemap)
                {
                    if (ed->tile_visual)
                    {
                        tilemap->tile_visual = ed->tile_visual;
                    ...
                    char path[256];
                    snprintf(path, sizeof(path), "%s.json", ed->new_tilemap_name);
                    mel_ed_tiles_set_tilemap(ed, tilemap, path);
...
static void draw_map_canvas(Mel_EdTiles* ed)
{
    ...
    if (igBeginChild_Str("MapCanvas", (ImVec2){0, 0}, ImGuiChildFlags_Borders, ImGuiWindowFlags_HorizontalScrollbar))
    {
        ...
        {
            Mel_TilemapLayer* layer = &ed->tilemap->layers[l];
            ...
            for (u32 y = 0; y < map_h; y++)
            {
                ...
                {
                    i32 tile_id = layer->tiles[y * map_w + x];
                    if (tile_id < 0) continue;
        ...
        if (igIsWindowHovered(0))
        {
            ...
            if (ed->selection.active && !io->MouseDown[0])
            {
                ...
                if (io->KeyCtrl && igIsKeyPressed_Bool(ImGuiKey_X, false))
                {
                    ...
                    {
                        Mel_TilemapLayer* layer = &ed->tilemap->layers[ed->selected_layer];
                        for (i32 cy = sy; cy <= ey; cy++)
                        {
                            for (i32 cx = sx; cx <= ex; cx++)
                            {
                                ...
                                {
                                    layer->tiles[cy * ed->tilemap->width + cx] = -1;
                                }
                            }
                        }
                        ed->dirty = true;
...
static void draw_layer_panel(Mel_EdTiles* ed)
{
    ...
    if (igButton("-##dellayer", (ImVec2){25, 0}))
    {
        if (ed->selected_layer >= 0 && (u32)ed->selected_layer < ed->tilemap->layer_count)
        {
            mel_tilemap_remove_layer(ed->tilemap, (u32)ed->selected_layer);
    ...
    if (igButton("^##layerup", (ImVec2){25, 0}))
    {
        ...
        {
            mel_tilemap_move_layer(ed->tilemap, (u32)ed->selected_layer, (u32)(ed->selected_layer - 1));
            ed->selected_layer--;
            ed->dirty = true;
        }
    }
    ...
    if (igBeginPopup("AddLayer", 0))
    {
        ...
        {
            mel_tilemap_add_layer(ed->tilemap, ed->new_layer_name);
            ed->dirty = true;
            igCloseCurrentPopup();
        }
        igEndPopup();
    ...
    if (igBeginChild_Str("LayerList", (ImVec2){0, 200}, ImGuiChildFlags_Borders, 0))
    {
        ...
        {
            Mel_TilemapLayer* layer = &ed->tilemap->layers[i];
            ...
            {
                char label[128];
                snprintf(label, sizeof(label), "%s###layer", layer->name ? layer->name : "(unnamed)");
                ...
                if (igSelectable_Bool(label, ed->selected_layer == (i32)i, ImGuiSelectableFlags_AllowDoubleClick, (ImVec2){0, 0}))
                {
                    ...
                    if (igIsMouseDoubleClicked_Nil(0))
                    {
                        ...
                        strncpy(ed->rename_layer_buffer, layer->name ? layer->name : "", sizeof(ed->rename_layer_buffer) - 1);
                    }
                }
            }
...
static void draw_import_dialog(Mel_EdTiles* ed)
{
    ...
    if (igBegin("Import Tile Sheet", &ed->show_import_dialog, 0))
    {
        ...
        const char* preview_label = strlen(ed->import_texture_path) > 0 ? ed->import_texture_path : "(select texture)";
        if (igBeginCombo("##texturecombo", preview_label, 0))
        {
            if (ed->registry)
            {
                u32 tex_count = mel_asset_registry_texture_count(ed->registry);
                for (u32 i = 0; i < tex_count; i++)
                {
                    const char* path = mel_asset_registry_texture_path(ed->registry, i);
                    bool selected = strcmp(path, ed->import_texture_path) == 0;
                    if (igSelectable_Bool(path, selected, 0, (ImVec2){0, 0}))
                    {
                        strncpy(ed->import_texture_path, path, sizeof(ed->import_texture_path) - 1);
                        ed->import_preview_texture = mel_asset_registry_load_texture(ed->registry, path);
                    }
                }
                if (tex_count == 0)
                {
        ...
        {
            if (ed->registry && strlen(ed->import_texture_path) > 0)
            {
                if (!ed->tile_visual)
                {
                    ed->tile_visual = mel_asset_registry_create_tile_visual(ed->registry, "imported");
                    strcpy(ed->tile_visual_path, "imported.json");
                }

                Mel_TileSource* source = mel_tile_visual_add_source(ed->tile_visual, ed->import_texture_path, true);
                if (source)
                {
                    ...
                    source->texture = ed->import_preview_texture ? ed->import_preview_texture :
                        mel_asset_registry_load_texture(ed->registry, ed->import_texture_path);
                    if (source->texture)
                    {
                        ...
                        {
                            source->columns = source->texture->image.width / source->tile_width;
                        }
                        ...
                        {
                            source->rows = source->texture->image.height / source->tile_height;
                        }
                    }

                    mel_tile_visual_regenerate_tiles(ed->tile_visual);
                    ed->dirty = true;
                }
        ...
        igText("Preview");
        if (ed->import_preview_texture && ed->import_preview_texture->descriptor)
        {
            f32 tex_w = (f32)ed->import_preview_texture->image.width;
            f32 tex_h = (f32)ed->import_preview_texture->image.height;
            ...
            ImTextureRef_c tex_ref = {._TexData = nullptr, ._TexID = (ImTextureID)ed->import_preview_texture->descriptor};
        ...
        {
            igTextDisabled("Enter path and click Preview");
        }
...
void mel_ed_tiles_brush_from_selection(Mel_EdTiles* ed)
{
    ...
    for (u32 y = 0; y < height; y++)
    {
        for (u32 x = 0; x < width; x++)
        {
            u32 map_x = (u32)sx + x;
            u32 map_y = (u32)sy + y;
            ed->brush.current.tiles[y * width + x] = layer->tiles[map_y * ed->tilemap->width + map_x];
        }
    }
    ...
    for (u32 by = 0; by < bh; by++)
    {
        for (u32 bx = 0; bx < bw; bx++)
        {
            ...
            if (map_x >= 0 && map_y >= 0 && (u32)map_x < ed->tilemap->width && (u32)map_y < ed->tilemap->height)
            {
                ...
                if (tile >= 0)
                {
                    mel_tilemap_set_tile(ed->tilemap, (u32)ed->selected_layer, (u32)map_x, (u32)map_y, tile);
                    ed->dirty = true;
                }
            }
        }
...
void mel_ed_tiles_erase(Mel_EdTiles* ed, i32 x, i32 y)
{
    ...
    {
        mel_tilemap_set_tile(ed->tilemap, (u32)ed->selected_layer, (u32)x, (u32)y, -1);
        ed->dirty = true;
    }
}
...
void mel_ed_tiles_fill(Mel_EdTiles* ed, i32 x, i32 y)
{
    ...
    i32 target_tile = mel_tilemap_get_tile(ed->tilemap, (u32)ed->selected_layer, (u32)x, (u32)y);
    ...
    while (stack_size > 0)
    {
        ...
        if (mel_tilemap_get_tile(ed->tilemap, (u32)ed->selected_layer, (u32)cx, (u32)cy) != target_tile)
            continue;
        ...
        if (fill_tile >= 0)
        {
            mel_tilemap_set_tile(ed->tilemap, (u32)ed->selected_layer, (u32)cx, (u32)cy, fill_tile);
            ed->dirty = true;
        }
