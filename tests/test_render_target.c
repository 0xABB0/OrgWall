...
MEL_TEST(render_target_accessors_direct, .tags = "render")
{
    Mel_Render_Target t = {0};
    t.type   = MEL_TARGET_OFFSCREEN;
    t.width  = 1920;
    t.height = 1080;
    t.format = MEL_GPU_FORMAT_R8G8B8A8_UNORM;

    MEL_ASSERT_EQ(mel_render_target_width(&t),  1920u);
    MEL_ASSERT_EQ(mel_render_target_height(&t), 1080u);
    MEL_ASSERT_EQ(mel_render_target_format(&t), (Mel_Gpu_Format)MEL_GPU_FORMAT_R8G8B8A8_UNORM);
}

MEL_TEST(render_target_image_view_null_when_not_set, .tags = "render")
{
    Mel_Render_Target t = {0};
    t.type = MEL_TARGET_OFFSCREEN;
    MEL_ASSERT_NULL(mel_render_target_image_view(&t));
}
...
MEL_TEST(render_target_offscreen_create_destroy, .tags = "render, visual")
{
    ...
    Mel_Render_Target* target = mel_render_target_offscreen(
        .width  = 512,
        .height = 512,
    ...
    mel_render_target_destroy(target);
}
