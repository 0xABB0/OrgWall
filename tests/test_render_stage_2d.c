...
typedef struct {
    bool acquire_result;
    bool present_result;
} Stage2D_Mock_Swapchain;

static bool stage2d_mock_acquire(Mel_Swapchain* sc, Mel_Gpu_Device* dev)
{
    (void)sc;
    (void)dev;
    return true;
...
static bool stage2d_mock_present(Mel_Swapchain* sc, Mel_Gpu_Device* dev, VkCommandBuffer cmd, VkFence fence)
{
    (void)sc;
    (void)dev;
    (void)cmd;
...
static const Mel_Swapchain_Vtable s_stage2d_mock_vtable = {
    .acquire = stage2d_mock_acquire,
    .present = stage2d_mock_present,
    .resize = stage2d_mock_resize,
    .shutdown = stage2d_mock_shutdown,
};

static Mel_Swapchain_Handle make_stage2d_mock_swapchain(void)
{
    Stage2D_Mock_Swapchain* mock = mel_alloc_type(mel_alloc_heap(), Stage2D_Mock_Swapchain);
    *mock = (Stage2D_Mock_Swapchain){0};
...
MEL_TEST(render_stage_2d_builds_named_world_hud_layers_without_manual_recipe_wiring, .tags = "render")
{
    Mel_Swapchain_Handle swapchain = make_stage2d_mock_swapchain();
    ...
    bool ok = mel_render_stage_2d_init(&stage,
        .name = S8("stage2d"),
        .swapchain = swapchain,
        .world_camera = &camera,
    ...
    MEL_ASSERT(mel_view_handle_valid(mel_render_stage_2d_view(&stage, MEL_RENDER_STAGE_2D_LAYER_WORLD)));
    MEL_ASSERT(mel_view_handle_valid(mel_render_stage_2d_view(&stage, MEL_RENDER_STAGE_2D_LAYER_HUD)));
    MEL_ASSERT(mel_view_handle_valid(mel_render_stage_2d_view(&stage, MEL_RENDER_STAGE_2D_LAYER_DEBUG)));
    MEL_ASSERT(mel_view_handle_valid(mel_render_stage_2d_view(&stage, MEL_RENDER_STAGE_2D_LAYER_UI)));
    MEL_ASSERT(mel_frame_recipe_handle_valid(mel_render_stage_2d_recipe(&stage)));
    MEL_ASSERT(mel_frame_plan_handle_valid(mel_render_stage_2d_plan(&stage)));
    MEL_ASSERT(mel_render_stage_2d_renderer(&stage) != nullptr);
...
static void stage2d_test_widget_clicked(void* user_data)
{
    bool* clicked = user_data;
    *clicked = true;
}

MEL_TEST(render_stage_2d_ui_widget_layer_renders_on_ui_layer_and_processes_events, .tags = "render")
{
    Mel_Swapchain_Handle swapchain = make_stage2d_mock_swapchain();
    ...
    button.on_click = stage2d_test_widget_clicked;
    ...
    Mel_Render_Stage_2D_Widget_Layer layer;
    ok = mel_render_stage_2d_widget_layer_init(&stage, &layer,
        .name = S8("ui_widgets"),
        .root = &root.base,
    ...
    mel_render_stage_2d_widget_layer_process_event(&layer, &event);
    ...
    mel_render_stage_2d_widget_layer_shutdown(&stage, &layer);
