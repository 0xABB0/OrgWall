...
- ~~[ENGINE] the game should automatically automatically handle simulations~~ DONE: Full simulation system implemented. `Mel_Sim_Ctx` with fixed contexts (`Mel_Sim_Fixed`), variable updates, time scale, user data, `mel_sim_tick`. Engine registration via `mel_engine_register_sim`/`mel_engine_unregister_sim` (intrusive linked list). `mel_engine_frame` iterates registered sims and calls `mel_sim_tick`. `on_update` callback removed entirely — all frame logic goes through sims. All 9 examples + game/main.c migrated. Remaining: deferred mutations (add/remove during callbacks)
